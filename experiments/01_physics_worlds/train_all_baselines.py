...
class GFlowNetExploration(BaselineModel):
    ...
    def build_model(self):
        ...
        self.predictor = keras.Model(
            inputs=[state_input, param_input],
            outputs=output,
            name='predictor'
        )
    ...
    def train(self, train_data, val_data, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            for task_X, task_y in tasks:
                ...
                epoch_mae += float(keras.ops.mean(keras.ops.abs(y_val[:100] - val_pred)))
...
def main():
    ...
    for name, res in results.items():
        ...
        print(f"{name:<15} {res['overall_mse']:>12.4f} {in_dist:>12.4f} {near_ood:>12.4f} {far_ood:>12.4f}")
    ...
    report.append("| Model | Overall MSE | In-Dist MSE | Near-OOD MSE | Far-OOD MSE | Training Time |")
    report.append("|-------|-------------|-------------|--------------|-------------|---------------|")
    ...
    for name, res in results.items():
        ...
        report.append(f"| {name} | {res['overall_mse']:.4f} | {in_dist:.4f} | "
    ...
    report.append(f"- **Best far-OOD extrapolation**: {best_far_ood} (MSE: {best_far_ood_score:.4f})")
    ...
    for name, res in results.items():
        ...
        if in_dist > 0:
            ...
            report.append(f"- {name}: {degradation:.1f}% increase from in-dist to far-OOD")
