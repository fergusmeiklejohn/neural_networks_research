...
# ERM baseline
erm_baseline_mean, erm_baseline_ci = calculate_ci(two_ball_results['ERM']['baseline_mse'])
...
tta_deg_factor, (tta_deg_lower, tta_deg_upper) = calculate_degradation_ci(
    two_ball_results['TTA']['ood_mse'],
    two_ball_results['TTA']['tta_mse']
)

print(f"\n### Test-Time Adaptation")
print(f"TTA MSE: {tta_mean:.5f} ± {tta_ci:.5f} (95% CI)")
print(f"Degradation vs baseline: {tta_degradation:.2f}x (95% CI: {tta_deg_lower:.2f}-{tta_deg_upper:.2f})")
print(f"Percentage increase: {(tta_degradation - 1) * 100:.0f}%")

# Statistical test
t_stat, p_value = paired_t_test(two_ball_results['TTA']['ood_mse'], two_ball_results['TTA']['tta_mse'])
print(f"Paired t-test (baseline vs TTA): t={t_stat:.3f}, p={p_value:.4f}")

# MAML results
maml_baseline_mean, maml_baseline_ci = calculate_ci(two_ball_results['MAML']['baseline_mse'])
...
print(f"Degradation: {(maml_tta_mean / maml_baseline_mean):.0f}x ({((maml_tta_mean / maml_baseline_mean - 1) * 100):.0f}% increase)")

print("\n## Pendulum System Results")
print("-" * 60)

# ERM baseline for pendulum
pend_erm_baseline_mean, pend_erm_baseline_ci = calculate_ci(pendulum_results['ERM']['baseline_mse'])
pend_erm_ood_mean, pend_erm_ood_ci = calculate_ci(pendulum_results['ERM']['ood_mse'])
pend_erm_degradation, (pend_erm_deg_lower, pend_erm_deg_upper) = calculate_degradation_ci(
    pendulum_results['ERM']['baseline_mse'],
    pendulum_results['ERM']['ood_mse']
)

print(f"\n### ERM Baseline")
print(f"Fixed length MSE: {pend_erm_baseline_mean:.6f} ± {pend_erm_baseline_ci:.6f} (95% CI)")
print(f"Time-varying MSE: {pend_erm_ood_mean:.6f} ± {pend_erm_ood_ci:.6f} (95% CI)")
print(f"Degradation: {pend_erm_degradation:.2f}x (95% CI: {pend_erm_deg_lower:.2f}-{pend_erm_deg_upper:.2f})")

# Physics-aware TTA variants
for variant, name in [('Prediction_TTA', 'Prediction Consistency'), 
    ...
    t_stat, p_value = paired_t_test(
        pendulum_results[variant]['baseline_mse'], 
        pendulum_results[variant]['tta_mse']
    )
    print(f"Paired t-test: t={t_stat:.3f}, p={p_value:.4f}")

# PeTTA results
petta_standard_mean, petta_standard_ci = calculate_ci(pendulum_results['PeTTA']['standard_tta_mse'])
