...
class ExtremeOODPhysicsGenerator:
    ...
    def generate_magnetic_field_trajectory(self, B=0.001, charge=1.0, n_steps=50):
        ...
        for t in range(n_steps):
            ...
            # Handle collisions
            for pos, vel in [(pos1, vel1), (pos2, vel2)]:
                if pos[0] <= self.ball_radius or pos[0] >= self.width - self.ball_radius:
                    vel[0] *= -0.8
                    pos[0] = np.clip(pos[0], self.ball_radius, self.width - self.ball_radius)
                if pos[1] <= self.ball_radius or pos[1] >= self.height - self.ball_radius:
                    vel[1] *= -0.8
                    pos[1] = np.clip(pos[1], self.ball_radius, self.height - self.ball_radius)
    ...
    def generate_dataset(self, physics_type, n_trajectories=100, **kwargs):
        ...
        for i in range(n_trajectories):
            ...
            if physics_type == 'rotating_frame':
                ...
                rest_length = kwargs.get('rest_length', 200.0) * (0.8 + 0.4 * np.random.random())
...
def main():
    ...
    for physics_type, config in configs.items():
        ...
        # Quick stats
        print(f"  Avg displacement: {np.mean([np.linalg.norm(t[-1, :2] - t[0, :2]) for t in trajectories]):.1f} pixels")
