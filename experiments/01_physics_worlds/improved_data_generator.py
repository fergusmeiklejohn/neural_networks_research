...
NOVEL_REGIMES = {
    ...
    'space_station': {'gravity': -20, 'friction': 0.8, 'elasticity': 0.7, 'damping': 1.0},
    'thick_atmosphere': {'gravity': -981, 'friction': 0.6, 'elasticity': 0.8, 'damping': 0.75},
    'super_earth': {'gravity': -1962, 'friction': 0.4, 'elasticity': 0.7, 'damping': 0.92}
}
...
class ImprovedPhysicsDataGenerator:
    ...
    def _calculate_split_sizes(self) -> Dict[str, int]:
        ...
        return {
            ...
            'test_interpolation': int(total * self.config.test_interpolation_proportion),
            'test_extrapolation': int(total * self.config.test_extrapolation_proportion),
    ...
    def _generate_physics_config_random(self, ranges: Dict[str, Tuple[float, float]]) -> PhysicsConfig:
    ...
    def _generate_physics_config_grid(self, ranges: Dict[str, Tuple[float, float]], 
                                     num_samples: int) -> List[PhysicsConfig]:
    ...
    def _generate_physics_config_extrapolation(self, ranges: Dict[str, List[Tuple[float, float]]], 
                                             num_samples: int) -> List[PhysicsConfig]:
    ...
    def _generate_physics_config_novel(self, regimes: Dict[str, Dict], 
                                     num_samples: int) -> List[PhysicsConfig]:
    ...
    def _generate_single_sample(self, sample_id: int, physics_config: PhysicsConfig, 
    ...
    def generate_split_dataset(self, split_name: str) -> str:
        ...
        print(f"Generating {split_name} dataset ({self.split_sizes[split_name]} samples)...")
        ...
        if split_def['sampling'] == 'random_uniform':
            ...
            physics_configs = self._generate_physics_config_grid(split_def['ranges'], num_samples)
        elif split_def['sampling'] == 'systematic_extrapolation':
            physics_configs = self._generate_physics_config_extrapolation(split_def['ranges'], num_samples)
        elif split_def['sampling'] == 'predefined_regimes':
            physics_configs = self._generate_physics_config_novel(split_def['regimes'], num_samples)
        else:
        ...
        for sample_id, physics_config in enumerate(tqdm(physics_configs, desc=f"Generating {split_name}")):
            try:
                sample = self._generate_single_sample(sample_id, physics_config, split_name)
        ...
        print(f"Successfully generated {len(samples)} {split_name} samples ({failed_samples} failed)")
    ...
    def _generate_isolation_report(self, datasets: Dict[str, str]):
        ...
        train_coverage = report['datasets'].get('train', {}).get('parameter_coverage', {})
        ...
        for test_split in ['test_interpolation', 'test_extrapolation', 'test_novel']:
            if test_split in report['datasets']:
                ...
                overlap_analysis = self._analyze_parameter_overlap(train_coverage, test_coverage)
                report['isolation_verification'][f'train_vs_{test_split}'] = overlap_analysis
    ...
    def _analyze_parameter_overlap(self, train_coverage: Dict, test_coverage: Dict) -> Dict:
        ...
        for param in ['gravity', 'friction', 'elasticity', 'damping']:
            if param in train_coverage and param in test_coverage:
                train_min, train_max = train_coverage[param]['min'], train_coverage[param]['max']
                test_min, test_max = test_coverage[param]['min'], test_coverage[param]['max']
                ...
                if has_overlap:
                    overlap_fraction = (overlap_max - overlap_min) / (test_max - test_min)
                else:
                ...
                overlap_analysis[param] = {
                    ...
                    'extrapolation_direction': 'both' if test_min < train_min and test_max > train_max 
                                             else 'lower' if test_max < train_min
                                             else 'higher' if test_min > train_max
                                             else 'interpolation'
                }
