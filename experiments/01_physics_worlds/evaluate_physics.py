...
import keras
from models.core.distribution_inventor import DistributionInventor
from experiments.physics_worlds.data_generator import load_physics_dataset
from experiments.physics_worlds.physics_env import PhysicsWorld, PhysicsConfig, Ball
...
class PhysicsEvaluator:
    ...
    def evaluate_rule_extraction(self, test_data: List[Dict], sample_size: int = 100) -> Dict[str, float]:
        ...
        for i, sample in enumerate(tqdm(test_data[:sample_size], desc="Rule extraction")):
            try:
        ...
        for rule, errors in extraction_errors.items():
            if errors:
                ...
                extraction_metrics[f'{rule}_accuracy_10%'] = np.mean(np.array(errors) < 0.1)
                extraction_metrics[f'{rule}_accuracy_20%'] = np.mean(np.array(errors) < 0.2)
        
        return extraction_metrics
    
    def evaluate_modification_consistency(self, test_data: List[Dict], sample_size: int = 50) -> Dict[str, float]:
        ...
        for i, sample in enumerate(tqdm(test_data[:sample_size], desc="Modification consistency")):
            try:
                trajectory = np.array(sample['trajectory'])
                    ...
                    if result['success']:
                        ...
                        if isinstance(expected_change, float):
                            if expected_change > 1.0:  # Should increase
                                directional_accuracy.append(modified_value > original_value)
        ...
        return {
            ...
            'success_rate': len(consistency_scores) / (sample_size * len(modification_tests))
        }
    
    def evaluate_trajectory_quality(self, test_data: List[Dict], sample_size: int = 50) -> Dict[str, float]:
        ...
        for i, sample in enumerate(tqdm(test_data[:sample_size], desc="Trajectory quality")):
            try:
                trajectory = np.array(sample['trajectory'])
                ...
                if result['success']:
                    ...
                    energy_conservation = self._check_energy_conservation(generated_traj)
                    ...
                    plausibility = self._check_physical_plausibility(generated_traj, physics_config)
    ...
    def evaluate_distribution_space(self, test_data: List[Dict], sample_size: int = 30) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        if 'quality_scores' in result:
                            novelty_scores.append(result['quality_scores'].get('modification_novelty', 0))
    ...
    def _check_physical_plausibility(self, trajectory: np.ndarray, physics_config: Dict) -> float:
    ...
    def _create_visualizations(self, output_dir: str):
        ...
        if 'rule_extraction' in self.results:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, "rule_extraction_accuracy.png"), dpi=300)
            plt.close()
        ...
        if 'distribution_space' in self.results:
            ...
            if success_rates:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, "modification_success_rates.png"), dpi=300)
                plt.close()
        
        print(f"Visualizations saved to {output_dir}")
    
    def run_full_evaluation(self, test_data_path: str, sample_sizes: Dict[str, int] = None) -> Dict[str, Any]:
        ...
        self.results['modification_consistency'] = self.evaluate_modification_consistency(
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Physics Distribution Inventor')
