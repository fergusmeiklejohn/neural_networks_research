...
def main():
    ...
    non_trainable_vars = [ops.convert_to_numpy(v) for v in model.non_trainable_variables]
    ...
    print(f"   First values: {y_adapted[0, 0, :4] if y_adapted.ndim > 2 else y_adapted[0, :4]}")
    ...
    if np.allclose(y_adapted, 0):
        ...
        print(f"   First layer weight range: [{weights.min():.4f}, {weights.max():.4f}]")
