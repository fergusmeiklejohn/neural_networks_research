...
from data_generator import DataConfig, PhysicsDataGenerator, generate_physics_datasets
...
def generate_quick_datasets():
    ...
    val_config = DataConfig(
        num_samples=50, sequence_length=200, output_dir="data/processed/physics_worlds"
    )
    ...
    test_config = DataConfig(
        num_samples=25, sequence_length=200, output_dir="data/processed/physics_worlds"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate complete Physics Worlds dataset"
    )
    parser.add_argument(
        "--quick",
        action="store_true",
