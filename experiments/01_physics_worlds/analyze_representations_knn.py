...
from baseline_models import GFlowNetBaseline, MAMLBaseline, ERMWithAugmentation, GraphExtrapolationBaseline
...
class KNNRepresentationAnalyzer:
    ...
    def analyze_ood_with_knn(self, train_reps, test_reps, test_labels):
        ...
        results['overall'] = {
            ...
            'pct_within_95': float(100 * np.sum(mean_distances <= threshold_95) / len(mean_distances)),
            'pct_within_99': float(100 * np.sum(mean_distances <= threshold_99) / len(mean_distances))
        }
        
        return results, mean_distances
    
    def plot_distance_distribution(self, distances, labels, threshold_95, threshold_99, save_path):
        ...
        ax.axhline(y=threshold_99, color='red', linestyle='--', 
...
def run_knn_analysis():
    ...
    # Load test data
    import pickle
    with open('data/processed/physics_worlds_v2/test_interpolation_data.pkl', 'rb') as f:
        test_interp_data = pickle.load(f)
    with open('data/processed/physics_worlds_v2/test_extrapolation_data.pkl', 'rb') as f:
    ...
    for model_name, model_path in models.items():
        ...
        try:
            ...
            analyzer.plot_distance_distribution(
                distances, test_labels,
                results['threshold_95'], results['threshold_99'],
                f'outputs/baseline_results/{model_name}_knn_distances.png'
            )
            
            # Print summary
            print(f"Results for {model_name}:")
            print(f"  Within 95th percentile: {results['overall']['pct_within_95']:.1f}%")
            print(f"  Within 99th percentile: {results['overall']['pct_within_99']:.1f}%")
            print(f"  Beyond 99th percentile: {100 - results['overall']['pct_within_99']:.1f}%")
