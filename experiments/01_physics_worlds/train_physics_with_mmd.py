...
class PhysicsMMDTrainer:
    ...
    def compute_total_loss(
        ...
        initial_conditions: Optional[keras.KerasTensor] = None,
        ...
        if "energy" in loss_components:
            total_loss += (
                self.physics_loss_weights["energy"] * loss_components["energy"]
            )

        if "momentum" in loss_components:
            total_loss += (
                self.physics_loss_weights["momentum"] * loss_components["momentum"]
            )
...
def create_physics_model_with_mmd(
    input_shape: Tuple[int, ...], output_shape: Tuple[int, ...], hidden_dim: int = 128
) -> keras.Model:
    """
...
if __name__ == "__main__":
    ...
    for mmd_weight in [0.0, 0.1, 0.5]:
        ...
        history = train_with_mmd(
            (X_train, y_train), (X_val, y_val), epochs=10, mmd_weight=mmd_weight
        )
