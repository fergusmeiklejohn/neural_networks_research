...
from distribution_modifier import ModificationDataProcessor, DistributionModifier


def main():
    """Evaluate the trained model."""
    print("Loading trained model...")
    model = keras.models.load_model('outputs/checkpoints/distribution_modifier_best.keras')
    ...
    for test in test_cases:
        ...
        for mod_desc in test['modifications']:
            ...
            # Predict
            pred_params, mod_factors, change_mask = model([base_params, desc_tokens], training=False)
            ...
            for i, name in enumerate(processor.param_names):
                ...
                if mask > 0.3:  # Parameter likely changed
                    ...
                    print(f"  {name:12s}: {original:.3f} -> {predicted:.3f} "
    ...
    for param_type, keywords in modification_types.items():
        ...
        for keyword in keywords:
            for direction in ['increase', 'decrease', 'more', 'less']:
                ...
                pred_params, _, change_mask = model([test_params, desc_tokens], training=False)
