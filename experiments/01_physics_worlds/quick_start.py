...
from models.core.distribution_inventor import create_distribution_inventor, DistributionInventorConfig
...
def demo_model_components():
    ...
    for rule in test_rules.keys():
        if rule in modified:
            ...
            mod_val = modified[rule].item() if isinstance(modified[rule], np.ndarray) else modified[rule]
    ...
    generated = generator.generate_trajectory(initial_conditions, physics_rules, sequence_length=50)
        ...
        try:
            ...
            for rule in ['gravity', 'friction', 'elasticity', 'damping']:
                if rule in orig_rules and rule in mod_rules:
                    orig_val = orig_rules[rule].item() if isinstance(orig_rules[rule], np.ndarray) else orig_rules[rule]
                    mod_val = mod_rules[rule].item() if isinstance(mod_rules[rule], np.ndarray) else mod_rules[rule]
                    change = (mod_val - orig_val) / abs(orig_val) * 100 if abs(orig_val) > 1e-6 else 0
                    print(f"    {rule}: {orig_val:.3f} → {mod_val:.3f} ({change:+.1f}%)")
...
def main():
    ...
    try:
        ...
        print("=" * 60)
        print("\nNext steps:")
        print("1. Generate full dataset: python experiments/01_physics_worlds/data_generator.py")
        print("2. Train model: python experiments/01_physics_worlds/train_physics.py")
        print("3. Evaluate model: python experiments/01_physics_worlds/evaluate_physics.py")
        print("\nFor detailed training:")
        print("python experiments/01_physics_worlds/train_physics.py --epochs 50 --batch_size 16")
