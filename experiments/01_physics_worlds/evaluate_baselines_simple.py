...
def test_gflownet_inspired_baseline(train_data, val_data, test_data_dict):
    ...
    for test_name, (X_test, y_test) in test_data_dict.items():
        ...
        print(f"  Prediction uncertainty (std): {results[test_name]['prediction_std']:.4f}")
...
def main():
    ...
    gflownet_results = test_gflownet_inspired_baseline(train_data, val_data, test_data_dict)
    ...
    print(f"MAML-inspired (no adapt)        | {maml_base:7.1f} | {maml_base_change:+.1f}%")
    
    # MAML adapted
    maml_adapted = maml_results.get('time_varying_adapted', float('inf'))
    maml_adapted_change = (maml_adapted / baseline_mse - 1) * 100
    print(f"MAML-inspired (10-shot adapt)   | {maml_adapted:7.1f} | {maml_adapted_change:+.1f}%")
