        ...
        if 'physics_config' in sample:
            ...
            if gravity_ms2 < -20.0:
                ...
                if traj_array.shape[1] >= 17:
                    ...
                    jupiter_params.append([gravity_ms2, config.get('friction', 0.1), config.get('elasticity', 0.8)])
...
class GFlowNetBaseline(keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        if training:
            noise = keras.random.normal(shape=ops.shape(encoded), 
...
def train_baseline(model_name, X_train, X_val, X_test_jupiter):
    ...
    print("\nTraining...")
    history = model.fit(
        X_train, X_train,  # Autoencoder style
...
def main():
    parser = argparse.ArgumentParser(description='Train baseline models on physics data')
    parser.add_argument('--model', type=str, choices=['gflownet', 'maml', 'simple'], 
                        default='gflownet', help='Which baseline to train')
