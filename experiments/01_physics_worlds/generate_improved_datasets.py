...
from improved_data_generator import generate_improved_physics_datasets, ImprovedDataConfig, ImprovedPhysicsDataGenerator
...
def analyze_improved_datasets(datasets_path: Dict[str, str]):
    ...
    for split_name, file_path in datasets_path.items():
        ...
        for param_name, param_stats in stats.items():
            print(f"  {param_name:>10}: [{param_stats['min']:>7.1f}, {param_stats['max']:>7.1f}] (μ={param_stats['mean']:.1f}, σ={param_stats['std']:.1f})")
...
def compare_old_vs_new_approach():
    ...
    if old_train_path.exists() and new_train_path.exists():
        ...
        print(f"Comparing {len(old_data)} old samples vs {len(new_data)} new samples...")
...
def main():
    parser = argparse.ArgumentParser(description='Generate improved Physics Worlds datasets with proper isolation')
    ...
    if not args.analyze_only:
        try:
            ...
            dataset_stats = analyze_improved_datasets(datasets)
            
            # Compare with old approach if available
            compare_old_vs_new_approach()
            
            # Save generation summary
            summary_path = Path(f"data/processed/physics_worlds_v2{'_quick' if args.quick else ''}/generation_summary.txt")
            with open(summary_path, 'w') as f:
                f.write(f"Improved Physics Worlds Data Generation Summary\n")
                f.write(f"Generated on: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Mode: {'Quick' if args.quick else 'Full'}\n")
                f.write(f"Generation time: {hours:02d}h {minutes:02d}m {seconds:02d}s\n")
                f.write(f"Files:\n")
        ...
        if datasets_dir.exists():
            datasets = {}
            for split in ['train', 'val_in_dist', 'val_near_dist', 'test_interpolation', 'test_extrapolation', 'test_novel']:
