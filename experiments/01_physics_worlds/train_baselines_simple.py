...
def train_baselines_simple():
    ...
    baselines = [
        ...
        {
            'name': 'Augmented_NN',
            'description': 'NN with data augmentation'
        }
    ]
    ...
    for config in baselines:
        ...
        if config['name'] == 'Augmented_NN':
            ...
            X_aug = np.vstack([X_train, X_train + 0.1 * np.random.randn(*X_train.shape)])
            y_aug = np.vstack([y_train, y_train + 0.1 * np.random.randn(*y_train.shape)])
        else:
            X_aug = X_train
            y_aug = y_train
    ...
    for name, res in results.items():
        print(f"{name:<20} {res['test_mse']:>10.4f} {res['test_mae']:>10.4f} {res['train_time']:>10.1f}")
