...
class PendulumBaselineTrainer:
    ...
    def train_all_baselines(self, data: Dict):
        ...
        gflownet = self.adapt_model_for_pendulum(
            PhysicsGFlowNetBaseline,
            {'exploration_bonus': 0.1, 'flow_steps': 5}
        )
        ...
        graphextrap = self.adapt_model_for_pendulum(
            PhysicsGraphExtrapolationBaseline,
            {'graph_layers': 3, 'node_dim': 64}
        )
        graphextrap.train(data['train'], data['val'], epochs=30)
        results['GraphExtrap'] = self.evaluate_model(graphextrap.model, data, "GraphExtrap")
    ...
    def save_results(self, results: Dict):
        ...
        ax1.bar(x - width/2, mse_fixed, width, label='Fixed Length', color='blue', alpha=0.7)
        ax1.bar(x + width/2, mse_ood, width, label='Time-Varying', color='red', alpha=0.7)
        ...
        for model in models:
            r = results[model]
            print(f"{model:<15} {r['mse_fixed']:<12.4f} {r['mse_ood']:<12.4f} {r['ood_degradation']:<12.1f}x")
