...
class OODVerifier:
    ...
    def create_physics_model(self, input_dim=8, hidden_dim=64):
        ...
        model = keras.Sequential([
            ...
            keras.layers.Dense(hidden_dim, activation='relu', name='representation'),
            keras.layers.BatchNormalization(),
    ...
    def fit_training_manifold(self, train_data):
        ...
        print(f"Training manifold fitted with {len(self.train_representations)} samples")
    ...
    def verify_ood(self, test_data):
        ...
        print(f"Distance ratio: {test_mean_dist.mean() / self.train_mean_dist.mean():.2f}x")
    ...
    def visualize_representations(self, train_data, test_data, save_path=None):
        ...
        plt.scatter(train_pca[:, 0], train_pca[:, 1], 
                   c='blue', alpha=0.6, label='Training (constant g)', s=50)
        ...
        plt.scatter(test_pca[~is_ood, 0], test_pca[~is_ood, 1], 
                   c='green', alpha=0.6, label='Test (interpolation)', s=50, marker='^')
        plt.scatter(test_pca[is_ood, 0], test_pca[is_ood, 1], 
                   c='red', alpha=0.6, label='Test (true OOD)', s=50, marker='s')
...
def main():
    ...
    if not const_files or not varying_files:
        print("Error: Could not find data files. Run generate_true_ood_data_minimal.py first.")
        return
    ...
    # Visualize
    viz_path = get_output_path() / "ood_verification" / f"representation_space_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    viz_path.parent.mkdir(parents=True, exist_ok=True)
    pca = verifier.visualize_representations(const_data, varying_data, save_path=viz_path)
    ...
    # Save results
    results_path = get_output_path() / "ood_verification" / f"verification_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
