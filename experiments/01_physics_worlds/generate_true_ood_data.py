...
class TrueOODPhysicsGenerator:
    ...
    def generate_time_varying_gravity(
        self, n_samples: int, amplitude: float = 0.3, frequency: float = 0.5
    ) -> Dict[str, np.ndarray]:
        ...
        def gravity_fn(t):
            return self.standard_gravity * (
                1 + amplitude * np.sin(2 * np.pi * frequency * t)
            )

        for i in range(n_samples):
            traj = self._simulate_trajectory(
                gravity_fn=gravity_fn, initial_state=self._random_initial_state()
            )
            trajectories.append(traj)
    ...
    def generate_height_dependent_gravity(
        self, n_samples: int, scale_height: float = 100.0
    ) -> Dict[str, np.ndarray]:
        ...
        for i in range(n_samples):
            traj = self._simulate_trajectory_height_dependent(
                gravity_fn_y=gravity_fn_y, initial_state=self._random_initial_state()
            )
            trajectories.append(traj)
    ...
    def generate_rotating_frame(
        self, n_samples: int, omega: float = 0.1
    ) -> Dict[str, np.ndarray]:
        ...
        for i in range(n_samples):
            traj = self._simulate_rotating_frame(
                omega=omega, initial_state=self._random_initial_state()
            )
            trajectories.append(traj)
    ...
    def generate_spring_coupled(
        self, n_samples: int, spring_constant: float = 100.0, rest_length: float = 50.0
    ) -> Dict[str, np.ndarray]:
    ...
    def _simulate_trajectory(
        self, gravity_fn: Callable[[float], float], initial_state: np.ndarray
    ) -> np.ndarray:
    ...
    def _simulate_trajectory_height_dependent(
        self, gravity_fn_y: Callable[[float], float], initial_state: np.ndarray
    ) -> np.ndarray:
    ...
    def _simulate_rotating_frame(
        self, omega: float, initial_state: np.ndarray
    ) -> np.ndarray:
        ...
        for t in range(1, self.timesteps):
            ...
            for i in range(self.n_balls):
                ...
                state[idx + 3] += (
                    self.standard_gravity + f_centrifugal[1] + f_coriolis[1]
                ) * self.dt
                state[idx + 3] *= 1 - self.friction * self.dt
    ...
    def _simulate_spring_system(
        self, k: float, rest_length: float, initial_state: np.ndarray
    ) -> np.ndarray:
...
def generate_all_datasets():
    ...
    for name, data in datasets.items():
        ...
        visualize_trajectory(
            traj, title=f"{name.replace('_', ' ').title()} Physics", save_path=save_path
        )
    ...
    summary = {
        ...
        "samples_per_dataset": {
            name: len(data["trajectories"]) for name, data in datasets.items()
        },
        "physics_parameters": {
            name: data["metadata"] for name, data in datasets.items()
        },
    }
