...
def run_hyperparameter_search(debug_mode=False):
    ...
    print("TTA V2 Hyperparameter Tuning" + (" (Debug Mode)" if debug_mode else " (Full Search)"))
    print("="*70)
    ...
    if debug_mode:
        ...
        hp_grid = {
            ...
            'consistency_weights': [0.0, 0.1, 0.5],  # Include 0 to test if losses are the issue
    ...
    print(f"Testing {len(hp_grid['learning_rates'])} learning rates from {hp_grid['learning_rates'][-1]} to {hp_grid['learning_rates'][0]}")
    ...
    if best_config and best_config['improvement'] > -20:
        ...
        promising_lrs = sorted(list(set([r['learning_rate'] for r in results if r['improvement'] > -30])))[:3]
    ...
    if best_config:
        ...
        for i, cfg in enumerate(sorted_results[:5]):
            ...
            print(f"   lr={cfg['learning_rate']:.0e}, steps={cfg['adaptation_steps']}, {mode_str}")
        
        # Analyze results
        print("\n--- Analysis ---")
        positive_configs = [r for r in results if r['improvement'] > 0]
        if positive_configs:
            print(f"Found {len(positive_configs)} configurations with positive improvement!")
            best_positive = max(positive_configs, key=lambda x: x['improvement'])
            print(f"Best improvement: {best_positive['improvement']:.1f}% with lr={best_positive['learning_rate']:.0e}")
        else:
...
def main():
    ...
    parser.add_argument('--debug', action='store_true', help='Run in debug mode with fewer configs')
