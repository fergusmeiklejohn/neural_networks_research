...
class DistributionModifier(keras.Model):
    ...
    def compute_loss(self, params, request_tokens, target_params, target_changes):
        ...
        pred_params, mod_factors, change_mask = self([params, request_tokens], training=True)
        ...
        target_change_ratios = ops.abs(target_params - params) / (ops.abs(params) + 1e-6)
...
class ModificationDataProcessor:
    ...
    def build_vocabulary(self, descriptions: List[str]):
        ...
        for word, _ in sorted_words[:self.vocab_size - 2]:  # Reserve 2 for special tokens
    ...
    def process_modification_pairs(self, mod_pairs: List[Dict]) -> Tuple[np.ndarray, ...]:
...
def test_modification_component():
    ...
    target_params = params * np.random.uniform(0.5, 1.5, params.shape).astype(np.float32)
