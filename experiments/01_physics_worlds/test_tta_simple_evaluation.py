...
def main():
    ...
    degradation = (varying_results['mse'] - const_results['mse']) / const_results['mse'] * 100
    ...
    for method in tta_methods:
        ...
        tta_results = evaluate_on_trajectories(
            model, varying_data['trajectories'], 
            with_tta=True, tta_method=method
        )
        results[method] = tta_results
        
        print(f"   MSE: {tta_results['mse']:.4f} ± {tta_results['mse_std']:.4f}")
        improvement = (varying_results['mse'] - tta_results['mse']) / varying_results['mse'] * 100
    ...
    print(f"Time-varying gravity (no TTA): {varying_results['mse']:.4f} ({degradation:.1f}% worse)")
    
    for method in tta_methods:
        improvement = (varying_results['mse'] - results[method]['mse']) / varying_results['mse'] * 100
        print(f"{method.upper()}: {results[method]['mse']:.4f} ({improvement:.1f}% improvement)")
