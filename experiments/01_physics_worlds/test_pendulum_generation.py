...
from pendulum_data_generator import PendulumDataConfig, PendulumDataGenerator, PendulumSimulator, PendulumConfig
...
def test_pendulum_physics():
    ...
    config_varying = PendulumConfig(
        length=1.0, gravity=9.8, damping=0.01,
        length_variation=0.2, length_frequency=0.1
    )
    ...
    print(f"Varying pendulum energy variation: {energy_var_varying:.3f} (should be > 0.02)")
    ...
    axes[0, 1].scatter(trajectory_varying['x'][::10], trajectory_varying['y'][::10], c='red', s=20)
    ...
    axes[1, 0].plot(trajectory_varying['t'], trajectory_varying['theta'], 'r-', label='Varying')
    ...
    # Length and energy
    axes[1, 1].plot(trajectory_varying['t'], trajectory_varying['length'], 'g-', label='Length')
    axes[1, 1].set_xlabel('Time (s)')
    axes[1, 1].set_ylabel('Length (m)', color='g')
    axes[1, 1].tick_params(axis='y', labelcolor='g')
    
    ax2 = axes[1, 1].twinx()
    ax2.plot(trajectory_varying['t'], trajectory_varying['energy'], 'orange', label='Energy')
...
def test_data_generation():
    ...
    print(f"Varying pendulum energy variation: {np.mean(varying_energies):.3f} ± {np.std(varying_energies):.3f}")
...
def visualize_mechanism_shift():
    ...
    plt.subplot(1, 2, 1)
    plt.plot(traj_fixed['theta'], traj_fixed['theta_dot'], 'b-', alpha=0.7, label='Fixed Length')
    plt.plot(traj_varying['theta'], traj_varying['theta_dot'], 'r-', alpha=0.7, label='Varying Length')
    plt.xlabel('θ (rad)')
    plt.ylabel('θ̇ (rad/s)')
