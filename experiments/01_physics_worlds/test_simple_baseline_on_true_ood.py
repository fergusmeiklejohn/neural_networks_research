...
def load_standard_physics_data():
    ...
    for sample in data:
        ...
        for t in range(len(traj)):
            ...
            if len(state) >= 17:
                features = [
                    state[1], state[2],   # x1, y1
                    state[9], state[10],  # x2, y2
                    state[3], state[4],   # vx1, vy1
                    state[11], state[12]  # vx2, vy2
                ]
            else:
...
def visualize_comparison(y_true, y_pred, ood_metadata, n_samples=3):
    ...
    for i in range(min(n_samples, len(y_true))):
        ...
        ax_pred = axes[1, i]
        pred_traj = y_pred[i]
        ax_pred.plot(pred_traj[:, 0], pred_traj[:, 1], 'b--', alpha=0.7, label='Ball 1 (pred)')
        ax_pred.plot(pred_traj[:, 2], pred_traj[:, 3], 'r--', alpha=0.7, label='Ball 2 (pred)')
...
def main():
    ...
    if len(X_train) > 50000:
        print(f"   Subsampling training data from {len(X_train)} to 50000 samples for faster training")
    ...
    ood_results, ood_predictions = evaluate_on_ood(model, X_ood, y_ood, "Harmonic Gravity")
    ...
    results_summary = {
        ...
        'conclusion': 'Catastrophic failure on time-varying gravity' if ood_results['mse'] > 10000 else 'Significant degradation'
    }
