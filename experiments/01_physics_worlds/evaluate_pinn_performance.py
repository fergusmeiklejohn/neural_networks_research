...
class PINNEvaluator:
    ...
    def __init__(self, model_path: str, baseline_path: Optional[str] = None):
        ...
        self.baseline = (
            keras.models.load_model(baseline_path) if baseline_path else None
        )
    ...
    def evaluate_on_split(self, data_path: str, split_name: str) -> Dict:
        ...
        # Process each sample
        for sample in tqdm(
            data[:100], desc=f"Processing {split_name}"
        ):  # Limit for testing
            ...
            pos_error = np.mean(
                np.abs(predictions["positions"].numpy() - true_positions)
            )
            vel_error = np.mean(
                np.abs(predictions["velocities"].numpy() - true_velocities)
            )
            metrics["trajectory_error"].append(pos_error + vel_error)

            # 2. Physics parameter extraction error
            param_error = np.mean(
                np.abs(predictions["physics_params"].numpy() - true_params)
            )
            ...
            energy_loss = self.physics_losses.energy_conservation_loss(
                physics_state, damping=sample["physics_config"]["damping"]
            )
    ...
    def generate_report(self, all_results: Dict) -> str:
        ...
        for split_name, metrics in all_results.items():
            if isinstance(metrics, dict) and "trajectory_error_mean" in metrics:
                ...
                report.append(
                    f"  - Trajectory Error: {metrics['trajectory_error_mean']:.4f} ± {metrics['trajectory_error_std']:.4f}"
                )
                report.append(
                    f"  - Param Extraction Error: {metrics['physics_param_error_mean']:.4f}"
                )
                report.append(
                    f"  - Energy Conservation: {metrics['energy_conservation_error_mean']:.4f}"
                )
                report.append("")
        ...
        report.append("")
        report.append(
            "1. Energy Conservation: PINN shows significantly better energy conservation"
        )
        report.append(
            "2. Extrapolation: Improved performance on out-of-distribution parameters"
        )
        report.append(
            "3. Smoothness: Physics-informed losses lead to more realistic trajectories"
        )
