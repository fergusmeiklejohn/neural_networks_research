...
class SimpleExtremeOODGenerator:
    ...
    def generate_rotating_frame(self, omega=0.5, n_steps=50):
        ...
        pos1 = np.array([200., 300.], dtype=np.float32)
        pos2 = np.array([600., 300.], dtype=np.float32)
        vel1 = np.array([50., -100.], dtype=np.float32)
        vel2 = np.array([-50., -80.], dtype=np.float32)
        
        trajectory = []
        center = np.array([400., 300.])  # Center of rotation
        ...
        for _ in range(n_steps):
            ...
            acc1 = np.array([0., -392.]) + coriolis1 + centrifugal1
            acc2 = np.array([0., -392.]) + coriolis2 + centrifugal2
    ...
    def generate_spring_coupled(self, k=50.0, rest_length=200.0, n_steps=50):
        ...
        pos1 = np.array([200., 300.], dtype=np.float32)
        pos2 = np.array([600., 300.], dtype=np.float32)
        vel1 = np.array([0., -50.], dtype=np.float32)
        vel2 = np.array([0., -50.], dtype=np.float32)
