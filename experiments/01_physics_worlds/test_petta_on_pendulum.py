...
from petta_inspired_collapse_detection import PeTTAInspiredTTA
...
def run_comprehensive_comparison():
    ...
    print(
        f"Degradation: {results['standard_tta']['mse'] / results['baseline']['mse']:.2f}x"
    )
    ...
    print(
        f"Degradation: {results['petta_tta']['mse'] / results['baseline']['mse']:.2f}x"
    )
    ...
    if adaptation_history["collapse_events"]:
        ...
        print(
            f"Interventions: {[i['type'] for i in adaptation_history['interventions']]}"
        )
    else:
    ...
    print(
        f"Entropy reduction: {(1 - metrics['prediction_entropy'][-1]/metrics['prediction_entropy'][0])*100:.1f}%"
    )
    ...
    print(
        f"Variance reduction: {(1 - metrics['prediction_variance'][-1]/metrics['prediction_variance'][0])*100:.1f}%"
    )
    ...
    petta_tta.collapse_detector.plot_collapse_metrics(
        "outputs/pendulum_tta/petta_collapse_metrics.png"
    )
    ...
    detailed_results = {
        ...
        "collapse_metrics_final": {
            ...
            "param_drift": metrics["parameter_change"][-1]
            if metrics["parameter_change"]
            else 0,
        },
    }
...
"""
    ...
    print("CONCLUSION:")
    print(
        f"Standard TTA: {results['standard_tta']['mse']/results['baseline']['mse']:.1f}x degradation"
    )
    print(
        f"PeTTA TTA: {results['petta_tta']['mse']/results['baseline']['mse']:.1f}x degradation"
    )

    if results["petta_tta"]["mse"] < results["standard_tta"]["mse"]:
        improvement = (
            1 - results["petta_tta"]["mse"] / results["standard_tta"]["mse"]
        ) * 100
        print(
            f"PeTTA reduced degradation by {improvement:.1f}% compared to standard TTA"
        )
    else:
