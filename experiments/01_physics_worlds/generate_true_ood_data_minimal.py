...
def main():
    ...
    print("\n3. Data Statistics:")
    print(
        f"   Constant gravity range: y ∈ [{const_data['trajectories'][:,:,[1,5]].min():.1f}, {const_data['trajectories'][:,:,[1,5]].max():.1f}]"
    )
    print(
        f"   Time-varying range: y ∈ [{varying_data['trajectories'][:,:,[1,5]].min():.1f}, {varying_data['trajectories'][:,:,[1,5]].max():.1f}]"
    )
