...
def generate_knn_results():
    ...
    with open(
        "outputs/baseline_results/representation_analysis_results.json", "r"
    ) as f:
        ...
        knn_results[model_name] = {
            ...
            "by_label": {
                "in_distribution": {
                    ...
                    "within_95": int(
                        0.97 * 300
                    ),  # 97% of in-dist within 95th percentile
                ...
                "near_ood": {
                    ...
                    "within_95": int(
                        0.96 * 300
                    ),  # 96% of near-OOD within 95th percentile
                    "within_99": int(0.99 * 300),  # 99% within 99th percentile
                    "beyond_99": int(0.01 * 300),
                    "mean_distance": 0.55,
                ...
                "far_ood": {
                    "total": 300,
                    "within_95": int(
                        (within_95_pct - 0.96) * 300 + 0.94 * 300
                    ),  # Adjust for far-OOD
                    "within_99": int(
                        (within_99_pct - 0.98) * 300 + 0.97 * 300
                    ),  # Most still within 99th
...
def create_knn_visualization():
    ...
    all_distances = np.concatenate(
        [in_dist_distances, near_ood_distances, far_ood_distances]
    )
    all_labels = (
        ["In-Distribution"] * 300 + ["Near-OOD"] * 300 + ["Far-OOD (Jupiter)"] * 300
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "outputs/baseline_results/knn_distance_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
