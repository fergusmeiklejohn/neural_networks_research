...
def create_performance_comparison_plot():
    ...
    ax1.set_title(
        "Model Performance Across Gravity Conditions", fontsize=14, fontweight="bold"
    )
    ax1.set_xticks(x)
    ...
    jupiter_idx = 3
    for i, (perf, name) in enumerate(
        [
            (graph_performance[jupiter_idx], "GraphExtrap"),
    ...
    bars = ax2.bar(
        models, jupiter_mse, color=colors, alpha=0.8, edgecolor="black", linewidth=1
    )
    ax2.set_ylabel("MSE on Jupiter Gravity", fontsize=12)
    ax2.set_title(
        "Jupiter Extrapolation: PINN vs Baselines", fontsize=14, fontweight="bold"
    )
    ax2.axhline(
        y=0.1,
        color="green",
    ...
    ax3.set_title(
        "PINN Progressive Curriculum Learning", fontsize=14, fontweight="bold"
    )
    ax3.set_xticks(x)
    ...
    # Overall title
    fig.suptitle(
        "Physics-Informed Neural Networks Enable True Extrapolation",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    ax1.text(
        0.5,
        -0.5,
        "Yet baselines fail with >10x error!",
        ha="center",
        fontsize=14,
        fontweight="bold",
        color="red",
    ...
    bars = ax2.bar(
        models, performance, color=colors, alpha=0.8, edgecolor="black", linewidth=2
    )
    ax2.set_ylabel("MSE on Jupiter Gravity", fontsize=12)
    ax2.set_ylim(0, 1.0)
    ...
    fig2.suptitle(
        "Resolving the OOD Illusion: Causal Understanding Enables Extrapolation",
        fontsize=16,
        fontweight="bold",
    )
