...
def test_physics_aware_tta():
    ...
    if not model_path.exists():
        ...
        if not model_path.exists():
            print("Error: No pre-trained model found. Run train_pendulum_baselines.py first.")
            return
    ...
    for adaptation_type in ['prediction', 'energy', 'hamiltonian']:
        ...
        print(f"  Factor: {eval_results['adaptation_factor']:.2f}x " +
    ...
    plt.subplot(1, 2, 1)
    plt.bar(x - width/2, [original_mse] * len(methods), width, 
            label='Original', color='blue', alpha=0.7)
    plt.bar(x + width/2, adapted_mses, width, 
            label='Adapted', color='red', alpha=0.7)
    plt.xlabel('TTA Method')
    plt.ylabel('MSE')
    plt.title('Pendulum OOD Prediction Error')
    plt.xticks(x, ['Prediction\nConsistency', 'Energy\nConsistency', 'Hamiltonian\nConsistency'])
    plt.legend()
    plt.yscale('log')
    ...
    for method in methods:
        r = results[method]
        print(f"{method.capitalize():<20} {r['mse_original']:<15.6f} " +
