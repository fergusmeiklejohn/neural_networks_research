...
def generate_physics_trajectory(num_timesteps=50, gravity=-9.8, friction=0.5, 
                              elasticity=0.8, damping=0.95):
    ...
    physics_params = np.array([gravity, friction, elasticity, damping], dtype=np.float32)
...
def train_stage(model, train_data, val_data, stage_name, epochs=20, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(n_batches):
            ...
            # Get current weights
            current_weights = [v.value for v in model.trainable_variables]
...
def main():
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        for test_name, test_data in [
            ...
            print(f"  {test_name}: MSE={results['mse']:.4f}, "
    ...
    print(f"  Improvement:      {(1 - final_results['Jupiter']['mse'] / min(baseline_far_ood_mse.values())) * 100:.1f}%")
