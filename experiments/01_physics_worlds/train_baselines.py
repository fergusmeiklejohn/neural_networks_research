...
from models.unified_evaluation import RepresentationSpaceAnalyzer
...
class PhysicsBaselineTrainer:
    ...
    def load_physics_data(self):
        ...
        print(
            f"Loaded train: {len(self.train_data['x'])}, "
    ...
    def generate_synthetic_physics_data(self):
        ...
        np.savez(
            self.data_dir / "physics_train.npz",
            states=train_states,
        ...
        np.savez(
            self.data_dir / "physics_test.npz",
            states=test_states,
    ...
    def test_modification_capability(self, baseline) -> float:
        ...
        adapted_baseline = baseline.adapt_to_modification(
            modification_request, examples={"x": moon_x, "y": moon_y}
        )
    ...
    def train_all_baselines(self):
        ...
        # 1. ERM + Augmentation
        results = self.train_baseline("ERM+Aug", ERMWithAugmentation, configs["erm"])
        all_results["erm"] = results

        # 2. GFlowNet
        results = self.train_baseline("GFlowNet", GFlowNetBaseline, configs["gflownet"])
        all_results["gflownet"] = results

        # 3. Graph Extrapolation
        results = self.train_baseline(
            "GraphExtrap", GraphExtrapolationBaseline, configs["graph"]
        )
        all_results["graph"] = results

        # 4. MAML
        results = self.train_baseline("MAML", MAMLBaseline, configs["maml"])
    ...
    def generate_comparison_report(self, results: Dict):
        ...
        report.append(
            "| Baseline | Overall MSE | Interpolation | Near-Extrap | Far-Extrap | Modification Score |"
        )
        report.append(
            "|----------|-------------|---------------|-------------|------------|-------------------|"
        )
        ...
        for name, res in results.items():
            ...
            interp = (
                res["extrapolation_metrics"].get("interpolation", {}).get("mse", "N/A")
            )
            near = (
                res["extrapolation_metrics"]
                .get("near_extrapolation", {})
                .get("mse", "N/A")
            )
            far = (
                res["extrapolation_metrics"]
                .get("far_extrapolation", {})
                .get("mse", "N/A")
            )
            mod_score = res["modification_score"]

            report.append(
                f"| {name} | {overall_mse:.4f} | {interp:.4f} | {near:.4f} | {far:.4f} | {mod_score:.3f} |"
            )
        ...
        for name, res in results.items():
            ...
            if res["extrapolation_metrics"]:
                ...
                for cat, metrics in res["extrapolation_metrics"].items():
                    report.append(
                        f"  - {cat}: MSE={metrics['mse']:.4f}, n={metrics['count']}"
                    )
        ...
        best_overall = min(
            results.items(), key=lambda x: x[1]["overall_metrics"]["mse"]
        )[0]
        ...
        for name, res in results.items():
            if "far_extrapolation" in res["extrapolation_metrics"]:
                extrap_scores.append(
                    (name, res["extrapolation_metrics"]["far_extrapolation"]["mse"])
                )
