...
def extract_physics_features(trajectories):
    ...
    for traj in trajectories:
        ...
        features.append([
            avg_y, avg_vy, y_range, final_y,
            max_down_v, time_to_bottom, total_dist, 
            avg_kinetic, avg_potential
        ])
...
def verify_ood_with_features(const_data, varying_data):
    ...
    # Plot
    plt.scatter(const_pca[:, 0], const_pca[:, 1], 
               c='blue', alpha=0.6, label='Constant gravity', s=50)
    
    # Color varying gravity by OOD status
    plt.scatter(varying_pca[~is_ood, 0], varying_pca[~is_ood, 1], 
               c='green', alpha=0.6, label=f'Varying g (interpolation: {100-ood_percentage:.0f}%)', 
               s=50, marker='^')
    plt.scatter(varying_pca[is_ood, 0], varying_pca[is_ood, 1], 
               c='red', alpha=0.6, label=f'Varying g (true OOD: {ood_percentage:.0f}%)', 
               s=50, marker='s')
    ...
    plt.text(0.02, 0.98, f'Variance explained: {var_explained[0]:.1%}, {var_explained[1]:.1%}',
