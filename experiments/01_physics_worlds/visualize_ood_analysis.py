...
def create_ood_summary_plot():
    ...
    ax1.set_title(
        "Jupiter Gravity Samples: Actually Interpolation!",
        fontsize=14,
        fontweight="bold",
    )
    ax1.set_xticks(x)
    ...
    for i, model in enumerate(models):
        ax1.text(
            i,
            interpolation_pct[i] / 2,
            f"{interpolation_pct[i]:.1f}%",
            ha="center",
            va="center",
            fontweight="bold",
        )
    ...
    ax2.set_title(
        'Poor Performance Despite Being "In-Distribution"',
        fontsize=14,
        fontweight="bold",
    )
    ax2.set_xticks(x)
    ...
    ax2.axhline(
        y=0.1, color="green", linestyle="--", alpha=0.5, label="Good performance"
    )
    ax2.set_ylim(0, 1.2)
    ...
    plt.suptitle(
        "OOD Analysis Reveals: Jupiter Gravity is NOT Out-of-Distribution!",
        fontsize=16,
        fontweight="bold",
    )
    plt.tight_layout()
    ...
    ax1.pie(
        [10, 30, 60],
        labels=["In-dist", "Near-OOD", "Far-OOD"],
    ...
    ax2.pie(
        [91.7, 8.3, 0],
        labels=["Interpolation", "Near-extrap", "Far-extrap"],
        colors=["#2ecc71", "#f39c12", "#e74c3c"],
        autopct="%1.0f%%",
        startangle=90,
    )
    ...
    plt.suptitle(
        "The OOD Illusion: Jupiter Gravity Analysis", fontsize=16, fontweight="bold"
    )
    plt.tight_layout()
