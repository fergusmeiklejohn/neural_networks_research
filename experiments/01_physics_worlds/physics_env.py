...
class PhysicsWorld:
    ...
    def _create_boundaries(self):
        ...
        floor = pymunk.Segment(
            self.space.static_body, (0, 0), (self.config.world_width, 0), 5
        )
        floor.friction = self.config.friction
        floor.elasticity = self.config.elasticity

        # Ceiling
        ceiling = pymunk.Segment(
        ...
        left_wall = pymunk.Segment(
            self.space.static_body, (0, 0), (0, self.config.world_height), 5
        )
    ...
    def add_ball(self, ball: Ball) -> pymunk.Body:
        ...
        body = pymunk.Body(
            ball.mass, pymunk.moment_for_circle(ball.mass, 0, ball.radius)
        )
            ...
            for body, shape, ball_info in self.balls:
                ball_state = {
                    ...
                    * (body.velocity.x**2 + body.velocity.y**2),
                    "potential_energy": ball_info.mass
                    * abs(self.space.gravity[1])
                    * body.position.y,
                }
    ...
    def visualize_trajectory(self, save_path: Optional[str] = None):
        ...
        for ball_idx, (_, _, ball_info) in enumerate(self.balls):
            ...
            # Plot trajectory
            axes[0, 0].plot(
                positions_x, positions_y, label=f"Ball {ball_idx+1}", linewidth=2
            )

            # Plot velocity over time
            axes[0, 1].plot(
                times[: len(velocities_x)],
                velocities_x,
                label=f"Ball {ball_idx+1} vx",
                alpha=0.7,
            )
            axes[0, 1].plot(
                times[: len(velocities_y)],
                velocities_y,
                label=f"Ball {ball_idx+1} vy",
                alpha=0.7,
                linestyle="--",
            )
            ...
            axes[1, 0].plot(
                times[: len(energies_k)],
                energies_k,
                label=f"Ball {ball_idx+1} KE",
                alpha=0.7,
            )
            axes[1, 0].plot(
                times[: len(energies_p)],
                energies_p,
                label=f"Ball {ball_idx+1} PE",
                alpha=0.7,
                linestyle="--",
            )
            ...
            axes[1, 1].plot(
                times[: len(total_energy)],
                total_energy,
                label=f"Ball {ball_idx+1} Total",
                alpha=0.7,
            )
