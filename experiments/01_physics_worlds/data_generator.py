...
class DataConfig:
    ...
    gravity_range: Tuple[float, float] = (-1500, -200)  # Wide range including low gravity
    friction_range: Tuple[float, float] = (0.05, 0.95)  # Near-frictionless to high friction
    elasticity_range: Tuple[float, float] = (0.1, 0.99) # Completely inelastic to super bouncy
    damping_range: Tuple[float, float] = (0.8, 0.99)    # High air resistance to vacuum-like
    ...
    max_velocity_threshold: float = 1500  # Maximum reasonable velocity (increased for collisions)
    ...
    def _validate_sample_quality(self, sample_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(trajectory) < self.config.min_trajectory_length:
                return False, f"Trajectory too short: {len(trajectory)} < {self.config.min_trajectory_length}"
            ...
            try:
                ...
                if not is_valid:
                    if retry < max_retries - 1:
                        warnings.warn(f"Sample {sample_id} retry {retry + 1}: {validation_msg}")
                        continue
                    else:
                        raise ValueError(f"Failed to generate valid sample after {max_retries} retries: {validation_msg}")
        ...
        for sample_id in tqdm(range(base_samples), desc="Generating modification pairs"):
            try:
                ...
                modifications = [
                    ...
                    {
                        'params': {'elasticity': max(0.1, base_config.elasticity * 0.5)},
                        'description': 'make objects less bouncy',
                        'type': 'elasticity_decrease'
                    },
                    {
                        'params': {'elasticity': min(0.99, base_config.elasticity * 1.2)},
                    ...
                    {
                        'params': {'gravity': base_config.gravity * 0.5, 'friction': base_config.friction * 0.5},
                        'description': 'space-like physics with low gravity and friction',
                        'type': 'space_physics'
                    },
                    {
                        'params': {'damping': 0.85, 'gravity': base_config.gravity * 0.7},
                    ...
                    pair_data = {
                        ...
                        'parameter_changes': {
                            param: {
                                ...
                                'modified': value,
                                'change_ratio': value / getattr(base_config, param) if getattr(base_config, param) != 0 else 0
...
def generate_physics_datasets():
    ...
    mod_file = generator.generate_modification_pairs(1000)  # 1000 base samples * 9 modifications = 9000 pairs
    ...
    print(f"  Quality rate: {quality_report['overall_statistics']['overall_quality_rate']:.2%}")
...
def generate_quality_report(datasets: Dict[str, str], output_dir: str = "data/processed/physics_worlds") -> Dict[str, Any]:
    ...
    for split, file_path in datasets.items():
        ...
        if data and 'quality_validation' in data[0]:
            quality_scores = [sample.get('quality_validation', {}).get('is_valid', False) for sample in data]
    ...
    report['overall_statistics'] = {
        ...
        'overall_quality_rate': np.mean(all_quality_scores) if all_quality_scores else 0.0,
...
def analyze_dataset_detailed(data: List[Dict[str, Any]], split: str) -> Dict[str, Any]:
    ...
    if physics_samples:
        ...
        elasticities = [sample['physics_config']['elasticity'] for sample in physics_samples]
    ...
    if gravities:  # Only calculate stats if we have physics data
        stats['physics_parameters'] = {
            'gravity': {'min': min(gravities), 'max': max(gravities), 'mean': np.mean(gravities), 'std': np.std(gravities)},
            'friction': {'min': min(frictions), 'max': max(frictions), 'mean': np.mean(frictions), 'std': np.std(frictions)},
            'elasticity': {'min': min(elasticities), 'max': max(elasticities), 'mean': np.mean(elasticities), 'std': np.std(elasticities)},
            'damping': {'min': min(dampings), 'max': max(dampings), 'mean': np.mean(dampings), 'std': np.std(dampings)}
        }
    else:
        stats['physics_parameters'] = {'note': 'No physics_config data found in samples'}
    ...
    if traj_samples:
        ...
        ball_counts = [sample['num_balls'] for sample in traj_samples if 'num_balls' in sample]
    else:
        traj_lengths = ball_counts = []
    
    if traj_lengths:
        stats['trajectory_stats'] = {
            'length': {'min': min(traj_lengths), 'max': max(traj_lengths), 'mean': np.mean(traj_lengths), 'std': np.std(traj_lengths)},
            'ball_count': {'min': min(ball_counts), 'max': max(ball_counts), 'mean': np.mean(ball_counts), 'std': np.std(ball_counts)} if ball_counts else {'note': 'No ball_count data'}
        }
    else:
    ...
    if 'metrics' in data[0]:
        energy_conservations = [sample['metrics'].get('energy_conservation', 0) for sample in data]
...
def analyze_dataset(file_path: str):
    ...
    # Quality metrics if available
    if data and 'quality_validation' in data[0]:
        valid_samples = [sample.get('quality_validation', {}).get('is_valid', False) for sample in data]
