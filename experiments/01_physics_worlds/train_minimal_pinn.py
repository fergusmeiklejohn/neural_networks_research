...
def load_physics_data():
    ...
    if len(train_data) > 0 and 'num_balls' in train_data[0]:
        ...
        print(f"Filtered to {len(train_data_2ball)} trajectories with 8-20 features (likely 2-ball)")
    
    # Extract trajectories from initial_balls format
    trajectories = []
    for sample in train_data_2ball:
        traj_array = np.array(sample['trajectory'])
        initial_balls = sample.get('initial_balls', [])
    ...
    if len(train_data_2ball) > 0:
        if 'physics_params' in train_data_2ball[0]:
            params_train = np.array([traj['physics_params'] for traj in train_data_2ball])
        elif 'physics_config' in train_data_2ball[0]:
            # Extract from physics_config
            params_list = []
            # Debug first config
            if len(train_data_2ball) > 0:
                print(f"Physics config structure: {train_data_2ball[0]['physics_config']}")
            ...
            for traj in train_data_2ball:
                ...
                if isinstance(config, dict):
                    ...
                    pixel_to_meter = 40.0
                    params = [
                        config.get('gravity', -392.0) / pixel_to_meter,  # Convert to m/s²
                        config.get('friction', 0.1),
                        config.get('elasticity', 0.8)  # Note: it's 'elasticity' not 'restitution'
                    ]
                else:
    ...
    X_val = np.array(val_trajectories) if val_trajectories else np.array([]).reshape(0, 50, 8)
    ...
    if 'test_interpolation' in test_sets and params_test_interp.shape[0] > 0:
        ...
        if np.any(earth_mask):
            test_sets['test_earth'] = (X_test_interp[earth_mask], params_test_interp[earth_mask])
    ...
    if 'test_extrapolation' in test_sets and params_test_extrap.shape[0] > 0:
        ...
        if np.any(jupiter_mask):
            test_sets['test_jupiter'] = (X_test_extrap[jupiter_mask], params_test_extrap[jupiter_mask])
    ...
    for name, (X, p) in test_sets.items():
        ...
        if p.shape[0] > 0:
            gravity_values = p[:, 0]
            print(f"    Gravity range: [{gravity_values.min():.1f}, {gravity_values.max():.1f}]")
    
    # Check if we have Jupiter-like gravity in extrapolation set
    if 'test_extrapolation' in test_sets and test_sets['test_extrapolation'][1].shape[0] > 0:
...
def evaluate_extrapolation(model, test_sets):
    ...
    for name, (X_test, params_test) in test_sets.items():
        ...
        print(f"{name}: MSE={mse:.4f}, True gravity={gravity:.2f}, "
...
def main():
    ...
    loss_fn = create_physics_loss(model, reconstruction_weight=1.0, physics_weight=10.0)  # Reduced from 100.0
    ...
    our_jupiter_mse = results['stages'][-1]['results'].get('test_jupiter', {}).get('mse', float('inf'))
    ...
    print(f"Friction: {float(model.friction[0]):.4f}")  # friction is now a property that returns array
