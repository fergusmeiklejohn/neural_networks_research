...
class SimpleRepresentationAnalyzer:
    ...
    def analyze_test_data(self, model, test_data, test_labels=None):
        ...
        for i, train_point in enumerate(self.train_representations_pca):
            ...
            mask[i] = False
            dists = np.linalg.norm(self.train_representations_pca[mask] - train_point, axis=1)
        ...
        for log_density, distance in zip(log_densities, distances):
            ...
            elif log_density > density_threshold - 2 or distance < distance_threshold * 1.5:
        ...
        if test_labels is not None:
            ...
            for label in unique_labels:
                ...
                results['by_label'][label] = {
                    ...
                    'avg_log_density': float(np.mean([log_densities[i] for i in label_indices])),
                    'avg_distance': float(np.mean([distances[i] for i in label_indices]))
                }
        
        return results, categories, log_densities, distances
    
    def plot_analysis(self, log_densities, distances, categories, test_labels=None, save_path=None):
        """Visualize the representation space analysis."""
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        
        # Plot 1: Density distribution
        ax = axes[0]
        colors = {'interpolation': 'green', 'near_extrapolation': 'orange', 'far_extrapolation': 'red'}
        
        for cat in ['interpolation', 'near_extrapolation', 'far_extrapolation']:
            indices = [i for i, c in enumerate(categories) if c == cat]
            if indices:
                ax.hist([log_densities[i] for i in indices], 
                       alpha=0.5, label=cat, color=colors[cat], bins=20)
        ...
        for cat in ['interpolation', 'near_extrapolation', 'far_extrapolation']:
            indices = [i for i, c in enumerate(categories) if c == cat]
            if indices:
                ax.hist([distances[i] for i in indices], 
                       alpha=0.5, label=cat, color=colors[cat], bins=20)
        ...
        if self.train_representations_pca.shape[1] >= 2:
            # Plot training data
            ax.scatter(self.train_representations_pca[:, 0], 
                      self.train_representations_pca[:, 1],
                      c='gray', alpha=0.3, s=10, label='Training')
            ...
            for cat in ['interpolation', 'near_extrapolation', 'far_extrapolation']:
                ...
                if indices and len(indices) > 0:
                    ...
                    ax.scatter(np.random.randn(len(indices)), 
                              np.random.randn(len(indices)),
                              c=colors[cat], alpha=0.7, s=30, label=f'Test: {cat}')
...
def analyze_baseline_representations():
    ...
    for name, class_name in baselines_to_analyze:
        ...
        try:
            ...
            if class_name == 'ERMWithAugmentation':
                ...
                model_config['augmentation_strategies'] = ['physics_noise', 'physics_interpolation']
            ...
            if hasattr(baseline, 'compute_graph_features'):
                ...
                results, categories, log_densities, distances = analyzer.analyze_test_data(
                    baseline.model, X_test_features, test_labels
                )
            else:
                analyzer.fit_on_training_data(baseline.model, X_train)
                results, categories, log_densities, distances = analyzer.analyze_test_data(
                    baseline.model, X_test, test_labels
                )
            ...
            print(f"- Interpolation: {results['interpolation_count']} ({results['interpolation_count']/results['total_samples']*100:.1f}%)")
            print(f"- Near-extrapolation: {results['near_extrapolation_count']} ({results['near_extrapolation_count']/results['total_samples']*100:.1f}%)")
            print(f"- Far-extrapolation: {results['far_extrapolation_count']} ({results['far_extrapolation_count']/results['total_samples']*100:.1f}%)")
            
            if 'by_label' in results:
                print("\nBreakdown by original labels:")
                for label, stats in results['by_label'].items():
                    print(f"\n{label}:")
                    print(f"  Actually interpolation: {stats['interpolation']}/{stats['total']} ({stats['interpolation']/stats['total']*100:.1f}%)")
                    print(f"  Actually near-extrap: {stats['near_extrapolation']}/{stats['total']} ({stats['near_extrapolation']/stats['total']*100:.1f}%)")
                    print(f"  Actually far-extrap: {stats['far_extrapolation']}/{stats['total']} ({stats['far_extrapolation']/stats['total']*100:.1f}%)")
                    print(f"  Avg log density: {stats['avg_log_density']:.2f}")
                    print(f"  Avg distance: {stats['avg_distance']:.2f}")
            
            # Create visualization
            plot_path = Path(f"outputs/baseline_results/{name}_representation_analysis.png")
            analyzer.plot_analysis(log_densities, distances, categories, test_labels, plot_path)
    ...
    for name, results in representation_results.items():
        if 'by_label' in results and 'far_ood' in results['by_label']:
            ...
            total = stats['total']
            print(f"{name:<15} {total} samples{'':<16} {actual_far}/{total} ({actual_far/total*100:.1f}%)")
