...
class CollapseDetector:
    ...
    def detect_collapse(
        ...
        if len(self.metrics_history["prediction_entropy"]) >= self.window_size:
            recent_entropy = self.metrics_history["prediction_entropy"][
                -self.window_size :
            ]
            recent_variance = self.metrics_history["prediction_variance"][
                -self.window_size :
            ]

            # Check for decreasing entropy (predictions becoming too similar)
            if (
                np.mean(recent_entropy)
                < 0.5 * self.metrics_history["prediction_entropy"][0]
            ):
                collapse_indicators["low_entropy"] = True

            # Check for vanishing variance (predictions converging to constant)
            if (
                np.mean(recent_variance)
                < 0.1 * self.metrics_history["prediction_variance"][0]
            ):
    ...
    def adapt_batch_with_monitoring(self, X_batch: np.ndarray, num_steps: int = 20):
        ...
        for step in range(num_steps):
            ...
            history = self.adapted_model.fit(
                X_batch, dummy_targets, epochs=1, batch_size=len(X_batch), verbose=0
            )
            ...
            if collapse_status["is_collapsing"]:
                print(
                    f"Step {step}: Collapse detected! Score: {collapse_status['collapse_score']:.3f}"
                )
