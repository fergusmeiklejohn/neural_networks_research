...
class PhysicsTrainingPipeline:
    ...
    def preprocess_data(self) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
        ...
        def extract_features(samples):
            ...
            for sample in samples:
                ...
                if len(traj) > 0:
                    ...
                    num_balls = min(
                        max_balls, len(initial_frame) // num_features_per_ball
                    )
                    ...
                    if num_balls > 0:
                        ...
                        if num_balls < max_balls:
                            padding = np.zeros(
                                (max_balls - num_balls, num_features_per_ball)
                            )
        ...
        print(
            f"  Train initial conditions: {train_features['initial_conditions'].shape}"
        )
    ...
    def create_data_generators(
        self, train_features, val_features, batch_size: int = 16
    ):
        ...
        def data_generator(features, modification_data, batch_size, shuffle=True):
            ...
            while True:
                ...
                for start_idx in range(0, n_samples, batch_size):
                    ...
                    modification_requests = np.random.randint(
                        0, 100, (batch_size_actual, 20)
                    )
        ...
        train_gen = data_generator(
            train_features, self.modification_data, batch_size, shuffle=True
        )
    ...
    def train(
        ...
        train_gen, val_gen = self.create_data_generators(
            train_features, val_features, batch_size
        )
        ...
        if use_wandb:
            wandb_callback = callbacks.WandbCallback(
                monitor="val_loss", save_model=False
            )
...
def main():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="data/processed/physics_worlds",
