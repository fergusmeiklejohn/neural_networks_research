...
class DistributionInventionEvaluator:
    ...
    def evaluate_interpolation_performance(
    ...
    def evaluate_novel_regime_performance(
        self, sample_size: Optional[int] = None
    ) -> Dict[str, float]:
        ...
        for regime_name, samples in regime_samples.items():
            regime_results[regime_name] = self._evaluate_rule_extraction(
                samples, f"novel_{regime_name}"
            )
    ...
    def _evaluate_rule_extraction(
        self, test_data: List[Dict], evaluation_type: str
    ) -> Dict[str, float]:
            ...
            try:
                ...
                if hasattr(self.model, "rule_extractor"):
                    ...
                    extracted = {
                        "gravity": float(
                            extracted[0][0]
                            if hasattr(extracted[0], "__getitem__")
                            else extracted[0]
                        ),
                        "friction": float(
                            extracted[0][1]
        ...
        # Calculate metrics
        results = {
            f"{evaluation_type}_success_rate": successful_extractions / total_samples
            if total_samples > 0
            else 0.0
        }
        ...
        for param, errors in extraction_errors.items():
            if errors:
                ...
                results[f"{evaluation_type}_{param}_accuracy_10%"] = np.mean(
                    np.array(errors) < 0.1
                )
                results[f"{evaluation_type}_{param}_accuracy_20%"] = np.mean(
                    np.array(errors) < 0.2
                )
            else:
                results[
                    f"{evaluation_type}_{param}_mae"
                ] = 1.0  # Max error if no successful extractions
    ...
    def evaluate_modification_consistency(
        ...
        for request, target_param, expected_change in modification_tests:
                ...
                try:
                    ...
                    if hasattr(self.model, "modify_distribution"):
                        ...
                        if callable(expected_change):
                            result["modified_rules"][target_param] = expected_change(
                                original_physics[target_param]
                            )
                        else:
                            result["modified_rules"][target_param] = (
                                original_physics[target_param] * expected_change
                            )
            ...
            modification_results[request] = {
                ...
                "directional_accuracy": np.mean(test_directions)
                if test_directions
                else 0.0,
        ...
        return {
            ...
            "modification_success_rate": np.mean(modification_success_rate)
            if modification_success_rate
            else 0.0,
    ...
    def evaluate_distribution_distance(
        ...
        train_params = np.array(
            [
                [
                    s["physics_config"]["gravity"],
                    s["physics_config"]["friction"],
                    s["physics_config"]["elasticity"],
                    s["physics_config"]["damping"],
                ]
                for s in train_data
            ]
        )
        ...
        for i, param in enumerate(["gravity", "friction", "elasticity", "damping"]):
            try:
                distance = stats.wasserstein_distance(
                    train_params[:, i], test_params[:, i]
                )
                distances[f"wasserstein_{param}"] = distance
            except Exception as e:
                warnings.warn(
                    f"Could not calculate Wasserstein distance for {param}: {e}"
                )
        ...
        for param in ["gravity", "friction", "elasticity", "damping"]:
            if f"wasserstein_{param}" in distances:
                normalized_dist = (
                    distances[f"wasserstein_{param}"] / param_ranges[param]
                )
                normalized_distances.append(normalized_dist)

        distances["overall_distribution_distance"] = (
            np.mean(normalized_distances) if normalized_distances else 0.0
        )

        return distances

    def evaluate_physics_plausibility(
        self, sample_size: Optional[int] = None
    ) -> Dict[str, float]:
    ...
    def run_comprehensive_evaluation(
        ...
        results = DistributionInventionResults(
            interpolation_accuracy=all_results.get(
                "interpolation_overall_accuracy", 0.0
            ),
            extrapolation_accuracy=all_results.get(
                "extrapolation_overall_accuracy", 0.0
            ),
    ...
    def print_results_summary(self, results: DistributionInventionResults):
        ...
        if results.invention_score > 0.7:
            ...
            print(
                "  ❌ Poor generalization - primarily interpolating within training distribution"
            )
        ...
        if results.regime_specific_results:
            ...
            for regime, regime_results in results.regime_specific_results.items():
                if (
                    isinstance(regime_results, dict)
                    and "overall_accuracy" in regime_results
                ):
                    print(f"  {regime:>12}: {regime_results['overall_accuracy']:.1%}")


def evaluate_model_with_improved_metrics(
    model, datasets_path: Dict[str, str], sample_size: Optional[int] = None
) -> DistributionInventionResults:
    """
