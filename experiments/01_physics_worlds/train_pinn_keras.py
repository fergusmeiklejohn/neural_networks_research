...
class PhysicsDataset:
    ...
    def generate_trajectory(self, gravity=-9.8, friction=0.5, elasticity=0.8, damping=0.95):
    ...
    def train_step(self, state, data):
        ...
        with self.compute_gradient_tape() as tape:
            ...
            if self.pinn.use_physics_loss:
                ...
                physics_losses = self.pinn.compute_physics_loss(full_predictions, physics_targets)
...
def main():
    ...
    for name, data, params in [
        ...
        print(f"{name}: MSE={eval_result['mse']:.4f}, Gravity Error={eval_result['gravity_error']:.2f}")
    ...
    # Combine all data
    X_train_3 = np.concatenate([train_em[:, :-1], train_moon[:, :-1], train_jupiter[:, :-1]])
    y_train_3 = np.concatenate([train_em[:, 1:], train_moon[:, 1:], train_jupiter[:, 1:]])
