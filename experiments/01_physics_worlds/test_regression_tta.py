...
def main():
    ...
    tta_configs = [
        ...
        # Regression-specific TTA
        {
            ...
            "lr": 1e-3,
            "steps": 5,
            "name": "RegressionTTA (lr=1e-3)",
        },
        {
            "method": "regression",
            "lr": 5e-4,
            "steps": 5,
            "name": "RegressionTTA (lr=5e-4)",
        },
        {
            "method": "regression",
            "lr": 1e-4,
            "steps": 5,
            "name": "RegressionTTA (lr=1e-4)",
        },
        {
            "method": "regression",
            "lr": 1e-4,
            "steps": 10,
            "name": "RegressionTTA (10 steps)",
        },
        # Physics-aware regression TTA
        {
            "method": "physics_regression",
            "lr": 5e-4,
            "steps": 5,
        ...
        },
        {
            ...
            "lr": 1e-4,
            "steps": 10,
            "name": "PhysicsRegressionTTA (10 steps)",
        },
    ]
    ...
    for config in tta_configs:
        ...
        results.append(
            {"name": config["name"], "mse": mean_mse, "improvement": improvement}
        )
    ...
    tta_model = TTAWrapper(
        model, tta_method="regression", learning_rate=1e-3, adaptation_steps=10
    )
    ...
    print(
        f"Weight restoration: {'✓ WORKING' if restoration_error < 1e-3 else '✗ FAILED'}"
    )
    ...
    for i, result in enumerate(results[:3]):
        ...
        print(
            f"   MSE: {result['mse']:.4f}, Improvement: {result['improvement']:+.1f}%"
        )
