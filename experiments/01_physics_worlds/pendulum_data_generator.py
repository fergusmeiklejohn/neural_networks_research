...
class PendulumSimulator:
    ...
    def simulate(
        self, theta0: float, theta_dot0: float, duration: float
    ) -> Dict[str, np.ndarray]:
        ...
        for i in range(steps):
            ...
            if i < steps - 1:
                ...
                k2v = self._compute_acceleration(
                    theta + k1x * dt / 2, theta_dot + k1v * dt / 2, t + dt / 2
                )
                k2x = theta_dot + k1v * dt / 2

                k3v = self._compute_acceleration(
                    theta + k2x * dt / 2, theta_dot + k2v * dt / 2, t + dt / 2
                )
                k3x = theta_dot + k2v * dt / 2

                k4v = self._compute_acceleration(
                    theta + k3x * dt, theta_dot + k3v * dt, t + dt
                )
...
class PendulumDataGenerator:
    ...
    def _generate_random_pendulum_config(
        self, mechanism: str = "fixed"
    ) -> PendulumConfig:
        ...
        try:
            ...
            if self._validate_trajectory(trajectory):
                return {
                    ...
                    "initial_conditions": {"theta": theta0, "theta_dot": theta_dot0},
                    "mechanism": mechanism,
                }
    ...
    def generate_dataset(
        self, mechanism: str = "fixed", num_samples: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        with mp.Pool() as pool:
            trajectories = list(
                tqdm(pool.imap(self._generate_trajectory, args_list), total=num_samples)
            )
...
def main():
    ...
    config = PendulumDataConfig(
        num_samples=10000, sequence_length=300  # Full dataset  # 5 seconds at 60fps
    )
