...
def main():
    ...
    for result in sorted(results, key=lambda x: x['mse'] if x['mse'] != float('inf') else 1e10):
        mse = result['mse']
        if mse == float('inf'):
            mse_str = "∞"
            ratio_str = "∞"
        ...
        print(f"| {result['model']:<20} | {mse_str:<12} | {ratio_str:<15} | {result['status']} |")
    ...
    results_data = {
        ...
        'summary': {
            ...
            'all_failed': all(r['mse'] > 1000 for r in results if r['model'] != 'GraphExtrap (paper)')
        }
    }
