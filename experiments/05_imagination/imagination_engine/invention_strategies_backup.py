...
class InventionStrategies:
    ...
    def counting_arithmetic(self, examples):
        ...
        if not self._validate_examples(examples):
            return None
            
        self._log("Trying counting and arithmetic operations...")
        ...
        def count_and_fill(grid: np.ndarray) -> np.ndarray:
            ...
            for color in non_bg_colors:
                ...
                # Fill first 'count' positions with a pattern
                filled = 0
                for i in range(h):
                    for j in range(w):
                        if filled < count and result[i, j] == 0:
                            result[i, j] = color
                            filled += 1
                        if filled >= count:
                            break
        ...
        # Try creating patterns based on position arithmetic
        def position_arithmetic(grid: np.ndarray) -> np.ndarray:
            result = grid.copy()
            h, w = grid.shape
            
            # Try modular patterns
            for i in range(h):
                for j in range(w):
                    if grid[i, j] == 0:  # Only modify background
        ...
        score = self._validate(position_arithmetic, examples)
        if score > 0.4:
            return InventedPrimitive(
                name="position_arithmetic",
                program="Create patterns using position arithmetic",
                function=position_arithmetic,
                atomic_sequence=["calculate", "set_pixel"],
                score=score,
                invention_time=0.0
            )
        
        # Try size-based operations
        def size_based_transform(grid: np.ndarray) -> np.ndarray:
            result = grid.copy()
            h, w = grid.shape
            
            # Different transforms based on grid size
            if h == w:  # Square grid
                # Add diagonal
                ...
                # Non-square, use different pattern
                center_h, center_w = h // 2, w // 2
                if result[center_h, center_w] == 0:
                    result[center_h, center_w] = 5
            
            return result
        
        score = self._validate(size_based_transform, examples)
        if score > 0.4:
            return InventedPrimitive(
                name="size_based_transform",
                program="Transform based on grid dimensions",
                function=size_based_transform,
                atomic_sequence=["measure", "conditional", "set_pixel"],
                score=score,
                invention_time=0.0
            )
    ...
    def counting_arithmetic(self, examples):
        ...
        self._log("Trying symmetry operations...")
        ...
        score = self._validate(rotate_180, examples)
        if score > 0.7:
            return InventedPrimitive(
                name="rotate_180",
                program="Rotate grid 180 degrees",
                function=rotate_180,
                atomic_sequence=["rotate", "rotate"],
                score=score,
                invention_time=0.0
            )
        
        # Try transpose (diagonal reflection)
        def transpose(grid: np.ndarray) -> np.ndarray:
            return np.transpose(grid)
        
        score = self._validate(transpose, examples)
        if score > 0.7:
            return InventedPrimitive(
                name="transpose",
                program="Transpose grid (diagonal reflection)",
                function=transpose,
                atomic_sequence=["transpose"],
                score=score,
                invention_time=0.0
            )
        
        # Try creating symmetrical version of input
        def make_symmetric_horizontal(grid: np.ndarray) -> np.ndarray:
            h, w = grid.shape
            mid = w // 2
            result = grid.copy()
            ...
            for i in range(h):
                for j in range(mid):
                    if mid + (mid - 1 - j) < w:
                        result[i, mid + (mid - 1 - j)] = result[i, j]
            
            return result
        
        score = self._validate(make_symmetric_horizontal, examples)
        if score > 0.5:
            return InventedPrimitive(
                name="make_symmetric_horizontal",
                program="Make grid horizontally symmetric by mirroring left half",
                function=make_symmetric_horizontal,
                atomic_sequence=["copy_region", "flip_horizontal", "overlay"],
                score=score,
                invention_time=0.0
            )
