#!/usr/bin/env python3
"""Automated primitive discovery system for ARC tasks - Version 4.

Enhanced with diagonal patterns, shape detection, and improved pattern coverage.
"""
...
class PrimitiveDiscovererV4(PrimitiveDiscovererV3):
    """Enhanced discoverer with diagonal and shape patterns."""
    ...
    def _detect_triangles(self, inp, out):
        ...
        for i in range(min(h, w)):
            for j in range(i + 1):
                if out[i, j] == 0 or inp[i, j] == out[i, j]:
                    is_triangle = False
                    break
            if not is_triangle:
                break

        if is_triangle and h > 2 and w > 2:
            triangles.append({"type": "top_left", "size": min(h, w)})
...
def test_enhanced_discovery():
    ...
    print("=" * 80)
    print("Testing Enhanced Primitive Discovery (V4)")
    print("=" * 80)

    discoverer = PrimitiveDiscovererV4(verbose=True)
