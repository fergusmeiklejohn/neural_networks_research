...
from minimal_binding_scan import MinimalBindingModel, test_binding_capability
...
def test_integration():
    ...
    logger.info(f"Generated task: {' '.join(command_words)} -> {' '.join(action_words)}")
    
    # Encode it
    command_encoded = generator.encode_words(command_words)
    action_encoded = generator.encode_actions(action_words)
    ...
    if actual_seq_len >= expected_seq_len:
        ...
        logger.warning(f"✗ Output length {actual_seq_len} < expected {expected_seq_len}")
