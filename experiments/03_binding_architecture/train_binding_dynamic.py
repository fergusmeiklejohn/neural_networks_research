...
class DynamicMemoryModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.pattern_detector = nn.Linear(embed_dim * 3, 2)  # Binary: is_storage_pattern
            ...
            for i in range(seq_len):
                ...
                if token == VOCAB['do']:
                    ...
                    action_mask = mx.where(
                        (mx.arange(batch_size)[:, None] == b) & (mx.arange(seq_len) == i),
                        1.0,
                        action_mask
                    )
    ...
    def __call__(self, 
        ...
        for t in range(seq_len):
            ...
            if mx.sum(stores_value) > 0:
                ...
                value_embeds = word_embeds[batch_indices, value_pos]  # (batch, embed_dim)
                processed_values = self.value_extractor(value_embeds)  # (batch, embed_dim)
                ...
                update_weights = binding_scores.squeeze(1) * stores_value  # (batch, num_slots)
                updates = update_weights[:, :, None] * processed_values[:, None, :]  # (batch, num_slots, embed_dim)
...
def test_specific_examples(model):
    ...
    for cmd_str, expected in test_cases:
        ...
        print(f"  Storage positions: {[i for i, m in enumerate(storage_mask) if m > 0]}")
...
def main():
    parser = argparse.ArgumentParser(description='Dynamic memory model for variable binding')
