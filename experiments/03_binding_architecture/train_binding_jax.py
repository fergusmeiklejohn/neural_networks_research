"""
JAX-compatible training script for Minimal Binding Model.

This version uses JAX's functional approach to gradient computation.
"""
...
class BindingTrainer:
    """Handles training of the binding model with JAX."""
    ...
    def compute_loss(self, commands, actions, training=True):
        """Compute loss for a batch."""
        outputs = self.model({'command': commands}, training=training)
        action_logits = outputs['action_logits']
        loss = self.loss_fn(actions, action_logits)
        return loss, outputs
        
    def train_step_jax(self, commands, actions):
        """Single training step using JAX gradient computation."""
        # Convert to JAX arrays if needed
        if not isinstance(commands, jnp.ndarray):
            commands = jnp.array(commands)
        if not isinstance(actions, jnp.ndarray):
            actions = jnp.array(actions)
        
        # Define loss function for JAX
        def loss_fn(trainable_variables):
            # Temporarily update model variables
            for var, new_val in zip(self.model.trainable_variables, trainable_variables):
                var.assign(new_val)
        ...
        # Get current variables as JAX arrays
        current_vars = [jnp.array(var.numpy()) for var in self.model.trainable_variables]
        
        # Compute gradients using JAX
        loss_val, grads = jax.value_and_grad(loss_fn)(current_vars)
        
        # Convert gradients back to numpy and apply
        grads_numpy = [np.array(g) for g in grads]
        self.optimizer.apply_gradients(zip(grads_numpy, self.model.trainable_variables))
        
        # Update metrics
        self.train_loss_metric.update_state(loss_val)
        
        # Get outputs for metrics
        outputs = self.model({'command': commands}, training=False)
        self.train_acc_metric.update_state(actions, outputs['action_logits'])
        
        return float(loss_val), outputs['bindings']
        
    def train_step_simple(self, commands, actions):
        """Alternative: Simple training step without manual gradients."""
    ...
    def evaluate(self, dataset, split_name='val'):
        ...
        for i in range(n_batches):
            ...
            for j in range(len(batch_commands)):
                pred = predictions_np[j]
                target = batch_actions[j]
                
                # Find non-padding positions
                non_pad = target != 0
                
                if np.all(pred[non_pad] == target[non_pad]):
                    total_correct += 1
    ...
    def test_modification_capability(self):
        ...
        for test_case in test_cases:
            ...
            # Convert to numpy
            orig_pred_np = orig_pred.numpy() if hasattr(orig_pred, 'numpy') else np.array(orig_pred)
            ...
            # Convert to numpy
            mod_pred_np = mod_pred.numpy() if hasattr(mod_pred, 'numpy') else np.array(mod_pred)
            
            # Check if predictions match expected
            orig_correct = np.all(orig_pred_np[0, :len(orig_act)] == orig_act)
            mod_correct = np.all(mod_pred_np[0, :len(mod_act)] == mod_act)
    ...
    def train(self, n_epochs: int = 50, batch_size: int = 32, use_jax_gradients: bool = False):
        """Full training loop."""
        ...
        logger.info(f"Using {'JAX' if use_jax_gradients else 'Keras'} gradient computation")
        
        # Choose training step method
        train_step = self.train_step_jax if use_jax_gradients else self.train_step_simple
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                loss, bindings = train_step(batch_commands, batch_actions)
...
def main():
    ...
    logger.info("Starting Minimal Binding Model training (JAX-compatible)...")
    
    # Create output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = get_output_path() / f"binding_model_jax_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Train model - use Keras gradients by default (simpler and more stable)
    history = trainer.train(n_epochs=50, batch_size=32, use_jax_gradients=False)
    ...
    if final_success_rate > 0.5:
        logger.info("\n✓ SUCCESS: Model achieves >50% modification success rate!")
    else:
