...
def train_step(model, optimizer, loss_fn, batch_commands, batch_actions):
    ...
    accuracy = tf.reduce_mean(
        tf.cast(tf.equal(predictions, batch_actions_padded), tf.float32) * tf.cast(mask, tf.float32)
    )
...
def test_specific_examples(model, generator):
    ...
    for command_str, expected_str in test_cases:
        ...
        logger.info(f"Command: '{command_str}' -> Expected: '{expected_str}', "
...
def main(epochs=20, batch_size=32, learning_rate=0.001):
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"  Train - Loss: {avg_train_loss:.4f}, Accuracy: {avg_train_accuracy:.4f}")
    ...
    for case in mod_test_cases:
        ...
        logger.info(f"  {case['modification_type']}: "
                   f"Original {'✓' if orig_correct else '✗'}, "
                   f"Modified {'✓' if mod_correct else '✗'}")
    
    mod_success_rate = sum(r['modified_correct'] for r in mod_results) / len(mod_results)
