...
class BindingTrainer:
    ...
    def train_step_jax(self, commands, actions):
        ...
        current_vars = [
            jnp.array(var.numpy()) for var in self.model.trainable_variables
        ]
    ...
    def train_step_simple(self, commands, actions):
        ...
        metrics = self.model.train_on_batch({"command": commands}, y_dict)
    ...
    def test_modification_capability(self):
        ...
        for test_case in test_cases:
            ...
            # Convert to numpy
            mod_pred_np = (
                mod_pred.numpy() if hasattr(mod_pred, "numpy") else np.array(mod_pred)
            )
    ...
    def train(
        self, n_epochs: int = 50, batch_size: int = 32, use_jax_gradients: bool = False
    ):
        """Full training loop."""
        ...
        logger.info(
            f"Using {'JAX' if use_jax_gradients else 'Keras'} gradient computation"
        )

        # Choose training step method
        train_step = (
            self.train_step_jax if use_jax_gradients else self.train_step_simple
        )
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch + 1) % 25 == 0:
                checkpoint_path = os.path.join(
                    self.output_dir, f"binding_model_epoch_{epoch + 1}.keras"
                )
