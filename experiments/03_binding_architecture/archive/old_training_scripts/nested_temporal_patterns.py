...
class NestedTemporalParser:
    ...
    def __init__(self, vocab: Dict[str, int]):
        ...
        self.temporal_tokens = {
            k: v for k, v in self.temporal_tokens.items() if k != -1
        }
    ...
    def parse_temporal_sequence(
        self, tokens: List[int], start: int, end: int
    ) -> Tuple[Optional[TemporalNode], int]:
        ...
        if not modifiers:
            ...
            return TemporalNode(modifier=TemporalModifier.ONCE, base_action=var_name), i

        # Build from innermost (rightmost) to outermost (leftmost)
        current_node = TemporalNode(modifier=modifiers[-1], base_action=var_name)

        # Wrap with remaining modifiers
        for modifier in reversed(modifiers[:-1]):
            current_node = TemporalNode(modifier=modifier, children=[current_node])
...
class NestedTemporalExecutor:
    ...
    def expand_temporal_node(
        self, node: TemporalNode, bindings: Dict[str, str]
    ) -> List[str]:
    ...
    def execute_nested_pattern(
        self, tokens: List[int], bindings: Dict[str, str]
    ) -> List[mx.array]:
...
def test_nested_temporal_patterns():
    ...
    for i, pattern in enumerate(all_patterns):
        print(
            f"  Pattern {i+1}: {describe_temporal_node(pattern)} - {pattern.compute_repetitions()} repetitions"
        )
