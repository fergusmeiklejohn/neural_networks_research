...
class SequencePlanner:
    ...
    def extract_command_from_segment(self, tokens: mx.array, segment: Tuple[int, int]) -> mx.array:
...
class SequentialDynamicMemoryModel(nn.Module):
    ...
    def detect_temporal_modifiers(self, tokens: mx.array, position: int) -> Tuple[bool, int, Optional[str]]:
        """
    ...
    def process_segment(self, command_ids: mx.array, segment: Tuple[int, int], 
                       slot_values: mx.array, bindings: Dict, stage: str) -> Tuple[List[mx.array], List[mx.array]]:
        ...
        for t in range(segment_length):
            ...
            if is_temporal and target_var is not None:
                ...
                binding_scores_unsqueezed = binding_scores[:, None, :]  # (batch, 1, num_slots)
                retrieved = (binding_scores_unsqueezed @ slot_values).squeeze(1)  # (batch, embed_dim)
...
class BindingAttention(nn.Module):
    ...
    def __call__(self, query: mx.array, keys: mx.array, training: bool = True) -> Tuple[mx.array, mx.array]:
        ...
        K = mx.repeat(mx.expand_dims(K, 0), batch_size, axis=0)  # (batch, num_slots, num_heads, head_dim)
        ...
        if training:
            binding_weights = self.gumbel_softmax(scores, temperature=self.temperature, hard=True)
        else:
            # During inference, use hard argmax
            binding_weights = mx.zeros_like(scores)
            indices = mx.argmax(scores, axis=-1, keepdims=True)
            binding_weights = mx.put_along_axis(binding_weights, indices, mx.array(1.0), axis=-1)
...
def train_step(model: SequentialDynamicMemoryModel, batch: Dict, 
    ...
    def loss_fn(model, command_ids, action_ids, stage):
        ...
        if 'recognition_logits' in recog_out:
            ...
            if mx.any(var_mask):
                ...
                losses['recognition'] = nn.losses.cross_entropy(
                    recog_logits[var_mask], 
                    recog_targets[var_mask],
                    reduction='mean'
                )
            else:
        ...
        for stage_name, stage_out in [('retrieval', retrieval_out), ('binding', binding_out)]:
        ...
        total_loss = losses['recognition'] * 0.2 + losses.get('retrieval', 0) * 0.3 + losses.get('binding', 0) * 0.5
...
def evaluate_model(model: SequentialDynamicMemoryModel, dataset: Dict, 
                  stage: str = "full_binding") -> Dict:
    ...
    for i in range(len(dataset['commands'])):
        ...
        if stage == "recognition":
            ...
            var_positions = [j for j, token in enumerate(command_tokens) if token in ['X', 'Y', 'Z']]
            
            if all(predictions[0, j].item() == VOCAB[command_tokens[j]] for j in var_positions):
                correct_stage1 += 1
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_interval == 0:
            ...
            print(f"Stage 1 (Recognition): {stage1_results['stage1_accuracy']*100:.1f}%")
            print(f"Stage 2 (Retrieval): {stage2_results['stage2_accuracy']*100:.1f}%")
            print(f"Stage 3 (Full Binding): {stage3_results['stage3_accuracy']*100:.1f}%")
            
            if 'sequential_accuracy' in stage3_results:
                print(f"Sequential Patterns: {stage3_results['sequential_accuracy']*100:.1f}% "
    ...
    if 'sequential_accuracy' in final_results:
        print(f"Sequential Pattern Accuracy: {final_results['sequential_accuracy']*100:.1f}%")
