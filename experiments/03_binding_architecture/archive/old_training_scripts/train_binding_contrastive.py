...
class FocusedBindingAttention(nn.Module):
    ...
    def __init__(
        ...
        self.var_type_embed = nn.Embedding(
            10, embed_dim
        )  # Support up to 10 variable types
...
class ContrastiveBindingModel(nn.Module):
    ...
    def __call__(self, command_ids, training=True):
        ...
        slot_values_exp = mx.broadcast_to(
            slot_values_exp, (batch_size, seq_len, self.num_slots, self.embed_dim)
        )
...
def generate_binding_batch(batch_size, vocab_size):
    ...
    for _ in range(batch_size):
        if np.random.random() < 0.3:
            ...
            actions_chosen = np.random.choice(
                ["jump", "walk", "turn", "run"], size=2, replace=False
            )
            cmd = [
                var_to_id[vars[0]],
                word_to_id["means"],
                word_to_id[actions_chosen[0]],
                var_to_id[vars[1]],
                word_to_id["means"],
                word_to_id[actions_chosen[1]],
                word_to_id["do"],
                var_to_id[vars[0]],
                word_to_id["then"],
                var_to_id[vars[1]],
            ...
            act = [
                action_to_id[actions_chosen[0].upper()],
                action_to_id[actions_chosen[1].upper()],
            ]
...
def train_step(model, commands, actions, optimizer):
    ...
    def loss_fn(model):
        ...
        if mx.any(mask):
            ce_loss = mx.sum(
                nn.losses.cross_entropy(logits_flat, actions_flat) * mask
            ) / mx.sum(mask)
        else:
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"  Loss: {epoch_losses['total']/steps_per_epoch:.3f} "
            f"(CE: {epoch_losses['ce']/steps_per_epoch:.3f}, "
            f"Contrast: {epoch_losses['contrast']/steps_per_epoch:.3f})"
        )
        print(
            f"  Entropy: {epoch_losses['entropy']/steps_per_epoch:.3f}, "
...
def test_modification_capability(model):
    ...
    print(
        f"X means jump do X -> {id_to_action.get(pred1, 'UNK')} {'✓' if pred1 == 1 else '✗'}"
    )
    print(
        f"X means walk do X -> {id_to_action.get(pred2, 'UNK')} {'✓' if pred2 == 2 else '✗'}"
    )
    ...
    print(f"\nBinding analysis:")
    print(
        f"  X bindings in test 1: position 0 -> slot {bind1[0]}, position 4 -> slot {bind1[4]}"
    )
    print(
        f"  X bindings in test 2: position 0 -> slot {bind2[0]}, position 4 -> slot {bind2[4]}"
    )
    print(
        f"  Consistent: {'✓' if bind1[0] == bind1[4] and bind2[0] == bind2[4] else '✗'}"
    )
