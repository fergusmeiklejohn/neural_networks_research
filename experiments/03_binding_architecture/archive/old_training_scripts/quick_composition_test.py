...
def test_patterns_with_fresh_model():
    ...
    test_cases = [
        ...
        (
            "Temporal sequence",
            "X means jump do X twice then do X",
            "Complex: Combines temporal and sequential",
        ),
        # Multiple variables
        (
            "Three variables",
            "X means jump Y means walk Z means turn do Y",
            "Should work: Within 4-slot limit",
        ),
        # Long-range
        (
            "Long-range",
            "X means jump Y means walk Z means turn now do X",
    ...
    for name, pattern, expectation in test_cases:
        ...
        try:
            ...
            print(
                f"  Model output: Temporal actions = {outputs.get('temporal_actions', 0)}"
            )
