...
class SimpleVersionedMemory:
    ...
    def retrieve(self, var_name: str, timestamp: int) -> Optional[str]:
        ...
        print(
            f"  [T={timestamp}] Retrieved '{var_name}' → '{value}' from slot {slot_id}"
        )
        return value
...
class StaticMemory:
    ...
    def bind(self, var_name: str, value: str, timestamp: int) -> None:
        ...
        if slot_id in self.memory:
            print(
                f"  [T={timestamp}] FAILED to bind '{var_name}' → '{value}' (slot {slot_id} already occupied)"
            )
            return
...
def demonstrate_rebinding_problem():
    ...
    complex_command = (
        "X means jump do X twice then X means walk do X then X means turn do X"
    )
