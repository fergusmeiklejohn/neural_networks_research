...
class BindingAttentionMLX(nn.Module):
    ...
    def __call__(self, word_embeds, memory_keys):
        ...
        memory_keys_proj = nn.Linear(memory_keys.shape[-1], self.hidden_dim)(
            memory_keys
        )
    ...
    def __call__(self, command_embeds, bound_variables):
        ...
        for t in range(seq_len):
            decoder_hidden = self.decoder_cell(
                attended_command[:, t, :], decoder_hidden
            )
...
class MinimalBindingModelMLX(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        bindings, binding_scores = self.binder(
            embeds_proj, self.variable_memory.slot_keys
        )
...
def train_mlx(epochs=20, batch_size=32, learning_rate=0.001):
    ...
    mx.save_safetensors(
        os.path.join(output_dir, "model_weights.safetensors"), weights_dict
    )
