    ...
    def bind_versioned(
        ...
        for b in range(batch_size):
            ...
            if slot_idx not in new_memory:
                new_memory[slot_idx] = {"versions": [], "current_version": -1}
            ...
            new_memory[slot_idx]["current_version"] = (
                len(new_memory[slot_idx]["versions"]) - 1
            )
            ...
            if len(new_memory[slot_idx]["versions"]) > self.max_versions:
                ...
                new_memory[slot_idx]["current_version"] = min(
                    new_memory[slot_idx]["current_version"], self.max_versions - 1
                )

        return new_memory

    def retrieve_versioned(
        self, binding_scores: mx.array, memory_state: Dict, position: int
    ) -> mx.array:
        ...
        # Weighted retrieval
        retrieved = mx.sum(binding_scores[:, :, None] * value_matrix, axis=1)
...
class VersionedMemoryModel(nn.Module):
    ...
    def detect_binding_pattern(
        self, tokens: List[int], position: int
    ...
    def process_segment_versioned(
        ...
        segment_list = [
            int(segment_tokens[0, i]) for i in range(segment_tokens.shape[1])
        ]
        ...
        for t in range(segment_tokens.shape[1]):
            ...
            is_binding, var_pos, val_pos = self.detect_binding_pattern(segment_list, t)
            ...
            if is_binding and var_pos is not None and val_pos is not None:
                ...
                binding_scores = self.binder(
                    var_embed, self.memory.slot_keys, self.memory.slot_keys
                )
            ...
            # Check for action positions
                ...
                binding_scores = self.binder(
                    current_embed, self.memory.slot_keys, self.memory.slot_keys
                )

                retrieved = self.memory.retrieve_versioned(
                    binding_scores, memory_state, global_position
                )
    ...
    def __call__(self, command_ids: mx.array, stage: str = "full_binding") -> Dict:
        ...
        for segment in segments:
            results = self.process_segment_versioned(
                command_ids, segment, memory_state, position_offset, stage
            )
...
def generate_rebinding_data(num_samples: int = 100) -> List[Dict]:
    ...
    for _ in range(num_samples):
        ...
        if pattern_type == "basic":
            ...
            action3 = np.random.choice(
                [a for a in actions if a not in [action1, action2]]
            )
        ...
        data.append(
            {"command": command, "expected": expected, "pattern_type": pattern_type}
        )
