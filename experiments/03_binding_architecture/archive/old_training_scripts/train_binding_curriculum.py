...
VOCAB = {
    ...
    "what": 13,
    "is": 14,
    "recall": 15,  # New tokens for curriculum
}
...
class CurriculumBindingModel(nn.Module):
    ...
    def __init__(
        ...
        self.recognition_head = nn.Linear(
...
class BindingAttention(nn.Module):
    ...
    def gumbel_softmax(
        ...
        gumbel_noise = -mx.log(
            -mx.log(mx.random.uniform(shape=logits.shape) + 1e-8) + 1e-8
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Curriculum learning for variable binding"
    )
...
def test_modification_capability(model):
    ...
    for i, test in enumerate(test_cases):
        ...
        # Check success
        if (
            orig_actions == test["expected_orig"]
            and mod_actions == test["expected_mod"]
        ):
            print("  Success: ✓")
            successes += 1
        else:
