...
class CompositionalBindingModel(nn.Module):
    ...
    def process_segment_versioned(self, command_ids: mx.array, segment: Tuple[int, int],
                                  bindings: Dict[str, str], stage: str) -> List[mx.array]:
        ...
        for i, token_id in enumerate(segment_tokens):
            ...
            if token_str and (token_str in bindings or token_str in ['jump', 'walk', 'turn', 'run'] or token_str in ['X', 'Y', 'Z', 'W']):
                ...
                if i < encoded.shape[1]:
                    ...
                    action_logits = self.action_head(token_encoding)  # Shape: [1, num_actions]
...
def train_compositional_model(model: CompositionalBindingModel, 
                              train_data: List[Tuple[str, List[str]]],
                              num_epochs: int = 50,
                              batch_size: int = 32,
                              learning_rate: float = 0.001) -> CompositionalBindingModel:
...
def test_compositional_model(model: CompositionalBindingModel):
    ...
    for command, expected in test_cases:
        ...
        if 'or' in command and len(predicted_actions) == 1:
            is_correct = predicted_actions[0] in expected or predicted_actions[0] in ['JUMP', 'WALK', 'RUN', 'TURN']
        else:
