...
class TemporalDynamicMemoryModel(nn.Module):
    ...
    def detect_temporal_patterns(
        self, command_ids: mx.array, position: int
        ...
        for offset in [1, 2]:
            if position - offset >= 0:
                ...
                is_variable = mx.logical_or(
                    prev_token == VOCAB["X"],
                    mx.logical_or(prev_token == VOCAB["Y"], prev_token == VOCAB["Z"]),
                )
    ...
    def __call__(
        ...
        for t in range(seq_len):
            ...
            is_temporal, repeat_count, var_id = self.detect_temporal_patterns(
                command_ids, t
            )
            ...
            if is_temporal and var_id is not None:
                ...
                _, var_binding_scores = self.binder(
                    var_embeds, self.slot_keys, training=False
                )

                # Retrieve value using binding
                retrieved = (var_binding_scores @ slot_values).squeeze(
                    1
                )  # (batch, embed_dim)
        ...
        if temporal_actions:
            ...
            temporal_stacked = mx.stack(
                temporal_actions, axis=1
            )  # (batch, num_temporal, embed_dim)
...
class BindingAttention(nn.Module):
    ...
    def __call__(
        ...
        K = mx.broadcast_to(
            K[None, :, :, :], (batch_size, self.num_heads, num_slots, self.head_dim)
        )
        ...
        # scores = Q @ K^T
        scores = (Q @ mx.transpose(K, (0, 1, 3, 2))).squeeze(
            2
        ...
        slot_probs = self.gumbel_softmax(
            scores, temperature=self.temperature, hard=training
        )
...
def main():
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        for epoch in range(args.epochs_per_stage):
            ...
            print(
                f"⚠ Stage {stage} completed {args.epochs_per_stage} epochs "
