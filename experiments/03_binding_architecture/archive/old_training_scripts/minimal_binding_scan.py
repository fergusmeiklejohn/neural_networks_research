...
class BindingAttention(layers.Layer):
    ...
    def call(self, word_embeds, memory_keys, training=None):
        ...
        binding_probs = ops.softmax(scores, axis=-1)  # (batch, n_heads, seq_len, n_slots)
...
class BoundVariableExecutor(layers.Layer):
    ...
    def call(self, command_embeds, bound_variables, training=None):
        ...
        attended_command = self.cross_attention(
            query=command_encoded,
            value=bound_variables,
            key=bound_variables
        )
...
class MinimalBindingModel(keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        bindings, binding_scores = self.binder(variables, self.variable_memory.slot_keys)
        ...
        bound_variables = ops.stack(bound_variables, axis=1)  # (batch, seq_len, slot_dim)
...
def create_dereferencing_task(vocab: Dict[str, int], action_vocab: Dict[str, int]) -> Tuple[np.ndarray, np.ndarray]:
    """
