...
class BindingAttention(nn.Module):
    ...
    def __init__(
        self, embed_dim: int = 128, num_heads: int = 8, temperature: float = 1.0
    ):
        super().__init__()
    ...
    def __call__(
        ...
        scores = mx.mean(
            scores, axis=1
        )  # Average over heads: (batch, seq_len, num_slots)

        if training:
            # Use Gumbel-Softmax during training for differentiable binding
            soft_scores, hard_indices = self.gumbel_softmax(
                scores, self.temperature, hard=True
            )
...
class BoundVariableExecutor(nn.Module):
    ...
    def __init__(
        self, embed_dim: int = 128, hidden_dim: int = 256, num_actions: int = 10
    ):
        super().__init__()
        self.embed_dim = embed_dim
...
class ProperBindingModel(nn.Module):
    ...
    def __call__(
        ...
        bindings, binding_scores = self.binder(
            word_embeds, slot_keys, training=training
        )
        ...
        slot_values_expanded = slot_values[
            None, None, :, :
        ]  # (1, 1, num_slots, embed_dim)
        slot_values_expanded = mx.broadcast_to(
            slot_values_expanded, (batch_size, seq_len, num_slots, embed_dim)
        )

        # Use soft attention to retrieve values (differentiable!)
        # binding_scores needs to be expanded to match slot_values_expanded
        binding_weights = binding_scores[
            :, :, :, None
        ]  # (batch, seq_len, num_slots, 1)

        # Weighted sum over slots
        bound_values = mx.sum(
            slot_values_expanded * binding_weights, axis=2
...
def generate_batch_from_dataset(generator, batch_size):
    ...
    return {"command": mx.array(padded_commands), "action": mx.array(padded_actions)}
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"Epoch {epoch+1}/{args.epochs} - Loss: {epoch_loss/num_batches:.4f}, "
