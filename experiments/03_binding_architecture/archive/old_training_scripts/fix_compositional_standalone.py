...
def separate_bindings_and_execution(tokens: List[int]) -> Tuple[Dict[int, int], List[int]]:
...
def resolve_and_execute(tree: Dict, bindings: Dict[int, int]) -> List[str]:
    ...
    if tree['type'] == 'leaf':
        ...
        left_actions = resolve_and_execute(tree['left'], bindings) if 'left' in tree else []
        right_actions = resolve_and_execute(tree['right'], bindings) if 'right' in tree else []
...
def process_command(command: str) -> Tuple[List[str], Dict]:
    ...
    vocab_reverse = {v: k for k, v in VOCAB.items()}
    debug_info = {
        'bindings': {vocab_reverse.get(k, ''): vocab_reverse.get(v, '') 
