...
from train_binding_mlx_proper import ProperBindingModel, train_step
...
def generate_phase3_batch(generator, batch_size):
    ...
    for _ in range(batch_size):
        ...
        if task_type == "multiple":
            ...
            action1, action2 = np.random.choice(
...
def train_phase(
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"  Epoch {epoch+1}/{num_epochs} - Loss: {epoch_loss/steps_per_epoch:.4f}, "
...
def main():
    ...
    print(
        f"  Direct pattern accuracy: {correct}/{len(test_patterns)} = {correct/len(test_patterns):.0%}"
    )
    ...
    phase2_acc = train_phase(
        model,
        generator,
        "Phase 2: Simple Variable Binding",
        generate_phase2_batch,
        optimizer,
        loss_fn,
    ...
    print(
        f"  Binding accuracy: {correct}/{len(test_bindings)} = {correct/len(test_bindings):.0%}"
    )
