...
from compositional_final_fix import FinalCompositionalParser, FinalExecutor, ParseNode, OperatorType
...
class RobustCompositionalModel(nn.Module):
    ...
    def _execute_tree(self, node: ParseNode, encoded: mx.array, bindings: Dict[int, str], 
                      command_ids: mx.array) -> List[mx.array]:
        ...
        if node.is_leaf():
            ...
            for i, token_pos in enumerate(range(node.start_pos, node.end_pos)):
                ...
                if token_str in ['X', 'Y', 'Z', 'W'] or token_str in ['jump', 'walk', 'turn', 'run']:
            ...
            if node.operator == OperatorType.AND:
                for child in node.children:
                    child_outputs = self._execute_tree(child, encoded, bindings, command_ids)
                    all_outputs.extend(child_outputs)
            
            elif node.operator == OperatorType.OR:
                # For OR, execute first child (simplified)
                if node.children:
                    child_outputs = self._execute_tree(node.children[0], encoded, bindings, command_ids)
                    all_outputs.extend(child_outputs)
...
def generate_comprehensive_data(num_samples: int = 1000) -> List[Tuple[str, List[str]]]:
    ...
    patterns = [
        # Basic AND
        lambda x, y, a1, a2: (f"{x} means {a1} {y} means {a2} do {x} and {y}", 
                              [a1.upper(), a2.upper()]),
        lambda x, y, a1, a2: (f"{x} means {a1} {y} means {a2} do {y} and {x}", 
                              [a2.upper(), a1.upper()]),
        
        # Basic THEN
        lambda x, y, a1, a2: (f"{x} means {a1} {y} means {a2} do {x} then {y}", 
                              [a1.upper(), a2.upper()]),
        
        # Complex patterns
        lambda x, y, a1, a2: (f"{x} means {a1} {y} means {a2} do {x} and {y} then {x}", 
                              [a1.upper(), a2.upper(), a1.upper()]),
        
        # Multiple do statements
        lambda x, y, a1, a2: (f"{x} means {a1} do {x} then {y} means {a2} do {y} and {x}", 
                              [a1.upper(), a2.upper(), a1.upper()]),
        
        # With modifiers
        lambda x, y, a1, a2: (f"{x} means {a1} do {x} twice and {y} means {a2} do {y}", 
                              [a1.upper(), a1.upper(), a2.upper()]),
        
        # OR operator
        lambda x, y, a1, a2: (f"{x} means {a1} {y} means {a2} do {x} or {y}", 
                              [a1.upper()]),  # Simplified: always choose first
