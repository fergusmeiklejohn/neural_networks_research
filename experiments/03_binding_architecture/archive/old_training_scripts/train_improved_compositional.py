...
def evaluate_compositional_accuracy(
    model: ImprovedCompositionalBindingModel, test_data: List[Dict]
) -> Dict[str, float]:
    ...
    for op_type in type_total:
        accuracy = (
            type_correct[op_type] / type_total[op_type]
            if type_total[op_type] > 0
            else 0
        )
...
def train_improved_compositional_model(num_epochs: int = 10, batch_size: int = 32):
    ...
    compositional_data = generate_improved_compositional_data(
        400
    )  # Focus on compositional
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(all_data):
            ...
            if outputs.shape[0] != labels.shape[0]:
                if outputs.shape[0] > labels.shape[0]:
                    ...
                    # Pad outputs
                    padding = mx.zeros(
                        (labels.shape[0] - outputs.shape[0], outputs.shape[-1])
                    )
            ...
            if i % 100 == 0 and i > 0:
                print(
                    f"  Batch {i}/{len(all_data)}: Loss = {float(loss):.4f}, "
        ...
        if epoch_acc > best_accuracy:
            ...
            save_path = os.path.join(
                get_output_path(), "improved_compositional_best.pkl"
            )
...
def test_specific_cases(model: ImprovedCompositionalBindingModel):
    ...
    test_cases = [
        ...
        (
            "X means jump Y means walk do X or Y",
            ["JUMP"],
        ),  # Should choose X deterministically
    ]
