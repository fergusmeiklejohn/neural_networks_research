...
from nested_temporal_patterns import NestedTemporalExecutor, NestedTemporalParser
...
def train_simple_nested(num_epochs: int = 10):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_data_shuffled):
            inputs = {"command": batch["command"], "stage": "full"}
            ...
            if outputs.shape[0] != labels.shape[0]:
                print(
                    f"Shape mismatch: outputs {outputs.shape} vs labels {labels.shape}"
                )
                if outputs.shape[0] > labels.shape[0]:
            ...
            if i % 50 == 0:
                print(
                    f"Batch {i}/{len(train_data)}, Loss: {float(loss):.4f}, Acc: {float(accuracy):.2%}"
                )
        ...
        if epoch_acc > best_accuracy:
            ...
            save_path = os.path.join(
                get_output_path(), "nested_temporal_simple_best.pkl"
            )
