...
class SequentialModelWithActionTracking(nn.Module):
    ...
    def process_segment_with_tracking(
        self,
            ...
            if is_storage:
                ...
                if t + 2 < segment_length:
                    ...
                    value_broadcast = mx.where(
                        mask, value_embed[:, None, :], value_broadcast
                    )
            ...
            for pos, repeat_count in action_positions:
                if t == pos:
                    ...
                    if var_id in bindings:
                        ...
                        retrieved = (one_hot_binding[:, None, :] @ slot_values).squeeze(
                            1
                        )
...
def test_improved_model():
    ...
    test_cases = [
        ...
        (
            "Z means turn do Z then X means run do X thrice",
            ["TURN", "RUN", "RUN", "RUN"],
        ),
    ]
