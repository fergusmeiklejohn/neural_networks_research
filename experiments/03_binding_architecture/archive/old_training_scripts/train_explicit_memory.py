...
class ExplicitMemoryModel(nn.Module):
    ...
    def __call__(self, words, training=True):
        ...
        memory = mx.broadcast_to(self.init_memory[None, :, :], 
                                (batch_size, self.num_slots, self.embed_dim))
        ...
        for t in range(seq_len):
            ...
            if t < 3:
                ...
                write_scores = mx.sum(memory * write_k[:, None, :], axis=2)  # (batch, num_slots)
                ...
                read_scores = mx.sum(memory * read_k[:, None, :], axis=2)  # (batch, num_slots)
                read_weights = mx.softmax(read_scores / 0.5, axis=1)  # Lower temperature for sharper attention
                all_read_weights.append(read_weights)
                
                # Read from memory
                read_weights_exp = read_weights[:, :, None]  # (batch, num_slots, 1)
                read_value = mx.sum(memory * read_weights_exp, axis=1)  # (batch, embed_dim)
...
def generate_memory_task_batch(batch_size=32):
    ...
    # Vocabulary
    vocab = {
        '<PAD>': 0, 'X': 1, 'Y': 2, 'A': 3, 'B': 4,
        'is': 5, 'recall': 6,
        'jump': 7, 'walk': 8, 'turn': 9, 'run': 10
    }
    
    actions = {
        'jump': 1, 'walk': 2, 'turn': 3, 'run': 4
    }
    ...
    for _ in range(batch_size):
        ...
        sentence = [
            vocab[vars[0]], vocab['is'], vocab[acts[0]],
            vocab[vars[1]], vocab['is'], vocab[acts[1]],
            vocab['recall'], vocab[vars[0]],
            vocab['recall'], vocab[vars[1]]
        ]
        ...
        for _ in range(50):  # 50 batches per epoch
            ...
            def loss_fn(model):
                ...
                if write_weights:
                    ...
                    # Total loss
                    total_loss = ce_loss - 0.01 * write_entropy  # Negative because we want high entropy
                else:
                    total_loss = ce_loss
    ...
    for sentence, description in test_cases:
        ...
        if write_weights:
            ...
            for i, w in enumerate(write_weights):
                ...
                print(f"  Position {word_idx} ({word}): Write to slot {slot} (confidence: {confidence:.2f})")
        ...
        if read_weights:
            ...
            for i, r in enumerate(read_weights):
                ...
                print(f"  Position {read_pos} ({word}): Read from slot {slot} (confidence: {confidence:.2f})")
