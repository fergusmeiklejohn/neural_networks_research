...
from compositional_operators import (
    CompositionalParser, ParseNode, OperatorType
)
        ...
        while i < len(node.tokens):
            if node.tokens[i] == self.do_token and i + 1 < len(node.tokens):
                ...
                if var_name and var_name in bindings:
                    ...
                    output = mx.where(
                        mx.arange(self.model.num_actions) == action_idx,
                        1000.0,
                        output
                    )
                    ...
                    segment = (node.start_pos + i, min(node.start_pos + i + 2, node.end_pos))
    ...
    def _execute_parallel(self, node: ParseNode, command_ids: mx.array,
        ...
        if len(node.children) == 2:
            ...
            if left_child.is_leaf() and self.do_token in left_child.tokens:
                left_outputs = self._execute_leaf(left_child, command_ids, bindings, stage)
...
def test_fixed_executor():
    ...
    test_cases = [
        ("X means jump Y means walk do X and Y", {'X': 'JUMP', 'Y': 'WALK'}, ['JUMP', 'WALK']),
        ("X means jump Y means walk do X then do Y", {'X': 'JUMP', 'Y': 'WALK'}, ['JUMP', 'WALK']),
        ("X means jump while true do X", {'X': 'JUMP'}, ['JUMP', 'JUMP', 'JUMP']),
        ("X means walk Y means jump do X and Y then do X", {'X': 'WALK', 'Y': 'JUMP'}, ['WALK', 'JUMP', 'WALK']),
    ]
    ...
    for command, bindings, expected in test_cases:
        ...
        print(f"Parse tree: operator={parse_tree.operator}, is_leaf={parse_tree.is_leaf()}")
