...
class ImprovedCompositionalParser:
    ...
    def __init__(self, vocab: Dict[str, int]):
        ...
        # Remove any -1 values
        self.operator_tokens = {k: v for k, v in self.operator_tokens.items() if k != -1}
    ...
    def _find_matching_do(self, tokens: List[int], start: int, end: int) -> int:
        ...
        for i in range(start, end):
            if tokens[i] == self.do_token and depth == 0:
                return i
            elif tokens[i] in self.operator_tokens and self.operator_tokens[tokens[i]] == OperatorType.LOOP:
                depth += 1
    ...
    def _parse_expression(self, tokens: List[int], start: int, end: int) -> ParseNode:
        ...
        if start < end and tokens[start] in self.operator_tokens:
            if self.operator_tokens[tokens[start]] == OperatorType.LOOP:
                ...
                if do_pos != -1:
                    node = ParseNode(operator=OperatorType.LOOP, start_pos=start, end_pos=end)
                    # Condition is between 'while' and 'do'
                    node.children.append(self._parse_expression(tokens, start + 1, do_pos))
                    # Body is after 'do'
                    node.children.append(self._parse_expression(tokens, do_pos + 1, end))
                    return node
        ...
        for i in range(end - 1, start - 1, -1):
            ...
            if tokens[i] == self.do_token:
                paren_depth += 1
            elif i > start and tokens[i-1] in self.operator_tokens and \
                 self.operator_tokens[tokens[i-1]] == OperatorType.LOOP:
                paren_depth -= 1
...
class ImprovedCompositionalExecutor:
    ...
    def execute(self, parse_tree: ParseNode, command_ids: mx.array, 
                bindings: Dict, stage: str = "full") -> List[mx.array]:
    ...
    def _execute_leaf(self, node: ParseNode, command_ids: mx.array,
                      bindings: Dict, stage: str) -> List[mx.array]:
    ...
    def _execute_sequence(self, node: ParseNode, command_ids: mx.array,
                          bindings: Dict, stage: str) -> List[mx.array]:
    ...
    def _execute_parallel(self, node: ParseNode, command_ids: mx.array,
                          bindings: Dict, stage: str) -> List[mx.array]:
    ...
    def _execute_loop(self, node: ParseNode, command_ids: mx.array,
                      bindings: Dict, stage: str) -> List[mx.array]:
    ...
    def _execute_choice(self, node: ParseNode, command_ids: mx.array,
                        bindings: Dict, stage: str) -> List[mx.array]:
...
def test_improved_parser():
    ...
    for command in test_cases:
        ...
        print(f"Tree operator: {tree.operator}, is_leaf: {tree.is_leaf()}, children: {len(tree.children)}")
        print_tree(tree, tokens, vocab=VOCAB)
        print()


def print_tree(node: ParseNode, tokens: List[int], indent: int = 0, vocab: Dict[str, int] = None):
