        ...
        for _ in range(num_layers):
            self.layers.append(
                nn.TransformerEncoderLayer(
                    dims=embed_dim, num_heads=num_heads, mlp_dims=embed_dim * 4
                )
            )
...
class FeedforwardBaseline(nn.Module):
    ...
    def __init__(
        self, vocab_size: int, context_window: int = 10, hidden_dim: int = 256
    ):
        super().__init__()
...
class RuleBasedBaseline:
    ...
    def predict(self, input_ids: mx.array) -> mx.array:
        ...
        for b in range(batch_size):
            # Convert to tokens
            tokens = [
                self.inv_vocab.get(int(input_ids[b, i]), "<UNK>")
                for i in range(seq_len)
            ]
...
def test_baselines():
    ...
    vocab.update(
        {"X": 10, "Y": 11, "means": 12, "do": 13, "jump": 14, "walk": 15, "twice": 16}
    )

    rule_based = RuleBasedBaseline(vocab)
    # Create test pattern: "X means jump do X twice"
    test_input = mx.array(
        [[10, 12, 14, 13, 10, 16] + [0] * 14]
    )  # X means jump do X twice
    output = rule_based.predict(test_input)
    print(f"Output shape: {output.shape}")
    print(
        f"Predictions at key positions: {output[0, 4:7]}"
    )  # Should predict jump at positions 4 and 5
