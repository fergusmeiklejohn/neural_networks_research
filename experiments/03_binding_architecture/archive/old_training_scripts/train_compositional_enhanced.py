...
class EnhancedCompositionalModel(IntegratedBindingModel):
    ...
    def separate_bindings_and_execution(
        self, command_ids: mx.array
    ) -> Tuple[List[Tuple[int, int]], mx.array]:
    ...
    def __call__(self, inputs: Dict[str, mx.array], stage: str = "full") -> mx.array:
        ...
        binding_segments, exec_tokens = self.separate_bindings_and_execution(
            command_ids
        )
        ...
        for start, end in binding_segments:
            ...
            _ = self.process_segment_versioned(
                command_ids, segment, bindings, stage="binding"
            )
...
def generate_enhanced_compositional_data(
    num_samples: int = 100,
) -> List[Dict[str, Any]]:
...
def train_enhanced_compositional():
    ...
    model = EnhancedCompositionalModel(
        vocab_size=len(VOCAB), num_actions=len(ACTIONS), embed_dim=128, num_slots=4
    )
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            if i % 100 == 0:
                print(
                    f"Epoch {epoch+1}/{num_epochs}, Batch {i}/{len(train_data)}, "
        ...
        if epoch_acc > best_accuracy:
            ...
            save_path = os.path.join(
                get_output_path(), "compositional_enhanced_best.pkl"
            )
...
def test_enhanced_model(model: EnhancedCompositionalModel):
    ...
    for command, expected in test_cases:
        ...
        binding_segs, exec_tokens = model.separate_bindings_and_execution(
            mx.array([tokens])
        )
