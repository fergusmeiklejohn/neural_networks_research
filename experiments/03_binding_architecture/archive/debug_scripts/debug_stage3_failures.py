...
from train_binding_curriculum import ACTIONS, VOCAB, generate_stage3_data
...
def analyze_stage3_failures():
    ...
    print(
        "   - When multiple variables are bound, later bindings may overwrite earlier ones"
    )
    ...
    print(
        "   - Combining temporal patterns with multiple variables exceeds model capacity"
    )
    print("   - Need better compositional mechanisms")
    print("\n4. **Training curriculum imbalance**")
    print(
        "   - Jump from Stage 2 (simple retrieval) to Stage 3 (full complexity) too large"
    )
