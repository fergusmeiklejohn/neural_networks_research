...
def test_persistence_methods():
    ...
    for name, param in model.parameters().items():
        print(
            f"  {name}: type={type(param)}, shape={param.shape if hasattr(param, 'shape') else 'N/A'}"
        )
        if isinstance(param, dict):
            for sub_name, sub_param in param.items():
                print(
                    f"    {sub_name}: type={type(sub_param)}, shape={sub_param.shape if hasattr(sub_param, 'shape') else 'N/A'}"
                )
