...
class ImprovedMemoryBindingModel(MemoryBasedBindingModel):
    ...
    def detect_binding_pattern(
        self, tokens: mx.array, hidden_states: mx.array, position: int
    ) -> Tuple[bool, int, int]:
...
def evaluate_model(
    model: ImprovedMemoryBindingModel, dataset: List[Dict], verbose: bool = False
) -> Dict[str, float]:
    ...
    for sample in dataset:
        ...
        if verbose and not correct:
            ...
            print(
                f"  Predicted: {[ACTIONS[i] for i in pred_actions] if pred_actions else []}"
            )
