...
class ImprovedMemoryNetwork(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_actions: int, embed_dim: int = 128,
                 hidden_dim: int = 256, num_vars: int = 4):
        super().__init__()
        ...
        # Vocabulary mappings
        self.vocab = {
            'do': 1, 'means': 2, 'is': 3, 'and': 4, 'or': 5,
            'then': 6, 'twice': 7, 'thrice': 8,
            'X': 10, 'Y': 11, 'Z': 12, 'W': 13
        }
        ...
        self.memory_keys = mx.array([[1, 0, 0, 0],  # X
                                     [0, 1, 0, 0],  # Y
                                     [0, 0, 1, 0],  # Z
                                     [0, 0, 0, 1]]) # W
    ...
    def parse_execution_plan(self, tokens: mx.array) -> Tuple[Dict[int, int], ExecutionNode]:
    ...
    def _parse_execution(self, tokens: List[int], bindings: Dict[int, int]) -> ExecutionNode:
    ...
    def execute_plan(self, plan: ExecutionNode, bindings: Dict[int, int]) -> List[mx.array]:
        ...
        for i in range(len(token_list)):
            ...
            if token in bindings and i + 2 < len(token_list):
                ...
                if next_token in [self.vocab['means'], self.vocab['is']]:
                    ...
                    self.memory_values = mx.array(self.memory_values)  # Ensure it's mutable
...
def test_improved_memory():
    ...
    from progressive_complexity_dataset import VOCAB, ACTIONS
