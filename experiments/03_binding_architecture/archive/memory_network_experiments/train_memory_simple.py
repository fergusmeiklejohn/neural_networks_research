...
def train_single_sample(model, optimizer, sample: Dict) -> float:
    ...
    def loss_fn(model):
        ...
        if predictions.shape[0] < n_expected:
            ...
            padding = mx.zeros(
                (n_expected - predictions.shape[0], predictions.shape[1])
            )
...
def evaluate_simple(model, dataset: List[Dict]) -> Dict[str, float]:
    ...
    results = {"overall": correct / total if total > 0 else 0}
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"  Level 1: {results.get('level_1', 0):.2%}, Level 2: {results.get('level_2', 0):.2%}"
        )
