...
def evaluate(model, dataset: List[Dict], verbose: bool = False) -> Dict[str, float]:
        ...
        if predictions.shape[0] > 0:
            ...
            if pred_indices == expected:
                ...
                if len(pred_indices) == len(expected):
                    ...
                    correct_actions = sum(p == e for p, e in zip(pred_indices, expected))
                    if correct_actions > 0:
                        partial_correct_by_level[level] += correct_actions / len(expected)
    ...
    for level in range(1, 5):
        if total_by_level[level] > 0:
            results[f'level_{level}_exact'] = correct_by_level[level] / total_by_level[level]
            results[f'level_{level}_partial'] = partial_correct_by_level[level] / total_by_level[level]
...
def analyze_memory_usage(model, sample: Dict):
    ...
    for i, var in enumerate(['X', 'Y', 'Z', 'W']):
        ...
        if var_token in bindings:
            ...
            print(f"  {var} (slot {slot}): mean={mx.mean(value).item():.4f}, std={mx.std(value).item():.4f}")
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f"  Level 1: {results.get('level_1_exact', 0):.1%} exact, {results.get('level_1_partial', 0):.1%} partial")
            print(f"  Level 2: {results.get('level_2_exact', 0):.1%} exact, {results.get('level_2_partial', 0):.1%} partial")
