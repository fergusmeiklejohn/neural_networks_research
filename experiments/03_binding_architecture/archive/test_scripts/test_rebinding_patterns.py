...
from train_sequential_action_positions import SequentialModelWithActionTracking, VOCAB, ACTIONS
...
class RebindingTestSuite:
    ...
    def extract_predictions(self, model, command: str, model_type: str = "versioned") -> List[str]:
    ...
    def evaluate_model(self, model, model_name: str, model_type: str = "versioned") -> Dict:
        ...
        # Overall summary
        results['overall_accuracy'] = results['correct_tests'] / results['total_tests'] if results['total_tests'] > 0 else 0
        
        print("\n" + "="*60)
        print(f"OVERALL ACCURACY: {results['overall_accuracy']:.1%} ({results['correct_tests']}/{results['total_tests']})")
        print("="*60)
    ...
    def compare_models(self):
        ...
        print("\n\nCOMPARISON SUMMARY")
        print("="*80)
        print(f"{'Category':<25} {'Versioned':<15} {'Standard':<15} {'Improvement':<15}")
        print("-"*70)
        ...
        print(f"{'OVERALL':<25} {v_overall:>14.1%} {s_overall:>14.1%} {overall_improvement:>+14.1%}")
        print("="*80)
...
def test_memory_versions():
    ...
    # First binding
    memory_state = memory.bind_versioned(var_embed, value1, binding_scores, 0, memory_state)
    print(f"After first binding: {len(memory_state)} slots, slot 0 has {len(memory_state[0]['versions'])} versions")
    
    # Second binding (rebind)
    memory_state = memory.bind_versioned(var_embed, value2, binding_scores, 10, memory_state)
    print(f"After rebinding: {len(memory_state)} slots, slot 0 has {len(memory_state[0]['versions'])} versions")
    
    # Third binding
    memory_state = memory.bind_versioned(var_embed, value3, binding_scores, 20, memory_state)
    print(f"After third binding: {len(memory_state)} slots, slot 0 has {len(memory_state[0]['versions'])} versions")
