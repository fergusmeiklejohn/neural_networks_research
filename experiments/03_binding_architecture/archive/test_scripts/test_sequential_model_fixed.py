...
def test_sequential_patterns():
    ...
    model_files = [
        ...
        if f.startswith("sequential_fixed_") and f.endswith(".npz")
    ]
    ...
    test_cases = [
        ...
        (
            "X means run do X then Y means jump do Y then Z means walk do Z",
            ["RUN", "JUMP", "WALK"],
        ),
        # Temporal patterns
        ...
        (
            "X means jump do X then Y means walk do Y twice then Z means turn do Z",
            ["JUMP", "WALK", "WALK", "TURN"],
        ),
    ]
