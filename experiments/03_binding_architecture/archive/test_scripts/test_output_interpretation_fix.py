...
def demonstrate_output_interpretation_fix():
    ...
    complex_command = "X means jump do X thrice then Y means walk do Y twice then Z means turn do Z"
    ...
    # Test with new model
    complex_ids = mx.array([[VOCAB.get(token, VOCAB['<PAD>']) for token in complex_command.split()]])
    ...
    if 'action_logits' in complex_outputs:
        ...
        print(f"- Shape matches expected: {complex_outputs['action_logits'].shape[1] == len(complex_expected)}")
