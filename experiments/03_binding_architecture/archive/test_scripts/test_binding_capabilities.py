...
class BindingTestSuite:
    ...
    def analyze_failure_modes(self, model):
        ...
        for test in failure_tests:
            # Encode and run
            command_ids = [
                self.generator.word_to_id.get(w, 0) for w in test["commands"]
            ]
...
def main():
    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            print(
                "No trained model found. Train a model first with train_binding_mlx_proper.py"
            )
            return
