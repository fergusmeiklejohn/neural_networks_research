...
import numpy as np
from train_sequential_planning import ACTIONS, VOCAB, SequentialDynamicMemoryModel
...
def train_simple_model():
    ...
    for epoch in range(20):
        ...
        for cmd, expected in zip(commands, expected_actions):
            ...
            act_ids = mx.array(
                [[ACTIONS.get(act, ACTIONS["<PAD>"]) for act in expected]]
            )
    ...
    for pattern, expected in test_patterns:
        ...
        if "action_logits" in outputs:
            ...
            for j in range(min(pred_np.shape[1], len(expected))):
                pred_id = (
                    pred_np[0, j].item()
                    if hasattr(pred_np[0, j], "item")
                    else int(pred_np[0, j])
                )
