...
class SimpleVariableBindingModel(nn.Module):
    ...
    def __init__(self, vocab_size=10, embed_dim=32, num_slots=3):
        ...
        self.bind_net = nn.Sequential(
            nn.Linear(embed_dim, 64), nn.ReLU(), nn.Linear(64, num_slots)
        )

        # Action decoder
        self.decoder = nn.Sequential(
            nn.Linear(embed_dim, 32), nn.ReLU(), nn.Linear(32, 4)  # 4 actions
        )
    ...
    def __call__(self, sentence):
        ...
        slot_contents_exp = slot_contents[
            :, None, :, :
        ]  # (batch, 1, num_slots, embed_dim)
        slot_contents_exp = mx.broadcast_to(
            slot_contents_exp, (batch_size, seq_len, self.num_slots, self.embed_dim)
        )

        binding_probs_exp = binding_probs[
            :, :, :, None
        ]  # (batch, seq_len, num_slots, 1)
        retrieved = mx.sum(
            slot_contents_exp * binding_probs_exp, axis=2
...
def train_simple_binding():
        ...
        for _ in range(20):  # 20 batches per epoch
            ...
            def loss_fn(model):
                ...
                kl_loss = mx.mean(
                    mx.sum(
                        first_binding
                        * mx.log(first_binding / (last_binding + 1e-8) + 1e-8),
                        axis=1,
                    )
                )
    ...
    for sentence, description, expected in test_cases:
        ...
        print(
            f"  Consistent: {'✓' if binding_pattern[0] == binding_pattern[-1] else '✗'}"
        )
        print()
