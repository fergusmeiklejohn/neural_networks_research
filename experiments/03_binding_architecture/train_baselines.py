...
from baseline_models import create_baseline_model, RuleBasedBaseline
...
def main():
    ...
    # Create vocabulary
    vocab = {
        '<PAD>': 0, '<UNK>': 1, 'X': 2, 'Y': 3, 'Z': 4, 'W': 5,
        'means': 6, 'do': 7, 'twice': 8, 'thrice': 9, 'then': 10,
        'jump': 11, 'walk': 12, 'turn': 13, 'look': 14, 'run': 15, 'stop': 16,
        'JUMP': 17, 'WALK': 18, 'TURN': 19, 'LOOK': 20, 'RUN': 21, 'STOP': 22
    }
    ...
    def dict_to_tuples(data_dict):
        ...
        for i in range(inputs.shape[0]):
            tuples.append((
                ...
                labels[i:i+1] if len(labels.shape) > 1 else mx.array([[labels[i]]]),
                mask[i:i+1] if len(mask.shape) > 1 else mx.ones((1, 1))
            ))
        return tuples
    
    # Combine all stages
    train_data = (dict_to_tuples(stage1_train) + 
                  dict_to_tuples(stage2_train) + 
                  dict_to_tuples(stage3_train))
    val_data = (dict_to_tuples(stage1_val) + 
    ...
    lstm_history = train_baseline(
        lstm_model, train_data, val_data,
        num_epochs=5, model_name="LSTM"  # Quick test with 5 epochs
    )
    results['lstm'] = {
    ...
    transformer_history = train_baseline(
        transformer_model, train_data, val_data,
        num_epochs=5, model_name="Transformer"  # Quick test
    )
    results['transformer'] = {
    ...
    ff_history = train_baseline(
        ff_model, train_data, val_data,
        num_epochs=5, model_name="Feedforward"  # Quick test
    )
    results['feedforward'] = {
