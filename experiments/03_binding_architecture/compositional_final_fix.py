...
class FinalCompositionalParser:
    ...
    def _extract_all_bindings(
        self, token_list: List[int]
    ) -> Tuple[Dict[int, str], List[Tuple[int, int, Optional[str]]]]:
        ...
        while i < len(token_list):
            if i + 2 < len(token_list) and token_list[i + 1] == self.means_token:
                ...
                if i < len(token_list) and token_list[i] == self.do_token:
                    ...
                    while end < len(token_list):
                        if (
                            end + 1 < len(token_list)
                            and token_list[end + 1] == self.means_token
                        ):
                            break
            ...
            elif token_list[i] == self.do_token and not any(
                start <= i < end for start, end, _ in execution_ranges
            ):
                # Standalone 'do'
                start = i + 1
                end = start
    ...
    def _parse_execution_ranges(
        self, token_list: List[int], ranges: List[Tuple[int, int, Optional[str]]]
    ) -> ParseNode:
        ...
        if len(ranges) == 1:
            ...
            for start, end, modifier in ranges:
                if start > 0 and token_list[start - 3] == self.while_token:
                    nodes.append(
                        self._parse_while_expression(token_list, start - 3, end)
                    )
                else:
                    node = self._parse_expression(token_list, start, end)
                    if modifier and node.is_leaf():
...
class FinalExecutor:
    ...
    def _execute_node(
        self, node: ParseNode, bindings: Dict[int, str], outputs: List[mx.array]
    ):
