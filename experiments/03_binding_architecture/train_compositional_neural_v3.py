...
from compositional_final_fix import FinalCompositionalParser, ParseNode, FinalExecutor
...
VOCAB = {
    '<PAD>': 0, '<SOS>': 1, '<EOS>': 2,
    'jump': 3, 'walk': 4, 'turn': 5, 'run': 6,
    'means': 7, 'do': 8, 'twice': 9, 'thrice': 10,
    'X': 11, 'Y': 12, 'Z': 13, 'W': 14,
    'and': 15, 'then': 16, 'while': 17, 'or': 18, 'true': 19
}

ACTIONS = {
    '<PAD>': 0, 'JUMP': 1, 'WALK': 2, 'TURN': 3, 'RUN': 4
}
...
class BindingAwareModel(nn.Module):
    ...
    def encode_bindings(self, command_ids: mx.array, bindings: Dict[int, str]) -> Dict[str, mx.array]:
        ...
        for pos, action_name in bindings.items():
            ...
            if pos < len(command_ids):
                ...
                if var_name in ['X', 'Y', 'Z', 'W']:
                    ...
                    binding_input = mx.concatenate([var_emb[0], means_emb[0], action_emb[0]])
    ...
    def __call__(self, inputs: Dict[str, mx.array]) -> mx.array:
        ...
        outputs = self.execute_neural(parse_tree, command_ids, binding_vectors, context[0])
    ...
    def execute_neural(self, node: ParseNode, command_ids: mx.array, 
                       binding_vectors: Dict[str, mx.array], context: mx.array) -> List[mx.array]:
        ...
        if node.is_leaf():
            ...
            if node.operator.value == 'AND':
                ...
                for child in node.children:
                    child_outputs = self.execute_neural(child, command_ids, binding_vectors, context)
                    all_outputs.extend(child_outputs)
            
            elif node.operator.value == 'OR':
                # Execute first child (simplified)
                if node.children:
                    child_outputs = self.execute_neural(node.children[0], command_ids, binding_vectors, context)
                    all_outputs.extend(child_outputs)
...
def generate_training_data(num_samples: int = 1000) -> List[Tuple[str, List[str]]]:
    ...
    patterns = [
        # Basic patterns
        lambda v1, a1, v2, a2: (f"{v1} means {a1} {v2} means {a2} do {v1} and {v2}", 
                                 [a1.upper(), a2.upper()]),
        lambda v1, a1, v2, a2: (f"{v1} means {a1} {v2} means {a2} do {v2} and {v1}", 
                                 [a2.upper(), a1.upper()]),
        lambda v1, a1, v2, a2: (f"{v1} means {a1} {v2} means {a2} do {v1} then {v2}", 
                                 [a1.upper(), a2.upper()]),
        
        # With modifiers
        lambda v1, a1, v2, a2: (f"{v1} means {a1} do {v1} twice and {v2} means {a2} do {v2}", 
                                 [a1.upper(), a1.upper(), a2.upper()]),
        
        # Complex patterns
        lambda v1, a1, v2, a2: (f"{v1} means {a1} {v2} means {a2} do {v1} and {v2} then {v1}", 
                                 [a1.upper(), a2.upper(), a1.upper()]),
        lambda v1, a1, v2, a2: (f"{v1} means {a1} do {v1} then {v2} means {a2} do {v2} and {v1}", 
                                 [a1.upper(), a2.upper(), a1.upper()]),
    ]
...
def train_model(model: BindingAwareModel, train_data: List[Tuple[str, List[str]]],
    ...
    def loss_fn(model, command_tokens, expected_actions):
        ...
        for i, expected in enumerate(expected_actions):
            if i < outputs.shape[0]:
                ...
                total_loss += mx.mean(nn.losses.cross_entropy(outputs[i:i+1], target))
                count += 1
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0:
            logger.info(f"Epoch {epoch}/{num_epochs} - Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2%}")
...
def main():
    ...
    if accuracy > 0.8:
        ...
        params_dict = {name: param for name, param in trained_model.parameters().items() 
