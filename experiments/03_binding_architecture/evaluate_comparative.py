...
def evaluate_on_patterns(model, test_patterns: List[Dict], model_type: str = "baseline"):
...
def evaluate_all_models():
    ...
    transformer_results = evaluate_on_patterns(transformer, test_patterns, model_type="baseline")
    transformer_accuracy = sum(r['correct'] for r in transformer_results) / len(transformer_results)
    ...
    vocab = {'X': 2, 'Y': 3, 'means': 6, 'do': 7, 'jump': 11, 'walk': 12, 
             'turn': 13, 'twice': 8, 'thrice': 9, 'then': 10}
