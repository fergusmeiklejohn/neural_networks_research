    ...
    for i in range(min(num_examples, len(base_examples))):
        ...
        if i < len(modifications):
            ...
            if orig_word in base_cmd:
                ...
                mod_act = base_act.replace(
                    f"I_{orig_word.upper()}", f"I_{new_word.upper()}"
                )
            else:
...
def generate_test_cases(data_gen, num_examples=10):
    ...
    cmd_train, act_train = (
        data_gen.cmd_train[:num_examples],
        data_gen.act_train[:num_examples],
    )
...
def predict_with_model(model, command, modification=None):
    ...
    # Get prediction
    inputs = {"command": cmd_input, "target": dummy_target, "modification": mod_input}
...
def main():
    ...
    if not vocab_path.exists():
        vocab_path = Path(
            "compositional_language_complete_20250722_185804/outputs/safeguarded_training/vocabulary.json"
        )
    ...
    local_results_dir = Path(
        "compositional_language_complete_20250722_185804/outputs/safeguarded_training"
    )
