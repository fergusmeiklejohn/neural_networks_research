                    ...
                    if not module_path.exists():
                        ...
                        if len(parts) > 1:
                            ...
                            with open(module_path, "r") as f:
                                ...
                                item_name = parts[-1]
                                if (
                                    f"def {item_name}" not in content
                                    and f"class {item_name}" not in content
                                ):
                                    failed_imports.append(
                                        f"{imp} ('{item_name}' not found in {module_path})"
                                    )
                else:
...
"""
            ...
            if "Dry run completed successfully" in result.stdout:
                ...
                if "AttributeError" in result.stderr:
                    ...
                    self.warnings.append(
                        "Dry run did not complete (may need actual data)"
                    )
    ...
    def _run_linting(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(issues) > 5:
                    self.warnings.append(
                        f"Linting found {len(issues)} issues (showing first 5)"
                    )
    ...
    def _check_antipatterns(self):
        ...
        for i, line in enumerate(lines):
            if any(
                path in line for path in ["/home/", "/Users/", "C:\\", "/notebooks/"]
            ):
        ...
        if self.errors:
            ...
            for error in self.errors:
                if "save" in error and "AttributeError" in error:
                    print(
                        "  • Replace 'tokenizer.save()' with 'tokenizer.save_vocabulary()'"
                    )
                elif "create_model" in error and "ImportError" in error:
                    print(
                        "  • Import create_model from models.py, not train_progressive_minimal.py"
                    )
