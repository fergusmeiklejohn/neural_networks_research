...
def analyze_catastrophic_interference(history):
    ...
    for i, stage in enumerate(stages_data):
        ...
        print(
            f"  Final Accuracy: {stage['final_accuracy']:.4f} ({stage['final_accuracy']*100:.1f}%)"
        )
        if stage["val_loss"]:
            print(f"  Validation Loss: {stage['val_loss']:.4f}")
            print(
                f"  Validation Accuracy: {stage['val_accuracy']:.4f} ({stage['val_accuracy']*100:.1f}%)"
            )
    ...
    if len(stages_data) > 1:
        ...
        if stage2_final_loss > stage1_final_loss * 2:
            ...
            print(
                f"  Loss remains {stage2_final_loss/stage1_final_loss:.1f}x higher than baseline"
            )
...
def main():
    ...
    analyze_catastrophic_interference(history)
