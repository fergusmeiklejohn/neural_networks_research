...
def demonstrate_evaluation_illusion():
    ...
    train_data = task.generate_examples(
        1000, include_modifications=True, exclude_modifier="twice"
    )
    ...
    val_data = task.generate_examples(
        200, include_modifications=True, exclude_modifier="twice"
    )
    ...
    print(
        f"But it completely fails on the modification 'twice': {results['mod_twice']:.1f}%"
    )
