...
def modify_script_for_local_testing(script_path: str) -> str:
    ...
    # 2. Reduce batch size
    modifications.append(
        ("'batch_size': 32", "'batch_size': int(os.environ.get('BATCH_SIZE', '2'))")
    )
        ...
        if result.returncode == 0:
            ...
            if "ValueError: Dimensions must be equal" in error_output:
                ...
                if shapes:
                    ...
                    print(
                        f"  Got shape: {shapes[0].split(',')[1] if ',' in shapes[0] else 'unknown'}"
                    )
                print(
...
def check_common_issues():
    ...
    if not Path("data/processed/modification_pairs.pkl").exists():
        issues.append(
            "Modification data not found - run: python modification_generator.py"
        )
...
def main():
    ...
    if success:
        ...
        print("\nNext steps:")
        print(
            "1. Push to production: git push origin branch && gh pr create && gh pr merge"
        )
