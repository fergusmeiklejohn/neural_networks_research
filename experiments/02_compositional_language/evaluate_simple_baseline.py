...
def create_generate_function(model, tokenizer, max_length=50):
    ...
    def generate_action(command_ids, modification=None, start_token=None, end_token=None, **kwargs):
        ...
        current_token = np.full((batch_size, 1), tokenizer.action_to_id['<START>'], dtype=np.int32)
        generated_tokens = []
        
        for _ in range(max_length):
            # Predict next token
            predictions = model.predict([command_ids, current_token, modification_input], verbose=0)
...
def main():
    ...
    results = evaluator.evaluate_all_sets(model, validation_dir, batch_size=args.batch_size)
    ...
    mod_accs = [results.get(k, {}).get('accuracy', 0) 
                for k in results if k.startswith('val_mod_') and isinstance(results[k], dict)]
    ...
    if mod_accs:
        ...
        if avg_mod_acc < 0.1 and base_acc > 0.1:
            ...
            print("The model's modification performance is less than half of base performance.")
        else:
