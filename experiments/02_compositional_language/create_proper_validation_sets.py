...
from modification_generator import ModificationGenerator, ModificationPair


def create_comprehensive_validation_sets(
    base_samples: List[SCANSample],
    output_dir: Path,
    seed: int = 42
) -> Dict[str, List[Dict]]:
    """
    ...
    for val_key, swap_rules, description in modification_rules:
        ...
        relevant_samples = [s for s in samples if any(word in s.command.split() for word in swap_rules.keys())]
    ...
    all_mods = []
    for key in ['val_mod_walk_skip', 'val_mod_jump_hop', 'val_mod_look_scan', 'val_mod_left_right']:
    ...
    for swap_rules, description in composed_rules:
        relevant_samples = [s for s in samples if 
                          any(word in s.command.split() for word in swap_rules.keys())]
        ...
        for sample in selected:
            ...
            for orig, repl in swap_rules.items():
                ...
                if orig in mod_gen.ACTION_MAP and repl in ['skip', 'hop', 'scan']:
                    ...
                    orig_action = mod_gen.ACTION_MAP.get(orig, orig.upper())
                    mod_action = mod_action.replace(f'I_{orig.upper()}', action_map.get(repl, f'I_{repl.upper()}'))
...
def apply_unseen_modification(sample: SCANSample, swap_rules: Dict[str, str]) -> Tuple[str, str]:
    ...
    for orig, repl in swap_rules.items():
        ...
        if orig in ['turn', 'run', 'walk', 'jump', 'look']:
            ...
            if 'beside' == repl and 'I_TURN_LEFT I_TURN_LEFT I_TURN_LEFT I_TURN_LEFT' in mod_action:
...
def save_validation_sets(validation_sets: Dict[str, List[Dict]], output_dir: Path):
    ...
    metadata = {
        ...
        'validation_sets': {
            key: {
                'count': len(samples),
                'description': get_set_description(key)
            }
            for key, samples in validation_sets.items()
        }
    }
