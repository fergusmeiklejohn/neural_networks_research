...
class CompositionalRuleExtractor(keras.Model):
    ...
    def __init__(self,
        ...
        self.primitive_head = keras.layers.Dense(5, activation='sigmoid')  # 5 primitives
        self.direction_head = keras.layers.Dense(3, activation='sigmoid')  # left, right, around
        self.modifier_head = keras.layers.Dense(3, activation='sigmoid')   # twice, thrice, opposite
...
class RuleModificationComponent(keras.Model):
    ...
    def __init__(self,
        ...
        # Layer normalization
        self.layer_norms = [keras.layers.LayerNormalization() for _ in range(num_layers)]
    ...
    def call(self, rule_embeddings, modification_request, training=None):
        ...
        for i, (cross_attn, layer_norm) in enumerate(zip(self.cross_attention_layers, self.layer_norms)):
...
def create_model(command_vocab_size: int, 
                 action_vocab_size: int,
...
def test_models():
    ...
    # Test data
    command = tf.random.uniform((batch_size, seq_len), 0, command_vocab_size, dtype=tf.int32)
    modification = tf.random.uniform((batch_size, 5), 0, command_vocab_size, dtype=tf.int32)
