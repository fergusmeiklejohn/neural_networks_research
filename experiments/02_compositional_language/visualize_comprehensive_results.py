...
degradations = [
    results["comparisons"][exp]["degradation_percent"] for exp in experiments
]
...
for i, (s1, s4) in enumerate(zip(stage1_accs, final_accs)):
    ax1.text(
        i - width / 2, s1 + 0.01, f"{s1:.3f}", ha="center", va="bottom", fontsize=8
    )
    ax1.text(
        i + width / 2, s4 + 0.01, f"{s4:.3f}", ha="center", va="bottom", fontsize=8
    )
...
for exp_name, comp in results["comparisons"].items():
    print(
        f"{exp_name:15} - Stage 1: {comp['stage1_accuracy']:.3f}, Final: {comp['final_accuracy']:.3f}"
    )
...
for exp_name, exp_data in results["experiments"].items():
    ...
    print(
        f"  Acc:  {stage1_acc:.3f} → {final_acc:.3f} ({acc_decrease:+.1f}% degradation)"
    )
