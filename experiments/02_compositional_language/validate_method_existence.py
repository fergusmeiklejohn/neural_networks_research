...
def validate_script_methods(script_path: str):
    ...
    validation_map = {
        'tokenizer': {
            ...
            'methods_to_check': ['save', 'load', 'save_vocabulary', 'load_vocabulary', 'build_vocabulary']
        },
        'mod_generator': {
            'class': 'ModificationGenerator',
            'module': 'modification_generator',
            'methods_to_check': ['load_modifications', 'save_modifications', 'generate_all_modifications']
        },
        'model': {
            'class': 'create_model',
            'module': 'models',
            'methods_to_check': ['save_weights', 'load_weights', 'compile', 'fit', 'evaluate']
        },
        'loader': {
        ...
        try:
            ...
            if hasattr(module, config['class']):
                ...
                if obj_name in ['tokenizer', 'mod_generator', 'loader']:
                    ...
                    # Check each method
                    available_methods = [m for m in dir(instance) if not m.startswith('_')]
    ...
    for obj_name, method_name, line_no in method_calls:
        if obj_name in validation_map:
            ...
            if f"{obj_name}.{method_name}()" not in validated and \
               f"{obj_name}.{method_name}() - Method not found!" in errors:
                script_specific_errors.append(f"Line {line_no}: {obj_name}.{method_name}()")
    ...
    if errors:
        ...
        for error in errors:
            if "tokenizer.save()" in error:
                ...
                print("  • Ensure ModificationGenerator has load_modifications() method")
