...
class SCANDataLoader:
    ...
    def create_isolated_splits(
        ...
        for sample in self.samples:
            if (
                sample.length > max_train_length
                and len(splits["test_length_extrap"]) < 100
            ):
        ...
        for sample in self.samples:
            ...
            if (
                mod_combo not in train_modifier_combos
                and len(splits["test_modifier_extrap"]) < 100
            ):
        ...
        extrap_samples = (
            splits["test_primitive_extrap"][:20]
        ...
        for split_name, samples in splits.items():
            if samples:
                ...
                print(
                    f"  {split_name}: {len(samples)} samples, avg length: {avg_length:.1f}"
                )
        ...
        print(
            f"  Test extrapolation has new combinations: {len(splits['test_primitive_extrap']) > 0}"
        )
        ...
        for split_name, samples in splits.items():
            ...
            metadata = {
                ...
                "avg_command_length": np.mean(
                    [len(s.command.split()) for s in samples]
                ),
                "avg_action_length": np.mean([s.length for s in samples]),
                "unique_primitives": list(
                    set(p for s in samples for p in s.primitives)
                ),
