...
class SCANTokenizer:
    ...
    def encode_command(self, command: str, max_length: int = 50) -> np.ndarray:
        ...
        ids = [
            self.command_to_id.get(token, self.command_to_id["<UNK>"])
            for token in tokens
        ]
    ...
    def encode_action(self, action: str, max_length: int = 100) -> np.ndarray:
        ...
        ids = [
            self.action_to_id.get(token, self.action_to_id["<UNK>"]) for token in tokens
        ]
...
def create_dataset(
    ...
    if modifications and modification_ratio > 0:
        ...
        selected_mods = np.random.choice(
            modifications,
            size=min(num_modifications, len(modifications)),
            replace=False,
        )
        ...
        for mod in selected_mods:
            ...
            tokenizer.encode_action(mod["original"]["action"])

            # Modified sample
            tokenizer.encode_command(mod["modified"]["command"])
            mod_action_enc = tokenizer.encode_action(mod["modified"]["action"])

            # Modification description (simplified for now)
            mod_desc = tokenizer.encode_command(
                mod["modification_description"], max_length=20
            )
        ...
        for epoch in range(stage_config["epochs"]):
            ...
            for batch_inputs, batch_targets in tqdm(dataset, desc="Training"):
                with tf.GradientTape() as tape:
                    ...
                    # Mask padding
                    mask = tf.cast(
                        batch_targets != tokenizer.action_to_id["<PAD>"], tf.float32
                    )
