...
class ModificationGenerator:
    ...
    def apply_word_swap(
        self, command: str, action: str, swap_rules: Dict[str, str]
    ) -> Tuple[str, str]:
        ...
        for original, replacement in swap_rules.items():
            if original in self.ACTION_MAP and replacement in self.ACTION_MAP:
                ...
                modified_action = modified_action.replace(
                    original_action, replacement_action
                )

        return modified_command, modified_action

    def generate_simple_swaps(
        self, samples: List[SCANSample]
    ) -> List[ModificationPair]:
        ...
        swap_configurations = [
            ...
            # Modifier swaps
            {
                "twice": "thrice",
                "thrice": "twice",
                "description": "twice and thrice are swapped",
            },
        ]
    ...
    def generate_action_modifications(
        self, samples: List[SCANSample]
    ) -> List[ModificationPair]:
    ...
    def generate_structural_modifications(
        self, samples: List[SCANSample]
    ) -> List[ModificationPair]:
        ...
        direction_samples = [
            s for s in samples if any(d in s.command for d in ["left", "right"])
        ]
        ...
        modifier_samples = [
            s for s in samples if any(m in s.command for m in ["twice", "thrice"])
        ]
    ...
    def generate_all_modifications(
        self, train_samples: List[SCANSample]
    ) -> Dict[str, List[ModificationPair]]:
        ...
        modifications = {
            ...
            "structural_modifications": self.generate_structural_modifications(
                train_samples
            ),
    ...
    def save_modifications(self, modifications: Dict[str, List[ModificationPair]]):
        ...
        metadata = {
            ...
            "modification_types": {
                mod_type: len(pairs) for mod_type, pairs in modifications.items()
            },
        }
