...
class ExperimentValidator:
    ...
    def log_test(self, name, success, details=""):
        """Log test result."""
        self.results["tests"][name] = {"success": success, "details": details}
    ...
    def test_data_loading(self):
        ...
        try:
            ...
            self.log_test(
                "Data loading", True, f"Loaded {len(splits['train'])} train samples"
            )
    ...
    def test_modifications(self, train_samples):
        ...
        try:
            ...
            sample_mod = (
                mod_dict["simple_swaps"][0] if mod_dict["simple_swaps"] else None
            )
            if sample_mod:
                required_attrs = [
                    "original_sample",
    ...
    def test_models(self, tokenizer):
        ...
        try:
            ...
            self.log_test("Model v1", True, f"Parameters: {model_v1.count_params():,}")
        ...
        try:
            ...
            self.log_test("Model v2", True, f"Parameters: {model_v2.count_params():,}")
        ...
        try:
            ...
            for batch in base_dataset.take(1):
                ...
                if inputs["command"].shape[0] != 8:
                    raise ValueError(
                        f"Incorrect batch size: {inputs['command'].shape[0]}"
                    )
    ...
    def test_training_loop(self, model, dataset):
        ...
        try:
            ...
            self.log_test("Training loop", True, f"Loss: {loss:.4f}, Acc: {acc:.4f}")
    ...
    def run_all_tests(self):
        ...
        train_data = [
            {"command": s.command, "action": s.action} for s in train_samples[:200]
        ]
        ...
        try:
            model = create_model_v2(
                len(tokenizer.command_to_id), len(tokenizer.action_to_id), d_model=64
            )
