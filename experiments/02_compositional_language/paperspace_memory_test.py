...
for config in configs_to_test:
    ...
    try:
        ...
        attn_reduced = tf.tile(attn_reduced, [1, 1, d_model])  # (batch_size, seq_len, d_model)
        
        # FFN with 4x expansion
        ffn = tf.random.normal((batch_size, seq_len, d_model * 4))
        ffn_reduced = tf.reduce_mean(ffn, axis=-1, keepdims=True)  # (batch_size, seq_len, 1)
        ffn_reduced = tf.tile(ffn_reduced, [1, 1, d_model])  # (batch_size, seq_len, d_model)
