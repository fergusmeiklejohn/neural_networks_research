...
class ModificationAwareEvaluator:
            ...
            try:
                ...
                if hasattr(model, "generate_action"):
                    predictions = model.generate_action(
                        ...
                        modification=tf.constant(mod_tensor)
                    ...
                    predictions = self._generate_v2(
                        model, cmd_tensor, mod_tensor, max_act_len
                    )
    ...
    def evaluate_all_sets(
        self, model: keras.Model, validation_dir: Path, batch_size: int = 32
        ...
        for set_name in val_sets:
            ...
            try:
                ...
                # Print results
                print(
                    f"  Accuracy: {results['accuracy']:.2%} ({results['correct']}/{results['total']})"
                )
...
def load_model_with_config(
    model_path: Path, vocab_path: Path
    ...
    print(
        f"Vocabulary sizes - Command: {command_vocab_size}, Action: {action_vocab_size}"
    )
...
def main():
    ...
    results = evaluator.evaluate_all_sets(
        model=model, validation_dir=validation_dir, batch_size=32
    )
    ...
    mod_accs = [
        v["accuracy"] for k, v in results.items() if k != "val_base" and "accuracy" in v
    ]
