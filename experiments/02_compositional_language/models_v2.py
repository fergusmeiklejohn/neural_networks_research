    ...
    def call(self, rule_embeddings, modification_request, training=None):
        ...
        for gated_layer in self.gated_layers:
            modified_embeddings, gate = gated_layer(
                modified_embeddings, mod_signal_expanded, training=training
            )
            gates.append(gate)
        ...
        if modification is not None:
            ...
            def keep_original():
                ...
                for _ in range(self.rule_modifier.num_layers):
                    dummy_gate = tf.zeros(
                        [batch_size, seq_len, self.rule_modifier.d_model]
                    )
            ...
            rule_embeddings, gates = tf.cond(
                mod_sum > 0, apply_modification, keep_original
            )
...
def create_model_v2(
    command_vocab_size: int, action_vocab_size: int, d_model: int = 256, **kwargs
) -> CompositionalLanguageModelV2:
...
if __name__ == "__main__":
    ...
    model = create_model_v2(command_vocab_size=20, action_vocab_size=10, d_model=128)
