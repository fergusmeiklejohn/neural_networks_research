...
class SafeTrainingPipeline:
    ...
    def train_stage(self, 
        ...
        for epoch in range(epochs):
            ...
            # Save best model
            val_metric = val_metrics.get('accuracy', -val_metrics.get('loss', float('inf')))
    ...
    def train_step(self, model, batch, optimizer, loss_fn) -> float:
        ...
        # Clip gradients
        gradients = [tf.clip_by_norm(g, 1.0) if g is not None else None 
                    for g in gradients]
    ...
    def validate(self, model: keras.Model, dataset: tf.data.Dataset) -> Dict[str, float]:
        ...
        for batch in dataset.take(100):  # Limit validation for speed
            ...
            correct = tf.reduce_sum(tf.cast(predictions == batch['targets'], tf.float32))
