...
from models_v2 import GatedModificationLayer, create_model_v2
...
try:
    ...
    try:
        ...
        print(
            f"✓ Rule extraction works, embeddings shape: {rule_outputs['embeddings'].shape}"
        )
    except Exception as e:
    ...
    try:
        ...
        modified, gates = model.rule_modifier(
            rule_embeddings, test_inputs["modification"], training=True
        )
    ...
    try:
        logits = model.sequence_generator(
            rule_embeddings, test_inputs["target"], training=True
        )
