...
class SimpleSeq2SeqModel(keras.Model):
    ...
    def __init__(
        ...
        self.encoder_lstm = keras.layers.LSTM(
            d_model, return_sequences=True, return_state=True, dropout=dropout_rate
        )
        ...
        self.attention = keras.layers.MultiHeadAttention(
            num_heads=4, key_dim=d_model // 4, dropout=dropout_rate
        )
    ...
    def decode_step(self, decoder_input, encoder_output, decoder_state, training=None):
        ...
        lstm_output, new_h, new_c = self.decoder_lstm(
            decoder_input, initial_state=decoder_state, training=training
        )
    ...
    def call(self, inputs, training=None):
        ...
        decoder_output, _, _ = self.decoder_lstm(
            decoder_embedded, initial_state=encoder_state, training=training
        )
    ...
    def generate_action(self, command_ids, modification=None, max_length=50):
        ...
        encoder_output, decoder_state = self.encode(
            command_ids, modification, training=False
        )
...
def create_mixed_dataset(
    ...
    for sample in train_data:
        ...
        all_examples.append(
            {"command": command, "action": action, "modification": None}
        )
    ...
    dataset = dataset.padded_batch(
        batch_size,
        padded_shapes={"commands": [None], "actions": [None], "modification": [8]},
        padding_values={"commands": 0, "actions": 0, "modification": 0.0},
    )
...
def train_simple_baseline(args):
    ...
    output_dir = (
        base_dir
        / "outputs"
        / f'simple_baseline_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    )
    ...
    for epoch in range(args.epochs):
        ...
        # Train for one epoch
        epoch_history = model.fit(train_dataset, epochs=1, verbose=1)
        ...
        if args.use_proper_validation and (epoch + 1) % 5 == 0:
            ...
            history["val_results"].append({"epoch": epoch + 1, "results": val_results})

            # Print summary
            print(f"\nValidation Results (Epoch {epoch + 1}):")
            print(
                f"Base accuracy: {val_results.get('val_base', {}).get('accuracy', 0):.4f}"
            )
            if "aggregate_metrics" in val_results:
                metrics = val_results["aggregate_metrics"]
                print(
                    f"Average modification accuracy: {metrics.get('average_modification_accuracy', 0):.4f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--d_model", type=int, default=128, help="Model dimension (default: 128)"
    )
    parser.add_argument(
