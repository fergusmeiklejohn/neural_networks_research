...
def save_checkpoint(model, stage, epoch, metrics, storage_dir, output_dir):
    ...
    if storage_dir:
        ...
        metrics_path = os.path.join(
            storage_dir, "metrics", f"stage_{stage}_epoch_{epoch}.json"
        )
...
def train_with_safeguards(storage_dir):
    ...
    # Training history
    training_history = {"config": config, "stages": []}
    ...
    for stage_idx, (stage_name, base_data, mods, epochs, lr) in enumerate(stages, 1):
        ...
        for epoch in range(epochs):
            ...
            if epoch == epochs - 1:  # Last epoch
                ...
                print(
                    f"Validation - Loss: {val_metrics[0]:.4f}, Accuracy: {val_metrics[1]:.4f}"
                )
        ...
        if storage_dir:
            shutil.copy(
                history_path, os.path.join(storage_dir, "training_history.json")
            )
...
def comprehensive_evaluation(storage_dir):
    ...
    results_path = os.path.join(
        "outputs/safeguarded_training", "evaluation_results.json"
    )
...
def main():
    ...
    try:
        ...
        comprehensive_evaluation(storage_dir)
