...
def create_mixed_dataset(base_data, modification_data, mix_ratio=0.5, tokenizer=None, batch_size=32):
    """
    ...
    """
    # Calculate split
    total_size = len(base_data) + len(modification_data)
    ...
    sampled_mods = np.random.choice(modification_data, size=min(n_modified, len(modification_data)), replace=False)
    ...
    print(f"Mixed dataset: {len(base_with_dummy_mods)} base + {len(sampled_mods)} modified = {len(combined_data)} total")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_run', action='store_true', help='Quick test with minimal data')
