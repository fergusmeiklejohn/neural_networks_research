...
class CompositionalLanguageModel:
    ...
    def __init__(self, input_vocab_size: int, output_vocab_size: int, 
                 max_length: int = 20, embedding_dim: int = 128,
                 lstm_dim: int = 256):
    ...
    def build_seq2seq_model(self):
        ...
        decoder_lstm = layers.LSTM(
            self.lstm_dim, return_sequences=True, 
    ...
    def build_rule_extractor(self):
        ...
        attention = layers.MultiHeadAttention(
            num_heads=4, key_dim=self.lstm_dim // 4
        )(lstm_out, lstm_out)
    ...
    def build_rule_modifier(self):
        ...
        model = keras.Model(
            [rule_input, mod_input], modified_rule,
            name='rule_modifier'
        )
        return model
...
class TrainingPipeline:
    ...
    def setup_paths(self):
        ...
        if os.path.exists('/notebooks'):
            ...
            self.storage_path = self.base_path / 'experiments/02_compositional_language/storage'
        
        # Create directories
        self.output_dir = self.base_path / 'experiments/02_compositional_language/outputs'
        self.checkpoint_dir = self.output_dir / 'checkpoints'
        self.log_dir = self.output_dir / 'logs'
        
        for dir_path in [self.output_dir, self.checkpoint_dir, self.log_dir, self.storage_path]:
    ...
    def save_checkpoint(self, model: CompositionalLanguageModel, epoch: int, 
                       metrics: Dict[str, float], stage: str = "main"):
        ...
        model.rule_extractor.save_weights(str(checkpoint_path) + "_extractor.weights.h5")
    ...
    def load_data(self, data_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if data_path is None:
            data_path = self.base_path / 'experiments/02_compositional_language/data/scan_data.json'
        ...
        self.log(f"Data loaded - Train: {len(data['train']['x'])}, "
                f"Val: {len(data['val']['x'])}, Test: {len(data['test']['x'])}")
        
        return data
    
    def train_epoch(self, model: CompositionalLanguageModel, 
        ...
        history = model.model.fit(
            [train_x, train_decoder_input], train_y,
    ...
    def evaluate_modifications(self, model: CompositionalLanguageModel,
                             modifications: Dict[str, List[Tuple[str, str]]],
                             vocab: Dict[str, Dict[str, int]]) -> Dict[str, float]:
    ...
    def run_training(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            # Log metrics
            self.log(f"Train Loss: {metrics['train_loss']:.4f}, "
        ...
        test_metrics = model.model.evaluate(
            [data['test']['x'], test_decoder_input], data['test']['y'],
            verbose=0
        )
        ...
        with open(self.metrics_file, 'w') as f:
            json.dump({
                ...
                'modification_results': mod_results if 'modifications' in data else {},
