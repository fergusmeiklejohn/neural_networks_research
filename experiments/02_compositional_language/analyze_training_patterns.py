...
def analyze_loss_patterns(experiment_data):
    ...
    for stage in stages:
        ...
        if len(history) > 1:
            ...
            loss_deltas = [abs(history[i]['loss'] - history[i-1]['loss']) 
...
def main():
    ...
    for exp_name, exp_data in results['experiments'].items():
        ...
        for i, stage_name in enumerate(patterns['stage_names']):
            print(f"\n  {stage_name}:")
            print(f"    Loss: {patterns['initial_losses'][i]:.3f} → "
                  f"{patterns['final_losses'][i]:.3f}")
            print(f"    Convergence speed: {patterns['convergence_speed'][i]:.6f}")
            
            if i > 0:
                loss_jump = patterns['initial_losses'][i] - patterns['final_losses'][i-1]
        ...
        print(f"    Learning modifications: {indicators['learning_modifications']}/3 stages")
        print(f"    Ignoring modifications: {indicators['ignoring_modifications']}/3 stages")
