...
class TrainingScriptValidator:
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        try:
                            ...
                            self.issues.append(f"Import Error: Cannot import '{alias.name}'")
                    ...
                    try:
                        ...
                        for alias in node.names:
                            if not hasattr(module, alias.name):
                                self.issues.append(f"Import Error: Cannot import '{alias.name}' from '{module_name}'")
                    except ImportError:
                        self.issues.append(f"Import Error: Cannot import module '{module_name}'")
    ...
    def _check_method_calls(self):
        ...
        patterns = [
            (r'(\w+)\.load_modifications\(\)', 'ModificationGenerator', 'modification_generator'),
                ...
                try:
                    ...
                    if module_name:
                        ...
                        try:
                            ...
                            method_name = pattern.split('\\.')[-1].replace('\\(', '').replace(')', '')
                    ...
                    self.warnings.append(f"Could not validate {expected_class}.{method_name}: {e}")
    ...
    def _check_file_operations(self):
        ...
        hardcoded_paths = re.findall(r'["\']\/(?:home|Users)\/[^"\']+["\']', self.content)
        if hardcoded_paths:
    ...
    def _check_common_pitfalls(self):
        ...
        if 'tokenizer.save(' in self.content:
            self.issues.append("Uses tokenizer.save() - should be tokenizer.save_vocabulary()")
        ...
        # Memory settings
        if 'TF_GPU_ALLOCATOR' not in self.content and 'tensorflow' in self.content.lower():
