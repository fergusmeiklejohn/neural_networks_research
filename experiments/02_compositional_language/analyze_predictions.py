...
def load_model_and_tokenizer(checkpoint_path, vocabulary_path):
    ...
    tokenizer.command_index_word = {v: k for k, v in tokenizer.command_word_index.items()}
...
def analyze_predictions(model, tokenizer, test_samples, modification_pairs):
    ...
    for i, mod_pair in enumerate(mod_examples):
        ...
        print(f"Modification Applied: {predicted_action != mod_pair.original_sample.action}")
        print(f"Correct Modification: {predicted_action == mod_pair.modified_sample.action}")
    ...
    print(f"Max embedding difference: {tf.reduce_max(tf.abs(embeddings_with_mod - embeddings_no_mod)):.4f}")
...
def main():
    ...
    parser.add_argument('--vocabulary', help='Path to vocabulary.json (default: same dir as checkpoint)')
    parser.add_argument('--num_examples', type=int, default=10, help='Number of examples to analyze')
    ...
    train_data = [{'command': s.command, 'action': s.action} for s in data_loader.train_samples]
    test_data_interpolation = [{'command': s.command, 'action': s.action} for s in data_loader.test_samples_interpolation]
    ...
    # Analyze predictions
    analyze_predictions(model, tokenizer, test_data_interpolation[:args.num_examples], modification_pairs)
