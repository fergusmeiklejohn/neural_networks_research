...
class SCANDataGenerator:
    ...
    def generate_simple_commands(self, n: int = 1000) -> List[Tuple[str, str]]:
        ...
        for action, output in list(self.primitives.items())[
            :4
        ]:  # walk, run, jump, look
        ...
        if len(commands) < n:
            ...
            for _ in range(n - len(commands)):
                ...
                cmd = (
                    f"{acts[0][0]} {connector1} {acts[1][0]} {connector2} {acts[2][0]}"
                )
    ...
    def create_rule_modifications(
        ...
        for cmd, out in commands:
            new_cmd = (
                cmd.replace("jump", "TEMP")
                .replace("walk", "jump")
                .replace("TEMP", "walk")
            )
            new_out = (
                out.replace("JUMP", "TEMP")
                .replace("WALK", "JUMP")
                .replace("TEMP", "WALK")
            )
        ...
        for cmd, out in commands:
            new_cmd = (
                cmd.replace("left", "TEMP")
                .replace("right", "left")
                .replace("TEMP", "right")
            )
            new_out = (
                out.replace("TURN_LEFT", "TEMP")
                .replace("TURN_RIGHT", "TURN_LEFT")
                .replace("TEMP", "TURN_RIGHT")
            )
                ...
                for action in ["WALK", "RUN", "JUMP", "LOOK"]:
                    if action in out:
                        ...
                        if count == 2:  # This was a 'twice' command
                            new_out = out.replace(
                                f"{action} {action}", f"{action} {action} {action}"
                            )
                            break
                else:
                    new_out = out
    ...
    def encode_data(self, data: List[Tuple[str, str]], max_length: int = 20):
        ...
        for cmd, out in data:
            ...
            input_ids = [
                self.input_vocab.get(token, self.input_vocab["<UNK>"])
            ...
            output_ids = [
                self.output_vocab.get(token, self.output_vocab["<UNK>"])
                for token in output_tokens
            ]

            # Pad or truncate
            if len(output_ids) < max_length:
                output_ids += [self.output_vocab["<PAD>"]] * (
                    max_length - len(output_ids)
                )
            else:
    ...
    def save_data(self, output_dir: str):
        ...
        data_dict = {
            ...
            "vocab": {"input": self.input_vocab, "output": self.output_vocab},
            "metadata": {
