...
class ImprovedValidator:
    ...
    def test_model_count_params(self):
        ...
        try:
            ...
            raise AssertionError(f"count_params should have failed but returned {params}")
    ...
    def test_full_training_pipeline(self):
        ...
        for model_name, model_func in [
            ("v1", lambda: create_model(len(tokenizer.command_to_id), 
                                       len(tokenizer.action_to_id), 
                                       d_model=64)),
            ("v2", lambda: create_model_v2_fixed(len(tokenizer.command_to_id), 
                                                len(tokenizer.action_to_id), 
                                                d_model=64))
        ]:
            print(f"\nTesting {model_name}...")
