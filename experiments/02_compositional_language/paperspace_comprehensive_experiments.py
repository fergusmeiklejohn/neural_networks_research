...
class ComprehensiveExperimentRunner:
    ...
    def setup_directories(self):
        ...
        self.output_dir = self.base_path / 'experiments' / '02_compositional_language' / f'comprehensive_results_{self.timestamp}'
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        # Persistent storage for Paperspace
        if os.path.exists('/storage'):
            self.storage_dir = Path(f'/storage/compositional_comprehensive_{self.timestamp}')
    ...
    def save_checkpoint(self, model, experiment_name, stage, epoch, metrics):
        ...
        metrics_path = self.output_dir / 'checkpoints' / f"{experiment_name}_metrics.jsonl"
    ...
    def load_data(self):
        ...
        self.train_data = [{'command': s.command, 'action': s.action} 
                          for s in self.train_samples]
        self.val_data = [{'command': s.command, 'action': s.action} 
    ...
    def create_mixed_dataset(self, base_data, modifications, mix_ratio, batch_size=32):
        ...
        n_base = int(n_modified / mix_ratio * (1 - mix_ratio)) if mix_ratio > 0 else len(base_data)
        
        # Sample
        sampled_base = np.random.choice(base_data, size=min(n_base, len(base_data)), replace=False)
        sampled_mods = np.random.choice(mod_data, size=min(n_modified, len(mod_data)), replace=False)
    ...
    def run_experiment(self, experiment_name, model_version='v1', use_mixed_training=False):
        ...
        if use_mixed_training:
            stages = [
                {'name': 'Stage 1: Basic SCAN', 'mix_ratio': 0.0, 'epochs': 3, 'lr': 0.001},
                {'name': 'Stage 2: Mixed 70/30', 'mix_ratio': 0.3, 'epochs': 3, 'lr': 0.0005},
                {'name': 'Stage 3: Mixed 50/50', 'mix_ratio': 0.5, 'epochs': 3, 'lr': 0.0002},
                {'name': 'Stage 4: Mixed 30/70', 'mix_ratio': 0.7, 'epochs': 3, 'lr': 0.0001}
            ]
        else:
            stages = [
                {'name': 'Stage 1: Basic SCAN', 'mix_ratio': 0.0, 'epochs': 3, 'lr': 0.001},
                {'name': 'Stage 2: Simple Mods', 'mix_ratio': 1.0, 'epochs': 3, 'lr': 0.0005},
                {'name': 'Stage 3: Complex Mods', 'mix_ratio': 1.0, 'epochs': 3, 'lr': 0.0002},
                {'name': 'Stage 4: Novel Gen', 'mix_ratio': 1.0, 'epochs': 3, 'lr': 0.0001}
            ]
        ...
        for stage_idx, stage in enumerate(stages):
            ...
            if stage['mix_ratio'] == 0.0:
                dataset = create_dataset(self.train_data[:5000], self.tokenizer, batch_size=32)
            ...
            for epoch in range(stage['epochs']):
                ...
                try:
                    ...
                    self.save_checkpoint(
                        ...
                        stage_idx+1, epoch+1, metrics
                    )
                    
                except Exception as e:
                    print(f"Error in training: {e}")
                    # Save emergency checkpoint
                    emergency_path = self.storage_dir / f'emergency_{experiment_name}_s{stage_idx}_e{epoch}.h5'
                    try:
            ...
            # Validate
            val_dataset = create_dataset(self.val_data[:1000], self.tokenizer, batch_size=32)
    ...
    def analyze_results(self):
        ...
        for exp_name, exp_data in self.results['experiments'].items():
            ...
            # Get key metrics
            stage1_acc = exp_data['stages'][0]['val_accuracy'] if exp_data['stages'] else 0
            final_acc = exp_data['stages'][-1]['val_accuracy'] if exp_data['stages'] else 0
            degradation = (stage1_acc - final_acc) / stage1_acc * 100 if stage1_acc > 0 else 0
