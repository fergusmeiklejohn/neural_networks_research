    ...
    try:
        ...
        for batch in dataset.take(1):
            if isinstance(batch, tuple) and len(batch) == 2:
                ...
                raise ValueError(
                    f"Dataset returns {type(batch)}, not (inputs, targets)!"
                )
        ...
        history = model.fit(dataset.take(1), epochs=1, verbose=1)  # Just one batch
        ...
        for stage_name, base_data, mods in stages:
            ...
            # Train for 1 step
            history = model.fit(stage_dataset.take(1), epochs=1, verbose=0)
