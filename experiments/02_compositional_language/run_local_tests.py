...
def test_environment_setup():
    ...
    try:
        ...
        model = keras.Sequential([layers.Dense(10, input_shape=(5,)), layers.Dense(1)])
    ...
    try:
        ...
        for dir_path in test_dirs:
            full_path = os.path.join(
                base, "experiments/02_compositional_language", dir_path
            )
...
def test_scan_data_generation():
    ...
    try:
        ...
        commands = [
            ...
            (
                "look around left",
                "TURN_LEFT LOOK TURN_LEFT LOOK TURN_LEFT LOOK TURN_LEFT LOOK",
            ),
...
def test_memory_usage():
    ...
    try:
        ...
        y_data = np.random.randint(
            0, 20, (data_size,)
        )  # 1D labels for sparse_categorical_crossentropy

        data_memory = process.memory_info().rss / 1024 / 1024
        print(
            f"   After data creation: {data_memory:.2f} MB (+{data_memory-initial_memory:.2f} MB)"
        )
        ...
        print(
            f"   After model build: {model_memory:.2f} MB (+{model_memory-data_memory:.2f} MB)"
        )
        ...
        print(
            f"   After training: {train_memory:.2f} MB (+{train_memory-model_memory:.2f} MB)"
        )
...
def main():
    ...
    for result in results.results:
        ...
        print(
            f"{symbol} {result['test']}: {result['status']} ({result['duration']:.2f}s)"
        )
        if result["message"]:
