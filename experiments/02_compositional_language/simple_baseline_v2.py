...
def create_simple_model(command_vocab_size: int, 
                       action_vocab_size: int,
                       d_model: int = 128) -> keras.Model:
    ...
    embedding = keras.layers.Embedding(
        max(command_vocab_size, action_vocab_size), 
        d_model,
        mask_zero=True
    )
    ...
    # Create model
    model = keras.Model(
        inputs=[command_input, action_input, modification_input],
        outputs=output
    )
...
def main():
    ...
    output_dir = base_dir / 'outputs' / f'simple_baseline_v2_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    commands, actions, targets, modifications = load_and_prepare_data(data_path, tokenizer)
