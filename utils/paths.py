...
def get_experiment_path(experiment_name: str, subpath: str = "", create: bool = True) -> Path:
    """
...
def get_checkpoint_path(experiment_name: str, model_name: str, create: bool = True) -> Path:
    """
...
def get_results_path(experiment_name: str, results_name: str, create: bool = True) -> Path:
    """
...
def find_files(pattern: str, search_path: Optional[Path] = None, recursive: bool = True) -> List[Path]:
    """
    ...
    logger.debug(f"Found {len(matches)} files matching pattern '{pattern}' in {search_path}")
    return matches
