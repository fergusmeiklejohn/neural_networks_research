...
class TTAEvaluator:
    ...
    def evaluate_trajectory_prediction(
        self, model: keras.Model, trajectories: np.ndarray, input_steps: int = 1
    ) -> Dict[str, float]:
    ...
    def run_evaluation(
        ...
        for data_type in data_types:
            ...
            for model_name in model_names:
                ...
                print(
                    f"    MSE: {const_metrics['mse']:.4f} ± {const_metrics['mse_std']:.4f}"
                )
                ...
                print(
                    f"    MSE: {ood_metrics['mse']:.4f} ± {ood_metrics['mse_std']:.4f}"
                )
                degradation = (
                    (ood_metrics["mse"] - const_metrics["mse"])
                    / const_metrics["mse"]
                    * 100
                )
                ...
                for tta_method in tta_methods:
                    ...
                    try:
                        ...
                        print(
                            f"    MSE: {tta_metrics['mse']:.4f} ± {tta_metrics['mse_std']:.4f}"
                        )
                        improvement = (
                            (ood_metrics["mse"] - tta_metrics["mse"])
                            / ood_metrics["mse"]
                            * 100
                        )
                        print(f"    Improvement over no-TTA: {improvement:.1f}%")
                        print(
                            f"    Adaptation time: {tta_metrics['adaptation_time']:.3f}s"
                        )
                        ...
                        results.append(
                            {
                                ...
                                "mean_adaptation_loss": tta_metrics.get(
    ...
    def visualize_results(self, save_path: Optional[Path] = None):
        ...
        pivot_mse = valid_results.pivot_table(
            values="mse", index="model", columns="tta_method", aggfunc="mean"
        )
    ...
    def save_results(self, output_dir: Optional[Path] = None):
        ...
        summary = {
            ...
            "best_result": self.results.loc[self.results["mse"].idxmin()].to_dict()
            if not self.results.empty
            else None,
            "average_improvements": self.results[self.results["tta_method"] != "none"]
            .groupby("tta_method")["improvement"]
            .mean()
            .to_dict()
            if "improvement" in self.results.columns
            else {},
        }
...
def main():
    ...
    if not results.empty:
        ...
        print(
            results.nsmallest(5, "mse")[["model", "tta_method", "mse", "improvement"]]
        )
