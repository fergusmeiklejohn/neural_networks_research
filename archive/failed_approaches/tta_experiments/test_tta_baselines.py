...
def test_model_with_tta(model_path, test_data, model_type="generic"):
    ...
    results["no_tta"] = {"mse": np.mean(mse_scores), "mse_std": np.std(mse_scores)}
    print(
        f"   MSE: {results['no_tta']['mse']:.4f} ± {results['no_tta']['mse_std']:.4f}"
    )

    # Test with TENT
    print("\n2. Testing with TENT...")
    try:
        tent_model = TTAWrapper(
        ...
        results["tent"] = {"mse": np.mean(mse_scores), "mse_std": np.std(mse_scores)}
        print(
            f"   MSE: {results['tent']['mse']:.4f} ± {results['tent']['mse_std']:.4f}"
        )

        # Calculate improvement
        improvement = (
            (results["no_tta"]["mse"] - results["tent"]["mse"])
            / results["no_tta"]["mse"]
            * 100
        )
    ...
    try:
        ...
        print(
            f"   MSE: {results['physics_tent']['mse']:.4f} ± {results['physics_tent']['mse_std']:.4f}"
        )

        improvement = (
            (results["no_tta"]["mse"] - results["physics_tent"]["mse"])
            / results["no_tta"]["mse"]
            * 100
        )
        print(f"   Improvement: {improvement:.1f}%")
    ...
    if model_type != "pinn":  # PINNs might not work well with TTT
        ...
        try:
            ...
            results["ttt"] = {"mse": np.mean(mse_scores), "mse_std": np.std(mse_scores)}
            print(
                f"   MSE: {results['ttt']['mse']:.4f} ± {results['ttt']['mse_std']:.4f}"
            )

            improvement = (
                (results["no_tta"]["mse"] - results["ttt"]["mse"])
                / results["no_tta"]["mse"]
                * 100
            )
            print(f"   Improvement: {improvement:.1f}%")
...
def main():
    ...
    model_paths = [
        (
            base_dir / "outputs/baseline_results/gflownet_model_20250715_062359.keras",
            "gflownet",
        ),
        (
            base_dir / "outputs/baseline_results/maml_model_20250715_062721.keras",
            "maml",
        ),
