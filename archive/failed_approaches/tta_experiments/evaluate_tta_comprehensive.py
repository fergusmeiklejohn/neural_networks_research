    ...
    try:
        ...
        for method_name, config in tta_configs.items():
            ...
            if method_name != "No TTA":
                improvement = (
                    (varying_results["No TTA"]["mse"] - result["mse"])
                    / varying_results["No TTA"]["mse"]
                    * 100
                )
            ...
            for method_name, config in tta_configs.items():
                if method_name in ["No TTA", "PhysicsTENT"]:  # Test subset for speed
                    ...
                    result = evaluate_with_tta(
                        model, ood_data["trajectories"], config, n_samples=10
                    )
        ...
        print("=" * 70)
        print(
            f"{'Method':<15} {'Const Grav':<12} {'Time-Vary':<12} {'Improvement':<12}"
        )
        print("-" * 51)
        ...
        for method in ["No TTA", "TENT", "PhysicsTENT", "TTT"]:
            if method in varying_results:
                ...
                degradation = (
                    (mse - baseline_results["mse"]) / baseline_results["mse"] * 100
                )

                if method != "No TTA":
                    improvement = (
                        (varying_results["No TTA"]["mse"] - mse)
                        / varying_results["No TTA"]["mse"]
                        * 100
                    )
        ...
        summary = {
            ...
            "ood_degradation": (
                varying_results["No TTA"]["mse"] - baseline_results["mse"]
            )
            / baseline_results["mse"]
            * 100,
            "best_tta_method": best_method[0],
            "best_tta_improvement": (
                varying_results["No TTA"]["mse"] - best_method[1]["mse"]
            )
            / varying_results["No TTA"]["mse"]
            * 100,
        ...
        print("\n" + "=" * 70)
        if any(
            varying_results[m]["mse"] < varying_results["No TTA"]["mse"]
            for m in tta_configs
            if m != "No TTA"
        ):
            print("✓ SUCCESS: TTA improves performance on true OOD scenarios!")
            print(
                f"  Best improvement: {summary['best_tta_improvement']:.1f}% with {best_method[0]}"
            )
        else:
