...
def evaluate_tta_multistep(model, test_trajectories, tta_config=None, 
                          input_steps=5, output_steps=10, n_samples=20):
    ...
    for lr in learning_rates:
        for steps in adapt_steps:
            ...
            result = evaluate_tta_multistep(
                model, test_data, tta_config,
                input_steps=input_steps, n_samples=10
            )
...
def main():
    ...
    for input_steps in input_lengths:
        ...
        baseline_result = evaluate_tta_multistep(
            ...
            None, input_steps=input_steps
        )
        print(f"  MSE: {baseline_result['mse']:.2f} (±{baseline_result['mse_std']:.2f})")
        ...
        for method_name, config in tta_methods.items():
            ...
            if 'improvement_mean' in result:
                ...
                print(f"  Samples improved: {result['improved_count']}/{result['n_samples']}")
