...
def simple_test():
    ...
    try:
        ...
        for method in methods:
            ...
            print(f"  Change from original: {np.mean(np.abs(pred_adapted - pred1)):.6f}")
        ...
        for i, layer in enumerate(model_copy.layers):
            if isinstance(layer, keras.layers.BatchNormalization):
                if i < len(bn_stats_before):
                    mean_change = np.mean(np.abs(layer.moving_mean.numpy() - bn_stats_before[i]['mean']))
                    var_change = np.mean(np.abs(layer.moving_variance.numpy() - bn_stats_before[i]['var']))
                    
                    if mean_change > 1e-6 or var_change > 1e-6:
                        print(f"  BN layer {i}: mean changed by {mean_change:.6f}, var by {var_change:.6f}")
                        bn_changed = True
