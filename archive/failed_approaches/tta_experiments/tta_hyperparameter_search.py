...
def create_flexible_model(
    input_shape=(1, 8), output_shape=(10, 8), architecture="dense", hidden_units=128
):
...
def main():
    ...
    for lr, steps, arch, units in product(
        ...
        # Create model
        model = create_flexible_model(architecture=arch, hidden_units=units)
        ...
        try:
            ...
            print(
                f"  Improvement: {result['improvement']:.1f}% (±{result['improvement_std']:.1f}%)"
            )
