    ...
    for i, layer in enumerate(model.layers):
        if hasattr(layer, "weights") and layer.weights:
            for weight in layer.weights:
                ...
                print(
                    f"    Shape: {w.shape}, Mean: {np.mean(w):.6f}, Std: {np.std(w):.6f}"
                )
...
def track_adaptation_changes(model, test_data, tta_method="tent", n_steps=5):
    ...
    tta_model = TTAWrapper(
        model, tta_method=tta_method, adaptation_steps=n_steps, learning_rate=1e-4
    )
                ...
                for weight in layer.weights:
                    if weight.name in initial_weights:
                        ...
                        if current.shape != initial.shape:
                            print(
                                f"    {weight.name} shape changed: {initial.shape} -> {current.shape}"
                            )
                        else:
    ...
    all_same = all(
        np.allclose(predictions_per_step[0], pred) for pred in predictions_per_step[1:]
    )
    if all_same:
...
def main():
    ...
    for method in methods:
        ...
        # Track adaptation
        result = track_adaptation_changes(
            model_copy, test_trajectory, method, n_steps=10
        )
    ...
    for method in methods:
        ...
        print(
            f"  Initial vs Final prediction diff: {np.mean(np.abs(results[method]['final'] - results[method]['initial'])):.6f}"
        )
