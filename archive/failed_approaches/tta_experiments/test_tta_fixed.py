...
def test_tta_on_ood_physics():
    ...
    try:
        ...
        if const_files:
            ...
            print(
                f"Loaded constant gravity data: {len(const_data['trajectories'])} trajectories"
            )
        else:
        ...
        if varying_files:
            ...
            print(
                f"Loaded time-varying gravity data: {len(ood_data['trajectories'])} trajectories"
            )
        else:
    ...
    print(
        f"In-distribution MSE: {np.mean(mse_in_dist):.4f} ± {np.std(mse_in_dist):.4f}"
    )
    ...
    print(
        f"OOD MSE (no TTA): {np.mean(mse_ood_no_tta):.4f} ± {np.std(mse_ood_no_tta):.4f}"
    )
    ...
    for tta_method in ["tent", "physics_tent"]:
        ...
        tta_model.reset()
        restored_pred = model.predict(
            test_ood_trajs[0][0:1].reshape(1, 1, 8), verbose=0
        )
        ...
        print(
            f"  OOD MSE (with {tta_method}): {np.mean(mse_ood_tta):.4f} ± {np.std(mse_ood_tta):.4f}"
        )
        print(
            f"  Improvement: {(1 - np.mean(mse_ood_tta)/np.mean(mse_ood_no_tta))*100:.1f}%"
        )
    ...
    print(
        f"  Degradation without TTA: {np.mean(mse_ood_no_tta)/np.mean(mse_in_dist):.1f}x"
    )
