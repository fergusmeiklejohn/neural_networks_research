...
def track_parameter_changes(model_before, model_after):
    ...
    for i, (var_before, var_after) in enumerate(
        zip(model_before.variables, model_after.variables)
    ):
...
def debug_single_adaptation(model, trajectory, tta_kwargs, debug_info):
    ...
    print(
        f"Change in MSE: {adapted_mse - baseline_mse:.4f} ({(adapted_mse/baseline_mse - 1)*100:.1f}%)"
    )

    # Track parameter changes
    param_changes = track_parameter_changes(
        type(model).from_config(
            model.get_config()
        ),  # Create dummy model with original weights
        model,
    )
    ...
    print(
        f"  Parameters changed: {total_changed:,} ({100*total_changed/total_params:.2f}%)"
    )

    # Show top 5 most changed layers
    sorted_changes = sorted(
        param_changes.items(), key=lambda x: x[1]["mean_abs_change"], reverse=True
    )
    print("\n  Top 5 layers by mean absolute change:")
    for layer_name, info in sorted_changes[:5]:
        if info["mean_abs_change"] > 1e-10:
            print(
                f"    {layer_name}: mean_change={info['mean_abs_change']:.2e}, "
    ...
    for t in range(min(3, len(y_pred_baseline))):
        for f in range(2):
            print(
                f"   {t:2d}  |    {f:2d}   | {y_pred_baseline[t,f]:8.4f} | "
...
def main():
    ...
    for config_info in debug_configs:
        result = debug_single_adaptation(
            model, ood_data["trajectories"][0], config_info["kwargs"], config_info
        )
    ...
    if not any_improvement:
        ...
        total_changed = sum(
            r["total_params_changed"] for r in all_results.values()
        ) / len(all_results)
        ...
        mean_pred_change = np.mean(
            [r["mean_pred_change"] for r in all_results.values()]
        )
