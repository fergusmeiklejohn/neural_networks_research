...
from models.test_time_adaptation.tta_wrappers import TTAWrapper, create_tta_baseline
...
def load_test_data(data_type: str = 'time_varying') -> Dict:
    ...
    if data_type == 'time_varying':
        ...
        return {
            'trajectories': np.random.randn(100, 50, 8),  # 100 trajectories, 50 timesteps, 8 features
...
def plot_tta_comparison(results: Dict[str, Dict], save_path: Optional[str] = None):
    ...
    for i, model in enumerate(models):
        mse_values = [results[model].get(method, {}).get('mse', np.nan) 
    ...
    # Time comparison
    for i, model in enumerate(models):
        time_values = [results[model].get(method, {}).get('time', np.nan) 
                       for method in methods]
...
def main():
    ...
    for data_type, results in all_results.items():
        ...
        for model in models:
            ...
            print(f"{model:12s}: Best method = {best_method:12s}, "
