...
def main():
    try:
        ...
        const_mse, _ = evaluate_model(
            model, const_data['trajectories'][80:], 
            "Constant Gravity (baseline)"
        )
        
        # Time-varying without TTA
        varying_mse, _ = evaluate_model(
            model, varying_data['trajectories'], 
            "Time-Varying Gravity (no TTA)"
        )
        ...
        physics_tent_mse, physics_tent_time = evaluate_model(
            ...
            adapt=True, tta_method='physics_tent'
        )
        ...
        print(f"OOD (PhysicsTENT): {physics_tent_mse:.4f} ({physics_tent_improvement:.1f}% improvement)")
