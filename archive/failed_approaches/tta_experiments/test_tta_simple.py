...
def generate_test_trajectory(n_steps=50, noise_level=0.1):
    ...
    for t in range(1, n_steps):
        # Update positions
        trajectory[t, [0, 4]] = trajectory[t-1, [0, 4]] + trajectory[t-1, [2, 6]] * dt
        trajectory[t, [1, 5]] = trajectory[t-1, [1, 5]] + trajectory[t-1, [3, 7]] * dt
...
def test_tta_methods():
    ...
    # Test 2: TENT
    print("\n2. Testing TENT...")
    tent_wrapper = TTAWrapper(model, tta_method='tent', adaptation_steps=1, learning_rate=1e-4)
    mse_tent = []
    ...
    improvement = (np.mean(mse_baseline) - np.mean(mse_tent)) / np.mean(mse_baseline) * 100
    ...
    print(f"   PhysicsTENT MSE: {np.mean(mse_physics_tent):.4f} ± {np.std(mse_physics_tent):.4f}")
    improvement = (np.mean(mse_baseline) - np.mean(mse_physics_tent)) / np.mean(mse_baseline) * 100
    print(f"   Improvement: {improvement:.1f}%")
    ...
    improvement = (np.mean(mse_baseline) - np.mean(mse_ttt)) / np.mean(mse_baseline) * 100
    print(f"   Improvement: {improvement:.1f}%")
    ...
    print(f"TENT:         {np.mean(mse_tent):.4f} ({(np.mean(mse_baseline) - np.mean(mse_tent)) / np.mean(mse_baseline) * 100:.1f}% improvement)")
    print(f"PhysicsTENT:  {np.mean(mse_physics_tent):.4f} ({(np.mean(mse_baseline) - np.mean(mse_physics_tent)) / np.mean(mse_baseline) * 100:.1f}% improvement)")
    print(f"TTT:          {np.mean(mse_ttt):.4f} ({(np.mean(mse_baseline) - np.mean(mse_ttt)) / np.mean(mse_baseline) * 100:.1f}% improvement)")
