...
def run_hyperparameter_search():
                    ...
                    for update_bn in hp_grid['update_bn_only']:
                        ...
                        if tta_method == 'tent':
                            ...
                            config_result = {
                                ...
                                'improvement': float((1 - result['mean_mse']/baseline_ood[0]) * 100)
                            }
    ...
    for i, config in enumerate(sorted_results[:5]):
        ...
        print(f"   lr={config['learning_rate']}, steps={config['adaptation_steps']}, "
    ...
    print(f"Validation MSE (50 trajectories): {np.mean(validation_errors):.4f} ± {np.std(validation_errors):.4f}")
    print(f"Validation improvement: {(1 - np.mean(validation_errors)/baseline_ood[0])*100:.1f}%")
