...
from models.test_time_adaptation.regression_tta_v2 import (
...
def test_gradient_computation():
    ...
    # Create TTA wrapper
    tta = RegressionTTAV2(
        model, adaptation_steps=1, learning_rate=1e-3, update_bn_only=False
    )
...
def test_bn_only_adaptation():
    ...
    tta = RegressionTTAV2(
        model, adaptation_steps=1, learning_rate=1e-2, update_bn_only=True
    )
    ...
    initial_bn_gamma = (
        model.get_layer("bn1").gamma.numpy().copy()
        if model.get_layer("bn1").gamma is not None
        else None
    )
    ...
    # Check what changed
    dense1_changed = not np.allclose(
        initial_dense1, model.get_layer("dense1").kernel.numpy()
    )
    bn_changed = initial_bn_gamma is not None and not np.allclose(
        initial_bn_gamma, model.get_layer("bn1").gamma.numpy()
    )

    print(f"Dense layer changed: {dense1_changed}")
    print(f"BatchNorm gamma changed: {bn_changed}")
    print(
        f"✓ BN-only adaptation {'working' if bn_changed and not dense1_changed else 'NOT working correctly'}"
    )
...
def test_state_restoration():
    ...
    print(
        f"✓ State restoration {'working' if restoration_error < 1e-5 else 'NOT working'}"
    )
...
def test_on_physics_data():
    ...
    configs = [
        {
            "name": "RegressionTTA (BN only)",
            "class": RegressionTTAV2,
            "update_bn_only": True,
            "lr": 1e-3,
        },
        {
            "name": "RegressionTTA (all params)",
            "class": RegressionTTAV2,
            "update_bn_only": False,
            "lr": 5e-4,
        },
        {
            "name": "PhysicsRegressionTTA",
            "class": PhysicsRegressionTTAV2,
            "update_bn_only": False,
            "lr": 5e-4,
        },
    ]
