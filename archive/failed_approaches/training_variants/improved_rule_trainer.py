...
import keras
import numpy as np
from distribution_invention_metrics import (
    DistributionInventionEvaluator,
)
from keras import callbacks, ops
from tqdm import tqdm

from models.core.physics_rule_extractor import (
    PhysicsRuleConfig,
    PhysicsRuleExtractor,
)


def load_improved_data_for_transformer(
    data_path: str, config: PhysicsRuleConfig, max_samples: int = None
):
    ...
    y_normalized = {
        ...
        "features": np.zeros(
            (num_samples, config.rule_embedding_dim)
        ),  # Dummy target for features
    }
...
def calculate_physics_accuracy(y_true_original, y_pred_original, tolerance=0.2):
    ...
    for i, param in enumerate(param_names):
        results[param] = {
            ...
            "rmse": np.sqrt(
                np.mean((y_pred_original[:, i] - y_true_original[:, i]) ** 2)
            ),
        }
...
class TransformerPhysicsLoss(keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        energy_penalty = ops.square(
            ops.maximum(0.0, y_pred["elasticity"] - 1.0)
        )  # Penalize > 1.0

        # Physical plausibility constraints
        gravity_penalty = ops.square(
            ops.maximum(0.0, ops.abs(y_pred["gravity"]) - 1.0)
        )  # Normalized gravity should be [0,1]
        friction_penalty = ops.square(
            ops.maximum(0.0, y_pred["friction"] - 1.0)
        ) + ops.square(ops.maximum(0.0, -y_pred["friction"]))
...
def train_improved_rule_extractor():
    ...
    if not data_dir.exists():
        print(
            "❌ No improved datasets found. Please run generate_improved_datasets.py first"
        )
        return None
    ...
    X_train, y_train_norm, y_train_orig = load_improved_data_for_transformer(
        data_dir / "train_data.pkl",
        config,
        max_samples=500,  # Start smaller for transformer
    )
            ...
            try:
                ...
                if len(trajectory.shape) == 2:
                    ...
                    if trajectory.shape[0] > self.config.sequence_length:
                        ...
                        padding = np.tile(
                            trajectory[-1:],
                            (self.config.sequence_length - trajectory.shape[0], 1),
                        )
                    ...
                    trajectory = trajectory.reshape(
                        self.config.sequence_length, self.config.feature_dim
                    )
                    trajectory = np.expand_dims(trajectory, 0)
                ...
                print(
                    f"Input trajectory shape: {trajectory.shape if hasattr(trajectory, 'shape') else type(trajectory)}"
                )
                if hasattr(trajectory, "shape") and len(trajectory.shape) >= 2:
                    ...
                    print(
                        f"First few features of first timestep: {trajectory.reshape(-1, trajectory.shape[-1])[0][:5]}"
                    )
    ...
    try:
            ...
            json.dump(
                {
                    ...
                    "parameter_accuracies": {
                        k: float(v) for k, v in results.parameter_accuracies.items()
                    },
                },
                f,
                indent=2,
        ...
        accuracy_metrics = calculate_physics_accuracy(
            y_val_orig_array, y_pred_orig, tolerance=0.2
        )

        print(f"\nBasic validation results:")
        print(
            f"Overall accuracy (20% tolerance): {accuracy_metrics['overall']['accuracy']:.3f}"
        )
