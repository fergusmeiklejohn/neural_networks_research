...
def train_rule_extractor(
    ...
    class AccuracyCallback(callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            if epoch % self.freq == 0:
                ...
                print(
                    f"  Overall 10% accuracy: {metrics['overall']['accuracy_10pct']:.3f}"
                )
                print(
                    f"  Overall 20% accuracy: {metrics['overall']['accuracy_20pct']:.3f}"
                )
                ...
                if use_wandb:
                    ...
                    for param in ["gravity", "friction", "elasticity", "damping"]:
                        wandb_metrics[f"{param}_acc_10pct"] = metrics[param][
                            "accuracy_10pct"
                        ]
                        wandb_metrics[f"{param}_acc_20pct"] = metrics[param][
                            "accuracy_20pct"
                        ]
                        wandb_metrics[f"{param}_mae"] = metrics[param]["mae"]

                    wandb_metrics["overall_acc_10pct"] = metrics["overall"][
                        "accuracy_10pct"
                    ]
                    wandb_metrics["overall_acc_20pct"] = metrics["overall"][
                        "accuracy_20pct"
                    ]
    ...
    if use_wandb:
        wandb_callback = callbacks.WandbCallback(monitor="val_loss", save_model=False)
    ...
    if use_wandb:
        ...
        final_wandb_metrics = {}
        for param in ["gravity", "friction", "elasticity", "damping"]:
            final_wandb_metrics[f"final_{param}_acc_10pct"] = final_metrics[param][
                "accuracy_10pct"
            ]
            final_wandb_metrics[f"final_{param}_acc_20pct"] = final_metrics[param][
                "accuracy_20pct"
            ]

        final_wandb_metrics["final_overall_acc_10pct"] = final_metrics["overall"][
            "accuracy_10pct"
        ]
        final_wandb_metrics["final_overall_acc_20pct"] = final_metrics["overall"][
            "accuracy_20pct"
        ]
