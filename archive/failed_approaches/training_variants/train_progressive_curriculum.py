...
class ProgressiveCurriculum:
    ...
    def __init__(
        ...
        self.stage_configs = {
            ...
            2: {
                ...
                "physics_weight_schedule": np.linspace(
                    0.1, 1.0, config.get("stage2_epochs", 50)
                ),
                "use_domain_randomization": False,
                "learning_rate": 5e-4,
                "focus_extrapolation": False,
    ...
    def _apply_domain_randomization(
        self, params: np.ndarray, range_factor: float
    ) -> np.ndarray:
    ...
    def _create_training_batch(
        self, batch_data: Dict, stage: int, epoch: int
        ...
        if config.get("use_domain_randomization", False):
            ...
            params = np.stack(
                [modified_batch["gravity"], modified_batch["friction"]], axis=-1
            )
    ...
    def _compute_metrics(self, stage: int, epoch: int) -> Dict[str, float]:
        ...
        in_dist_error = np.mean(
            np.abs(in_dist_preds - self.test_in_dist["future_trajectories"])
        )
        ...
        momentum_viol = self.physics_losses.momentum_conservation_loss(
            sample_preds
        ).numpy()
    ...
    def train_stage(self, stage: int) -> Dict[str, List[float]]:
        ...
        for epoch in range(config["epochs"]):
            ...
            for batch_idx in tqdm(range(n_batches), desc="Training"):
                ...
                with tf.GradientTape() as tape:
                    ...
                    predictions_dict = self.model(
                        modified_batch["trajectories"], training=True
                    )
                    ...
                    if physics_weight > 0:
                        physics_loss_dict = self.physics_losses.combined_physics_loss(
                            ...
                            {
                                "gravity": modified_batch["gravity"],
                                "friction": modified_batch["friction"],
                            },
                        )
            ...
            val_loss = np.mean(
                np.square(val_preds - self.val_data["future_trajectories"])
            )
            ...
            self.metrics_history["momentum_violation"].append(
                metrics["momentum_violation"]
            )
                ...
                wandb.log(
                    {
                        ...
                        f"stage{stage}/momentum_violation": metrics[
                            "momentum_violation"
                        ],
                        "epoch": epoch
                        + sum(
                            [self.stage_configs[s]["epochs"] for s in range(1, stage)]
                        ),
                    }
                )
    ...
    def _save_metrics(self):
        ...
        print(
            f"  Best Extrap Accuracy: {summary['best_extrap_accuracy']:.2%} (epoch {summary['best_extrap_epoch']})"
        )
...
def load_data():
    ...
    test_data = {
        "trajectories": np.concatenate(
            [test_interp_data["trajectories"], test_extrap_data["trajectories"]]
        ),
        "future_trajectories": np.concatenate(
            [
                test_interp_data["future_trajectories"],
                test_extrap_data["future_trajectories"],
            ]
        ),
        "gravity": np.concatenate(
            [test_interp_data["gravity"], test_extrap_data["gravity"]]
        ),
        "friction": np.concatenate(
            [test_interp_data["friction"], test_extrap_data["friction"]]
        ),
    }
