...
def load_full_data():
    ...
    data_dir = (
        Path(base_path)
        / "experiments/01_physics_worlds/data/processed/physics_worlds_v2_quick"
    )
...
def train_progressive_curriculum():
    ...
    val_dataset = tf.data.Dataset.from_tensor_slices(
        (val_data["trajectories"], val_data["future_trajectories"])
    ).batch(config["batch_size"])
    ...
    for stage_idx, stage_config in enumerate(stages):
        ...
        for epoch in range(stage_config["epochs"]):
            ...
            for batch in tqdm(train_dataset, desc="Training"):
                ...
                if stage_config.get("randomize", False):
                    noise_g = tf.random.uniform(
                        shape=gravity.shape, minval=-0.3, maxval=0.3
                    )
                    noise_f = tf.random.uniform(
                        shape=friction.shape, minval=-0.3, maxval=0.3
                    )
                ...
                with tf.GradientTape() as tape:
                    ...
                    if stage_config.get("use_physics", False):
                        ...
                        phys_loss = create_physics_loss(
                            predictions, targets, gravity, friction
                        )
            ...
            extrap_error = np.mean(
                np.abs(extrap_pred - test_extrap_data["future_trajectories"])
            )
            ...
            metrics = {
                ...
                f"stage_{stage_idx+1}/physics_loss": physics_loss / num_batches
                if stage_config.get("use_physics")
                else 0,
        ...
        # Save checkpoint
        checkpoint_path = (
            Path(config["checkpoint_dir"]) / f"stage_{stage_idx+1}_final.h5"
        )
    ...
    interp_error = np.mean(
        np.abs(interp_pred - test_interp_data["future_trajectories"])
    )
