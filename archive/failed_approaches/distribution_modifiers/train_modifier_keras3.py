...
def create_datasets(data, batch_size=32, val_ratio=0.15):
    ...
    return ((train_params, train_descs), train_targets), ((val_params, val_descs), val_targets)
...
def simple_training_loop(model, optimizer, train_data, val_data, epochs=50, batch_size=32):
...
def main():
    ...
    eval_results = evaluate_modification_types(base_modifier, processor, mod_pairs[:1000])
    ...
    for i, param_name in enumerate(processor.param_names):
        ...
        print(f"{param_name:12s} | Error: {param_error:.4f} | "
