...
def load_and_prepare_modification_data(data_path: str, max_samples: int = None):
    ...
    print(
        f"Data shapes: X_original={X_original_norm.shape}, X_modifications={X_modifications.shape}, y_target={y_target_norm.shape}"
    )
    print(
        f"Original physics range: {X_original_norm.min():.3f} to {X_original_norm.max():.3f}"
    )
    print(
        f"Target physics range: {y_target_norm.min():.3f} to {y_target_norm.max():.3f}"
    )
...
def calculate_modification_consistency(
    y_true_orig, y_pred_orig, X_modifications, tolerance=0.15
):
    ...
    results["overall"] = {"consistency": overall_consistency, "mae": overall_mae}
...
def train_distribution_modifier():
    ...
    (
        ...
        X_original_orig,
        y_target_orig,
    ) = load_and_prepare_modification_data(modification_data_path, max_samples=2000)
    ...
    X_orig_train_orig, X_orig_val_orig = (
        X_original_orig[:split_idx],
        X_original_orig[split_idx:],
    )
    ...
    train_gen = data_generator(
        X_orig_train, X_mod_train, y_train, batch_size, shuffle=True
    )
    ...
    # Calculate metrics
    metrics = calculate_modification_consistency(
        y_val_orig, y_pred_orig, X_mod_val, tolerance=0.15
    )

    print(f"\nDistribution Modification Results:")
    print(
        f"Overall consistency (15% tolerance): {metrics['overall']['consistency']:.3f}"
    )
    ...
    for key, value in metrics.items():
        if key.startswith("mod_type_"):
            ...
            print(
                f"  Modification {mod_type}: {value['consistency']:.3f} consistency, {value['samples']} samples"
            )
    ...
    for i in range(min(3, len(y_val_orig))):
        ...
        print(
            f"  Original: G={X_orig_val_orig[i,0]:.0f}, F={X_orig_val_orig[i,1]:.3f}, E={X_orig_val_orig[i,2]:.3f}, D={X_orig_val_orig[i,3]:.3f}"
        )
        print(
            f"  Target:   G={y_val_orig[i,0]:.0f}, F={y_val_orig[i,1]:.3f}, E={y_val_orig[i,2]:.3f}, D={y_val_orig[i,3]:.3f}"
        )
        print(
            f"  Predict:  G={y_pred_orig[i,0]:.0f}, F={y_pred_orig[i,1]:.3f}, E={y_pred_orig[i,2]:.3f}, D={y_pred_orig[i,3]:.3f}"
        )
...
def generate_synthetic_modification_data(output_path: str, num_samples: int = 2000):
    ...
    print(
        f"Generated and saved {len(modification_pairs)} modification pairs to {output_path}"
    )
