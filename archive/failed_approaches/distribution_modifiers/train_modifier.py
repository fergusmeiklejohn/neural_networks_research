...
class ModificationTrainer:
    ...
    def val_step(self, batch):
        ...
        relative_error = keras.ops.mean(
            keras.ops.abs(pred_params - target_params) / (keras.ops.abs(target_params) + 1e-6)
        )
        
        # Check which parameters changed correctly
        actual_changes = keras.ops.abs(target_params - params) / (keras.ops.abs(params) + 1e-6)
        pred_changes = keras.ops.abs(pred_params - params) / (keras.ops.abs(params) + 1e-6)
    ...
    def train(self, train_data, val_data, epochs=50, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            print(f"Val - Param Error: {avg_val_metrics['param_error']:.4f}, "
