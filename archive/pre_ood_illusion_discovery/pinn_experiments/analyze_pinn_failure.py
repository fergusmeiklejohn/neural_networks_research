...
# Load results
with open(
    "../../outputs/pinn_scaled_20250714_063917_results/final_results.json", "r"
) as f:
...
fig.suptitle(
    "PINN Failure Analysis: Why Physics-Informed Models Struggled", fontsize=16
)
...
for cond in ["earth", "moon", "jupiter"]:
    best = min(
        [
            baseline_results["baselines"][b]["final_metrics"][f"{cond}_mse"]
...
bars1 = ax.bar(
    x - width / 2, best_baseline_mse, width, label="Best Baseline", color="#2E7D32"
)
...
plt.tight_layout()
plt.savefig(
    "outputs/baseline_results/pinn_failure_analysis.png", dpi=300, bbox_inches="tight"
)
plt.close()
...
ax.text(
    ...
    fontsize=14,
    bbox=dict(
        boxstyle="round,pad=1", facecolor="#FFCDD2", edgecolor="#D32F2F", linewidth=2
    ),
)
...
plt.tight_layout()
plt.savefig(
    "outputs/baseline_results/pinn_failure_summary.png", dpi=300, bbox_inches="tight"
)
plt.close()
...
print(
    "This is a critical finding - physics-informed approaches need careful implementation."
)
