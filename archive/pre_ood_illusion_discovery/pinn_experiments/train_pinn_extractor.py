...
class ProgressiveTrainingCallback(callbacks.Callback):
    ...
    def __init__(
        self, total_steps: int = 300000, stage_transitions: Dict[int, str] = None
    ):
        super().__init__()
...
def load_physics_world_data(
    data_path: str, split: str = "train"
...
def create_progressive_dataset(
    ...
    if stage in ["in_distribution", "physics_introduction"]:
        ...
        if near_data is not None:
            ...
            X_train = np.concatenate([train_features, near_features[near_indices]])

            y_train = {
                key: np.concatenate(
                    [train_targets[key], near_targets[key][near_indices]]
                )
...
def train_physics_informed_model(
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        for epoch in range(epochs_per_stage):
            ...
            if epoch % 5 == 0:
                model.save(
                    os.path.join(model_save_path, f"pinn_{stage}_epoch_{epoch}.keras")
                )
