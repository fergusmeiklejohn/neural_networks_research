...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(n_batches):
            ...
            for var in model.trainable_variables:
                ...
                noise = np.random.randn(*var.shape).astype(np.float32) * 0.001 * float(loss_value)
    ...
    # Combine all data
    X_train_3 = np.concatenate([train_earth_mars[:, :-1], train_moon[:, :-1], train_jupiter[:, :-1]])
    y_train_3 = np.concatenate([train_earth_mars[:, 1:], train_moon[:, 1:], train_jupiter[:, 1:]])
