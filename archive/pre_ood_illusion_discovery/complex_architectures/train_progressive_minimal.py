...
class SimpleTransformerModel(keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        if target is not None:
            ...
            # Add encoder context
            decoder_output = decoder_output + tf.reduce_mean(
                encoded, axis=1, keepdims=True
            )

            # Project to vocabulary
            logits = self.output_layer(decoder_output)

            return {"logits": logits, "rule_outputs": {}}  # Compatibility

        return {"rule_embeddings": encoded, "rule_outputs": {}}
        ...
        outputs = self(
            {
                ...
                if modification is not None
                else tf.zeros((batch_size, 20), dtype=tf.int32),
            },
            training=False,
        )

        outputs["rule_embeddings"]
...
def train_progressive_curriculum_minimal(config: Dict):
    ...
    for stage in range(1, 5):
        ...
        for epoch in range(stage_config["epochs"]):
                ...
                try:
                    with tf.GradientTape() as tape:
                        ...
                        inputs = {
                            ...
                            "modification": batch.get(
                                "modification", tf.zeros_like(batch["command"][:, :20])
                            ),
                        }
            ...
            if num_batches > 0:
                ...
                try:
                    val_acc = compute_accuracy_simple(
                        model, val_dataset, tokenizer, max_samples=50
                    )
    ...
    for split_name in ["test_interpolation", "test_primitive_extrap"]:
        if split_name in splits:
            ...
            try:
                accuracy = compute_accuracy_simple(
                    model, test_dataset, tokenizer, max_samples=100
                )
