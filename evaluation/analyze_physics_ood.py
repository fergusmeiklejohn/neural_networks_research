...
class PhysicsOODAnalyzer:
        ...
        try:
            ...
            for test_name, test_set in test_data.items():
                ...
                analysis = self.verifier.analyze_dataset(
                    train_data, test_set, wrapped_model
                )
                ...
                # Save visualization
                viz_path = (
                    self.experiment_dir / f"ood_analysis_{model_name}_{test_name}.png"
                )
                self.verifier.visualize_analysis(
                    analysis, save_path=str(viz_path), show_hull=True, max_dims=2
                )
    ...
    def generate_summary_report(self, results_all: dict, output_path: Path):
        ...
        high_interpolation = [
            s for s in summary["test_scenarios"] if s["interpolation_pct"] > 70
        ]

        true_ood = [s for s in summary["test_scenarios"] if s["far_extrap_pct"] > 50]
...
def main():
    ...
    if not physics_exp_dir.exists() or not (physics_exp_dir / "data").exists():
        logger.info(
            "Real physics data not found, creating mock data for demonstration..."
        )
    ...
    if results:
        ...
        analyzer.generate_summary_report(results, report_path)
