...
class TrueOODVerifier:
    ...
    def fit_convex_hull(self, train_representations: np.ndarray):
        ...
        """
        logger.info(f"Fitting convex hull to {train_representations.shape[0]} training samples")
        ...
        if self.use_pca and train_representations.shape[1] > self.pca_components:
            ...
            logger.info(f"Reduced dimensions from {train_representations.shape[1]} to {self.pca_components}")
    ...
    def analyze_dataset(
        ...
        if hasattr(model, 'get_representations'):
            train_repr = model.get_representations(train_data, layer=representation_layer)
        ...
        logger.info(f"  Interpolation: {stats['n_interpolation']} ({stats['pct_interpolation']:.1f}%)")
        logger.info(f"  Near-extrapolation: {stats['n_near_extrapolation']} ({stats['pct_near_extrapolation']:.1f}%)")
        logger.info(f"  Far-extrapolation: {stats['n_far_extrapolation']} ({stats['pct_far_extrapolation']:.1f}%)")
    ...
    def visualize_analysis(
        ...
        if self.train_representations.shape[1] > max_dims:
            ...
            test_viz = {
                category: viz_pca.transform(np.array(points)) if len(points) > 0 else np.array([])
                for category, points in results.items()
                if category in ['interpolation', 'near_extrapolation', 'far_extrapolation']
            }
        else:
        ...
        if max_dims == 2 or train_viz.shape[1] == 2:
            ...
            ax.scatter(train_viz[:, 0], train_viz[:, 1], 
                      c='lightgray', alpha=0.3, s=20, label='Training')
            
            # Plot convex hull
            if show_hull and self.hull is not None:
                for simplex in self.hull.simplices:
                    ax.plot(train_viz[simplex, 0], train_viz[simplex, 1], 'k-', alpha=0.3)
            ...
            for category, points in test_viz.items():
                if len(points) > 0:
                    ax.scatter(points[:, 0], points[:, 1], 
                             c=colors[category], alpha=0.7, s=50,
            ...
            # Plot training points
            ax.scatter(train_viz[:, 0], train_viz[:, 1], train_viz[:, 2],
                      c='lightgray', alpha=0.3, s=20, label='Training')
