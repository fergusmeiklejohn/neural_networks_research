...
def load_results():
    ...
    with open('../../experiments/01_physics_worlds/outputs/baseline_results/knn_analysis_results.json', 'r') as f:
        knn_results = json.load(f)
    
    # Load baseline results
    with open('../../experiments/01_physics_worlds/outputs/baseline_results/all_baselines_results.json', 'r') as f:
...
def create_interpolation_bar_chart(knn_results):
    ...
    bars1 = ax.bar(x - width, within_95_pcts, width, label='Within 95th percentile', 
                    color='skyblue', edgecolor='black', linewidth=1)
    bars2 = ax.bar(x, within_99_pcts, width, label='Within 99th percentile',
                    color='lightgreen', edgecolor='black', linewidth=1)
    bars3 = ax.bar(x + width, beyond_99_pcts, width, label='Beyond 99th percentile',
...
def create_distribution_breakdown(knn_results):
    ...
    for i, model in enumerate(models):
        ...
        bars = ax1.bar(x + offset, within_99_rates, width, 
    ...
    for i, model in enumerate(models):
        ...
        ax2.plot(dist_labels, distances, 'o-', label=model, 
...
def create_conceptual_diagram():
    ...
    ax1.scatter(jupiter_x, jupiter_y, c='green', marker='^', s=50, 
               label='Jupiter (test)', edgecolor='black', linewidth=1)
    ...
    ax2.scatter(test_x, test_y, c='green', marker='^', s=50, 
               label='Test (91.7% inside hull)', edgecolor='black', linewidth=1)
