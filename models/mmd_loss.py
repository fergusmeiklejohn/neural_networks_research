...
class MMDLoss(keras.losses.Loss):
    ...
    def compute_kernel_matrix(
        self, X: keras.KerasTensor, Y: keras.KerasTensor
    ) -> keras.KerasTensor:
        """
...
class ConditionalMMDLoss(keras.losses.Loss):
    ...
    def call(
        ...
        context: Optional[keras.KerasTensor] = None,
    ) -> keras.KerasTensor:
        """
        ...
        ) * output_mmd + self.context_weight * conditional_mmd


def create_physics_loss_with_mmd(
...
if __name__ == "__main__":
    ...
    pred_trajectories = true_trajectories + 0.1 * np.random.randn(
        batch_size, trajectory_length * features
    )
    ...
    composite_loss = create_physics_loss_with_mmd(
        keras.losses.MeanSquaredError(), mmd_weight=0.1
    )
