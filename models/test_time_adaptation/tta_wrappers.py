...
from .regression_tta import PhysicsRegressionTTA, RegressionTTA
from .regression_tta_v2 import PhysicsRegressionTTAV2, RegressionTTAV2
from .tent import TENT, PhysicsTENT
from .tent_regression import PhysicsTENTRegression, TENTRegression
...
class OnlinePhysicsAdapter:
    ...
    def predict_next(self, context: Any) -> Any:
        ...
        should_adapt = (
            self.prediction_count % self.adaptation_frequency == 0
            and len(self.observation_buffer) >= self.window_size
        )
        ...
        prediction = self.tta_adapter.predict_next(
            ops.stack(self.observation_buffer), adapt=False  # Already adapted above
        )
