...
def flip_trajectory_horizontal(trajectory: Any) -> Any:
...
def rotate_trajectory(trajectory: Any, angle: float = None) -> Any:
...
def add_gaussian_noise(trajectory: Any, noise_scale: float = 0.01) -> Any:
    ...
    Returns:
        Noisy trajectory
    """
    noise = ops.random.normal(shape=ops.shape(trajectory), mean=0.0, stddev=noise_scale)

    return trajectory + noise


def time_reversal(trajectory: Any) -> Any:
...
def interpolate_trajectory(trajectory: Any, factor: int = 2) -> Any:
    ...
    for i in range(1, factor):
        alpha = i / factor
        interpolated[:, i::factor] = (1 - alpha) * trajectory[
            :, :-1
        ] + alpha * trajectory[:, 1:]
...
def create_augmented_batch(
    trajectory: Any, augmentations: List[Callable], num_augmentations: int = 2
) -> Any:
