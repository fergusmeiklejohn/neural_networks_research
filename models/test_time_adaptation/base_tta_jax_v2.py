...
class BaseTTAJaxV2(ABC):
    ...
    def _copy_complete_state(self) -> Dict[str, List[np.ndarray]]:
        ...
        return {
            'model_trainable': [ops.convert_to_numpy(v).copy() for v in self.model.trainable_variables],
            'model_non_trainable': [ops.convert_to_numpy(v).copy() for v in self.model.non_trainable_variables],
            'optimizer_vars': [ops.convert_to_numpy(v).copy() for v in self.optimizer.variables]
        }
    ...
    def adapt_step_jax(self, x: Any) -> Tuple[Any, float]:
        ...
        trainable_vars = [ops.convert_to_numpy(v) for v in self.model.trainable_variables]
        non_trainable_vars = [ops.convert_to_numpy(v) for v in self.model.non_trainable_variables]
        ...
        for var, new_val in zip(self.model.non_trainable_variables, updated_non_trainable):
            var.assign(new_val)
