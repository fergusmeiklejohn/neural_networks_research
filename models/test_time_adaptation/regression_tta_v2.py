...
class RegressionTTAV2(BaseTTAJaxV2):
    ...
    def _setup_adaptable_params(self):
        ...
        if self.update_bn_only:
            ...
            self.adaptable_param_indices = list(range(len(self.model.trainable_variables)))
...
class PhysicsRegressionTTAV2(RegressionTTAV2):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            model, adaptation_steps, learning_rate, reset_after_batch,
            consistency_weight, smoothness_weight, update_bn_only, **kwargs
        )
    ...
    def compute_physics_constraints(self, x: Any, y_pred: Any) -> Any:
        ...
        if ops.shape(y_pred)[-1] >= 8:
            ...
            if ops.shape(x)[-1] >= 8:
                ...
                momentum_change = ops.mean(ops.square(total_momentum - total_momentum_in))
            ...
            if ops.shape(x)[-1] >= 8:
                vel1_in = x[..., 2:4]
                vel2_in = x[..., 6:8]
                ke_in = 0.5 * (ops.sum(vel1_in**2, axis=-1) + ops.sum(vel2_in**2, axis=-1))
            ...
            out_of_bounds = ops.maximum(ops.abs(pos1) - max_position, 0.0) + \
