...
class ERMWithAugmentation(BaselineModel):
    ...
    def train(self, train_data, val_data, epochs: int = 100):
        ...
        augmented_y = np.tile(
            train_data["y"], (len(self.augmentation_strategies) + 1, 1)
        )

        # Compile and train
        self.model.compile(
            optimizer="adam", loss=self.config.get("loss", "mse"), metrics=["accuracy"]
        )
    ...
    def adapt_to_modification(self, modification_request, examples=None):
        ...
        self.model.fit(examples["x"], examples["y"], epochs=10, verbose=0)
        return self
...
class GFlowNetBaseline(BaselineModel):
    ...
    def train(self, train_data, val_data, epochs: int = 100):
        ...
        for epoch in range(epochs):
            ...
            self.trajectory_balance_loss(trajectories)
...
class GraphExtrapolationBaseline(BaselineModel):
    ...
    def train(self, train_data, val_data, epochs: int = 100):
        ...
        for epoch in range(epochs):
            ...
            self.graph_aware_loss(predictions, train_data["y"], graph_structure)
...
class MAMLBaseline(BaselineModel):
        ...
        for epoch in range(epochs):
            ...
            for task in task_batch:
                ...
                adapted_weights = self.inner_loop_update(
                    self.model.get_weights(), support_data
                )
    ...
    def adapt_to_modification(self, modification_request, examples=None):
        ...
        adapted_weights = self.inner_loop_update(self.model.get_weights(), examples)
...
class BaselineEvaluator:
    ...
    def evaluate_all_models(
        self, models: Dict[str, BaselineModel], test_data, modifications
    ):
        ...
        for model_name, model in models.items():
            ...
            extrap_results = model.evaluate_extrapolation(
                test_data, self.representation_analyzer
            )
    ...
    def evaluate_modifications(self, model, modifications):
        ...
        for mod in modifications:
            ...
            adapted = model.adapt_to_modification(mod["request"], mod.get("examples"))
    ...
    def generate_report(self):
        ...
        for model_name, results in self.results.items():
            ...
            report += (
                f"  Modification Success: {results['modification_success']:.2%}\n\n"
            )
