...
class PhysicsInformedTrajectoryTransformer(keras.Model):
    ...
    def _build_components(self):
        ...
        self.physics_attention = PhysicsGuidedAttention(
            hidden_dim=self.transformer_dim, num_heads=4
        )
        ...
        self.physics_param_head = layers.Dense(
            4
        ...
        if self.use_physics_losses:
            self.physics_losses = PhysicsLosses(
                energy_weight=self.energy_weight, momentum_weight=self.momentum_weight
            )
    ...
    def compute_physics_features(self, physics_state, training=None):
        ...
        if self.use_hnn:
            ...
            positions_flat = ops.reshape(
                physics_state["positions"], (batch_size, seq_len, -1)
            )
            ...
            for t in range(seq_len):
                state_t = hnn_state[:, t]
                h_value = self.hnn(
                    {"state": state_t, "masses": physics_state["masses"]}
                )
    ...
    def call(self, inputs, training=None):
        ...
        for transformer_block in self.transformer_blocks:
            transformer_features = transformer_block(
                transformer_features, training=training
            )

        # Stage 3: Extract physics state and compute physics features
        physics_state = self.extract_physics_state(inputs)
        physics_features = self.compute_physics_features(
            physics_state, training=training
        )
        ...
        if physics_features is not None:
            fusion_input = ops.concatenate(
                [refined_features, physics_features], axis=-1
            )
    ...
    def compute_loss(self, inputs, targets, training=None):
        ...
        position_loss = ops.mean(
            ops.square(predictions["positions"] - targets["positions"])
        )
        velocity_loss = ops.mean(
            ops.square(predictions["velocities"] - targets["velocities"])
        )
        ...
        if "physics_params" in targets:
            param_loss = ops.mean(
                ops.square(predictions["physics_params"] - targets["physics_params"])
            )
        else:
            param_loss = 0.0
...
def test_physics_informed_transformer():
    ...
    inputs[:, :, 1:5] = np.random.uniform(
        100, 700, (batch_size, seq_len, 4)
    )  # Positions
    inputs[:, :, 5:9] = np.random.uniform(
        -50, 50, (batch_size, seq_len, 4)
    )  # Velocities
    ...
    targets = {
        ...
        + np.random.normal(0, 0.1, outputs["positions"].shape),
        "velocities": outputs["velocities"]
        + np.random.normal(0, 0.1, outputs["velocities"].shape),
        "physics_params": np.array(
            [[-981.0, 0.7, 0.8, 0.95]] * batch_size, dtype=np.float32
        ),
    }
