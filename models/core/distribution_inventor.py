...
class ConsistencyEnforcer(layers.Layer):
    ...
    def call(
        self, extracted_rules, modified_rules, generated_trajectory, training=None
    ):
        ...
        trajectory_input = ops.concatenate(
            [flat_trajectory, modified_combined], axis=-1
        )
        ...
        overall_consistency = self.consistency_network(
            consistency_input, training=training
        )
...
class DistributionInventor(keras.Model):
    ...
    def __init__(
        self, config: DistributionInventorConfig, vocab_size: int = 1000, **kwargs
    ):
        super().__init__(**kwargs)
        self.config = config
        ...
        self.distribution_modifier = DistributionModifier(
            config.modifier_config, vocab_size
        )
    ...
    def call(self, inputs, training=None):
        ...
        modification_output = self.distribution_modifier(
            modification_inputs, training=training
        )
        ...
        generation_output = self.trajectory_generator(
            generation_inputs, training=training
        )
        ...
        trajectory_positions = generation_output["trajectory"][
            :, :, :, 1:3
        ]  # Extract positions
        flattened_positions = ops.reshape(
            trajectory_positions, [batch_size, -1]
        )  # Flatten positions
    ...
    def invent_distribution(
        ...
        modification_tokens = self.distribution_modifier._tokenize_request(
        ...
        if return_details:
            result.update(
                {
                    "original_rules": {
                        k: np.array(v)[0]
                        for k, v in outputs["extracted_rules"].items()
                        if k
                        not in ["independence_score", "consistency_score", "features"]
                    },
                    "modified_rules": {
                        k: np.array(v)[0]
                        for k, v in outputs["modified_rules"].items()
                        if k not in ["deltas", "weights"]
                    },
                    "quality_scores": {
                        "overall_consistency": np.array(
                            outputs["consistency_scores"]["overall_consistency"]
                        )[0],
                        "rule_coherence": np.array(
                            outputs["consistency_scores"]["rule_coherence"]
                        )[0],
                        "trajectory_plausibility": np.array(
                            outputs["consistency_scores"]["trajectory_plausibility"]
                        )[0],
                        "modification_novelty": np.array(
                            outputs["modification_novelty"]
                        )[0],
                        "trajectory_quality": np.array(outputs["trajectory_quality"])[
                            0
                        ],
                    },
            ...
            result["success"] = (
                result["quality_scores"]["overall_consistency"] > min_consistency
                and result["quality_scores"]["trajectory_quality"] > min_quality
            )

        return result

    def analyze_modification_space(
        self,
        base_trajectory: np.ndarray,
        ...
        for request in modification_requests:
            try:
                ...
                results.append(
                    {"modification_applied": request, "success": False, "error": str(e)}
                )
        ...
        if successful_mods:
            ...
            analysis["average_quality"] = {
                key: np.mean([q[key] for q in qualities]) for key in qualities[0].keys()
            }
...
class DistributionInventorLoss(keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        extraction_loss = keras.losses.mse(
            y_true.get(
                "true_rules", ops.zeros_like(y_pred["extracted_rules"]["gravity"])
            ),
        ...
        generation_loss = keras.losses.mse(
            y_true.get(
                "target_trajectory", ops.zeros_like(y_pred["generated_trajectory"])
            ),
            y_pred["generated_trajectory"],
        )

        # Consistency losses
        consistency_target = ops.ones_like(
            y_pred["consistency_scores"]["overall_consistency"]
        )
        consistency_loss = keras.losses.binary_crossentropy(
            consistency_target, y_pred["consistency_scores"]["overall_consistency"]
        )

        # Novelty loss (encourage appropriate novelty)
        novelty_target = self.config.novelty_target * ops.ones_like(
            y_pred["modification_novelty"]
        )
...
def create_distribution_inventor(
    config: Optional[DistributionInventorConfig] = None, vocab_size: int = 1000
...
if __name__ == "__main__":
    ...
    result = model.invent_distribution(
        test_trajectory, "increase gravity by 20%", test_initial
    )
