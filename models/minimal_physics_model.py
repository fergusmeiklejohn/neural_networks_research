...
class MinimalPhysicsModel(keras.Model):
    ...
    def extract_physics_features(self, states):
        ...
        v_theta1 = (pos1[..., 0:1] * vel1[..., 1:2] - pos1[..., 1:2] * vel1[..., 0:1]) / (r1**2 + 1e-6)
        
        v_r2 = ops.sum(pos2 * vel2, axis=-1, keepdims=True) / (r2 + 1e-6)
        v_theta2 = (pos2[..., 0:1] * vel2[..., 1:2] - pos2[..., 1:2] * vel2[..., 0:1]) / (r2**2 + 1e-6)
        ...
        features = ops.concatenate([
            r1, theta1, v_r1, v_theta1, L1,
            r2, theta2, v_r2, v_theta2, L2,
            r12
        ], axis=-1)
    ...
    def compute_accelerations(self, states):
        ...
        acc1_base = ops.stack([
            ...
            pixel_gravity * ops.ones_like(pos1[..., 1])  # y acceleration = g (in pixels)
        ], axis=-1)
        
        acc2_base = ops.stack([
            ops.zeros_like(pos2[..., 0]),
            pixel_gravity * ops.ones_like(pos2[..., 1])
        ], axis=-1)
    ...
    def integrate_trajectory(self, initial_state, steps=50):
        ...
        for _ in range(steps):
            ...
            # Combine new state
            new_state = ops.concatenate([new_pos1, new_pos2, new_vel1, new_vel2], axis=-1)
            state = new_state
    ...
    def call(self, inputs, training=None):
        ...
        # Integrate forward
        predicted_trajectory = self.integrate_trajectory(initial_state, steps=inputs.shape[1])
    ...
    def compute_physics_losses(self, y_true, y_pred):
        ...
        _, (pos1_true, pos2_true, vel1_true, vel2_true) = self.extract_physics_features(y_true)
        _, (pos1_pred, pos2_pred, vel1_pred, vel2_pred) = self.extract_physics_features(y_pred)
        
        # Kinetic energy
        KE_true = 0.5 * (ops.sum(vel1_true**2, axis=-1) + ops.sum(vel2_true**2, axis=-1))
        KE_pred = 0.5 * (ops.sum(vel1_pred**2, axis=-1) + ops.sum(vel2_pred**2, axis=-1))
