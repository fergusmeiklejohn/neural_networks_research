...
class SimplePhysicsInformedModel(keras.Model):
    ...
    def _build_layers(self):
        ...
        for i in range(self.num_layers):
            self.lstm_layers.append(layers.LSTM(self.hidden_dim, return_sequences=True))
    ...
    def compute_loss(self, inputs, targets, training=None):
        ...
        traj_loss = ops.mean(
            ops.square(predictions["trajectory"] - targets["trajectory"])
        )
...
def test_physics_model():
    ...
    targets = {
        ...
        "velocities": outputs["velocities"].numpy()
        + np.random.normal(0, 0.1, outputs["velocities"].shape),
        "physics_params": np.array(
            [[-9.8, 0.5, 0.8, 0.95]] * batch_size, dtype=np.float32
        ),
    }
